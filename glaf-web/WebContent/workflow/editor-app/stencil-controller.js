...
angular.module('activitiModeler')
    .controller('StencilController', ['$rootScope', '$scope', '$http', '$modal', '$timeout', function ($rootScope, $scope, $http, $modal, $timeout) {
    	...
    	// add by dane 2015-10-08
    	if(definedType=='systemplan'){
			$scope.propertyWindowState = {'hide': false,'collapsed': true};
		}
		else if(definedType=='workflow')
        {
        ...
        $scope.editorFactory.promise.then(function () {
            ...
            $scope.editor.registerOnEvent(ORYX.CONFIG.EVENT_SELECTION_CHANGED, function (event) {
                ...
                if (shapes && shapes.length > 0) {
                    ...
                    //add by dane 2015-10-08
                    var stencilItem = $scope.stencilItemGroups?$scope.getStencilItemById(selectedShape.getStencil().idWithoutNs()):null;
        			if(definedType=='workflow'){
					var $$ = parent.$;
					var $frame=$$("iframe").eq(1);
					var timestamp=new Date().getTime();
					var endstamp;
					while($frame.get(0)&&$frame.data("locked")){
						endstamp=new Date().getTime();
						if((endstamp-timestamp)/1000>2){
							$frame.data("locked", false);
						}
					}
					if($frame.get(0)&&!$frame.data("locked")){
						$frame.data("locked", true);
						$frame.attr({
							onload : "loadFunc(this)"
						})
                    }
					
					if(parent.$("iframe")[1]&&parent.$("iframe")[1].contentDocument.propform)
					{
						parent.$("iframe")[1].contentWindow.autoSaveTaskProperties();
					}
        			if(stencilItem){
            			//获取任务类型
                    	var taskType=stencilItem.id;
                    	//获取节点名称和ID
                        var taskName=selectedShape.properties['oryx-name'];
                        var taskId=selectedShape.properties['oryx-overrideid'];
                        var description=selectedShape.properties['oryx-documentation'];
                        var resourceId=$rootScope.editor.getSelection()[0].resourceId;
                        //显示属性配置页
                        parent.openPropertiesEdit("/mx/form/workflow/nodeDefined/view?elemType="+taskType+"&taskId="+taskId+"&taskName="+encodeURIComponent(encodeURIComponent(taskName))+"&description="+encodeURIComponent(encodeURIComponent(description))+"&modelId="+$scope.editor.id+"&resourceId="+resourceId);
        			}
        			else{
        				//获取节点名称和ID
                        var taskName=selectedShape.properties['oryx-name'];
                        var taskId=selectedShape.properties['oryx-process_id'];
                        var description=selectedShape.properties['oryx-documentation'];
                        //获取模型名称和关键字
                        if(modelKey&&modelKey!=""&&taskId=='process'){
                        	taskId=modelKey;
                        }
                        if(modelName&&modelName!=""&&taskName==''){
                        	taskName=decodeURIComponent(modelName);
                        }
                        var resourceId=selectedShape.resourceId;
                        parent.openPropertiesEdit("/mx/form/workflow/nodeDefined/view?elemType=workflow&taskId="+taskId+"&taskName="+encodeURIComponent(encodeURIComponent(taskName))+"&description="+encodeURIComponent(encodeURIComponent(description))+"&modelId="+$scope.editor.id+"&resourceId="+resourceId);
        			}
        			}
            ...
            $scope.editor.registerOnEvent(ORYX.CONFIG.EVENT_SELECTION_CHANGED, function (event) {
                ...
                if (shapes && shapes.length == 1) {
        			...
        			if (morphShapes && morphShapes.length > 0) {
	        			...
	        			//dane 2017-02-16 目前设计器在系统规划功能中使用时隐藏配置按钮
						if(definedType!='systemplan'){
        			...
        			var ruleSetButton = document.getElementById('ruleSet-button');
        			ruleSetButton.style.display = "block";
        			ruleSetButton.style.left = x + 48 +'px';
        			ruleSetButton.style.top = (shapeXY.y+bounds.height() + 2) + 'px';
        			...
        			if (stencilItem && (stencilItem.canConnect || stencilItem.canConnectAssociation)) {
	        			...
	        			jQuery('.Oryx_button').each(function(i, obj) {
	        				if (obj.id !== 'morph-button' && obj.id != 'delete-button'&&obj.id != 'ruleSet-button') {
	        					quickButtonCounter++;
            ...
            $scope.ruleSet = function() {
            	$scope.safeApply(function () {     		
            		...
            		if (shapes && shapes.length == 1)
            		{
            			...
            			var stencilItem = $scope.getStencilItemById($rootScope.currentSelectedShape.getStencil().idWithoutNs());
            			// Method to open shape select dialog (used later on)
                        var showRuleSetDialog = function()
                        {
                        	var pageId=parent.pageId;
                        	//获取任务类型
                        	var taskType=stencilItem.id;
                        	//获取节点名称和ID
                            var taskName=$rootScope.currentSelectedShape.properties['oryx-name'];
                            var taskId=$rootScope.currentSelectedShape.properties['oryx-overrideid'];
                            if(taskType=='ComponentUpdateTask')
                           {
	                            var url ='mx/page/workflow/componentRuleSetting?pageId=' +pageId+'&taskId='+taskId+'&taskName='+taskName;
	                            var iWidth = "90%";
 	                            var iHeight = "90%";
	                            //var iTop = (window.screen.availHeight - 30 - iHeight) / 2;
	                            //var iLeft = (window.screen.availWidth - 10 - iWidth) / 2;
	                           // var win = window.open(url, "构件更新规则设置", "width=" + iWidth + ", height=" + iHeight + ",top=" + iTop + ",left=" + iLeft + ",toolbar=no, menubar=no, scrollbars=no, resizable=no,location=no, status=no,alwaysRaised=yes,depended=yes");
	                            parent.openRuleSetDialog(url,"构件更新规则设置",iWidth,iHeight,true); 
                           }
                            else if(taskType=='DataOperationTask')
                        	{
                            	var url ='mx/dataset/sqlbuilder?id=96bcc739440144a7b80adac679028289&pageId=' +pageId+'&taskId='+taskId+'&taskName='+taskName;
 	                            var iWidth = "90%";
 	                            var iHeight = "95%";
 	                            //var iTop = (window.screen.availHeight - 30 - iHeight) / 2;
 	                            //var iLeft = (window.screen.availWidth - 10 - iWidth) / 2;
 	                            //var win = window.open(url, "数据操作规则设置", "width=" + iWidth + ", height=" + iHeight + ",top=" + iTop + ",left=" + iLeft + ",toolbar=no, menubar=no, scrollbars=no, resizable=no,location=no, status=no,directories=no,alwaysRaised=yes,depended=yes");
 	                           parent.openRuleSetDialog(url,"构件更新规则设置",iWidth,iHeight,true); 
                        	}
                        };
            ...
            $scope.deleteShape = function() {
              if(confirm("删除节点的同时构件的更新规则也将一同被删除，确认继续吗？"))
             {
