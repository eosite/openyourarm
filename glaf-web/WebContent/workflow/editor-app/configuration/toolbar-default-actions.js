...
KISBPM.TOOLBAR = {
    ACTIONS: {
        ...
        closeEditor: function(services) {
        	if(definedType!=""){
        		if(parent.parent.$("#addDialog"))
        		parent.parent.$("#addDialog").data("kendoWindow").close();
    		}
    		else
    		{
...
var SaveModelCtrl = [ '$rootScope', '$scope', '$http', '$route', '$location',
    ...
    $scope.save = function (successCallback) {
        ...
        // Parse dom to string 2015-08-26 dane 添加了IE11 bug问题
        ...
        $http({    method: 'PUT',
            ...
            .success(function (data, status, headers, config) {
            	...
            	//add by dane 2015-08-28 回调更新发布状态为未发布
                if(definedType=='pageflow')
				{
					parent.updateDeployStatus();
				}
                else if(definedType=='systemplan'){
					parent.saveSystemPlanActivitiSource(modelMetaData.modelId);
				}else{
                   parent.updateActivitiSource();
				}
                // Reset state
