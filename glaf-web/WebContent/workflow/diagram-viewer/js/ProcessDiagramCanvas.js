...
// add by dane 设置选中颜色
var SELECTED_HIGHLIGHT_COLOR = Color.get(205, 56, 142);
...
// TODO: generate polylineId or do something!!
		...
		polyClone.click(function(){
			...
			if (contextObject) {
				...
				ProcessDiagramGenerator.showFlowInfo(contextObject);
				 if (ProcessDiagramGenerator.options && ProcessDiagramGenerator.options.flowon && ProcessDiagramGenerator.options.flowon.click) {
					    var args = [instance, polyline.element, contextObject];
					    ProcessDiagramGenerator.options.flowon.click.apply(event, args);
					  }
			}
		}).dblclick(function(){
	...
	*/
	...
	//add by dane 2015-09-22 设置选中节点高亮
	selectedHighLightActivity: function(activityId){
		var shape = this.g.getById(activityId);
		if (!shape) {
			console.error("Activity " + activityId + " not found");
			return;
		...
		if (contextObject)
			...
			console.log("--> highLightActivity: ", shape, shape.data("contextObject"));
		shape.attr("stroke-width", THICK_TASK_BORDER_STROKE);
		shape.attr("stroke", SELECTED_HIGHLIGHT_COLOR);
	},
	//add by dane 2015-09-22 设置选中节点高亮
	selectedHighLightFlow: function(flowId){
		var shapeFlow = this.g.getById(flowId);
		if (!shapeFlow) {
			console.error("Flow " + flowId + " not found");
			return;
		...
		st.attr("stroke-width", SEQUENCEFLOW_HIGHLIGHT_STROKE);
		st.attr("stroke", SELECTED_HIGHLIGHT_COLOR);
		var withArrowHead = shapeFlow.data("withArrowHead");
		if (withArrowHead)
			st[1].attr("fill", SELECTED_HIGHLIGHT_COLOR);
	...
	//add by dane 2015-09-23 取消之前选中节点高亮
	cancelHighLight:function(oldElementId){
		var oldElement=this.g.getById(oldElementId);
		if(oldElement)
		{
			if(oldElement.type=='path')
		  {
			var oldst = oldElement.data("set");
			if(oldst){
				oldst.attr("stroke-width", SEQUENCEFLOW_STROKE);
				oldst.attr("stroke", SEQUENCEFLOW_COLOR);
			}
			var oldwithArrowHead = oldElement.data("withArrowHead");
			if (oldwithArrowHead)
				oldst[1].attr("fill", SEQUENCEFLOW_COLOR);
		  }
			else{
				oldElement.attr("stroke-width", NORMAL_STROKE);
				oldElement.attr("stroke", TASK_STROKE_COLOR);
			}
		}
	},
