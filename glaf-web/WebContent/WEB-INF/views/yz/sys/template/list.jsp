...
function createTemplateId(){
    location.href="<%=request.getContextPath()%>/mx/isdp/sys/template/edit?nodeId=${nodeId}";
}

function modifyTemplate(templateId){
    location.href="<%=request.getContextPath()%>/mx/isdp/sys/template/edit?templateId="+templateId+"&nodeId=${nodeId}";
}
	...
	<c:forEach items="${templates}" var="template">
		...
		<tr onmouseover="this.className='x-row-over';"
			...
			<td align="left" noWrap>&nbsp; <a
				href="<%=request.getContextPath()%>/mx/isdp/sys/template/view?templateId=${template.templateId}"> ${template.name}</a></td>
