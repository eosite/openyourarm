...
<script type="text/javascript" src="<%=contextPath%>/scripts/glaf-base.js"></script>
 ...
 function chooseFiledot(formName, elementId, elementName){
	    var x_selected =  document.getElementById(elementId);
	    var url="<%=request.getContextPath()%>/mx/isdp/filedot?method=choose&formName="+formName+"&elementId="+elementId+"&elementName="+elementName;
...
<form name="iForm" method="post" ENCTYPE="multipart/form-data"
	class="x-form"
	action="<%=request.getContextPath()%>/mx/isdp/sys/template/save"
...
<table border=0 cellspacing=0 cellpadding=2>
	<tbody>
		...
		<tr class="x-content-hight">
			<td align="left" width="18%">关联CELL模板</td>
			<td align="left" width="81%">
				<input name="objectId" id="objectId" type="hidden" size="50" class="input-xlarge x-text" maxlength="255" value="${template.objectId}">
				<input name="objectValue" id="objectValue" type="text" size="50" class="input-xlarge x-text" maxlength="255" value="${template.objectValue}" onclick="javascript:chooseFiledot('iform','objectId','objectValue');">
			</td>
		</tr>
