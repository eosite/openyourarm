...
<script type="text/javascript">
	...
	function searchKeyWord(pageNumber){
		...
		jQuery("#dataTable").datagrid({
			url:"<%=request.getContextPath()%>/rs/viewReportRest/searchKeyWord",
			queryParams:{"useDatabase":"<%=useDatabase%>","searchFlag":searchFlag,"searchKeyWord":searchKeyWord,"pageNumber":pageNumber,"pageSize":pageSize,"searchType":jQuery("#searchType").combobox("getValue"),"timeInput":jQuery("#timeInput").datebox("getValue"),"expression":jQuery("#expression").combobox("getValue")},
			...
			onDblClickRow:function(rowIndex, row){
				...
				window.showModalDialog("<%=request.getContextPath()%>/mx/docManage/yz/fileview/viewReport/reportTableList?useDatabase=<%=useDatabase%>&inspectionId="+row.id_enc,"viewFile","dialogHeight="+height+";dialogWidth="+width+";center=true;status=no;scroll=yes");
			}
		});
	}
