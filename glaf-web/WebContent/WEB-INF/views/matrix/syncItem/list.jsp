...
<script type="text/javascript">
   ...
   jQuery(function(){
		jQuery('#mydatagrid').datagrid({
				...
				striped: true,
				collapsible: true,
				url: '<%=request.getContextPath()%>/mx/sys/syncItem/json?syncId=${syncId}',
				remoteSort: false,
				singleSelect: true,
				idField: 'id',
				columns:[[
	...
	function addNew(){
	    var link="<%=request.getContextPath()%>/mx/sys/syncItem/edit?syncId=${syncId}";
	    jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function onRowClick(rowIndex, row){
	    var link = '<%=request.getContextPath()%>/mx/sys/syncItem/edit?id='+row.id;
	    jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function editSelected(){
	    ...
	    if (selected ){
		  var link = '<%=request.getContextPath()%>/mx/sys/syncItem/edit?id='+selected.id;
		  jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function deleteSelections(){
		...
		if(ids.length > 0 ){
		  if(confirm("数据删除后不能恢复，确定删除吗？")){
			...
			jQuery.ajax({
				   type: "POST",
				   url: '<%=request.getContextPath()%>/mx/sys/syncItem/delete?ids='+str,
				   dataType:  'json',
				   error: function(data){
					   alert('服务器处理错误！');
				   },
