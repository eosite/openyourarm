...
<script type="text/javascript">
	...
	function saveData(){
		...
		jQuery.ajax({
				   type: "POST",
				   url: '<%=request.getContextPath()%>/mx/sys/combinationApp/save',
				   data: params,
				   dataType: 'json',
				   error: function(data){
					   alert('服务器处理错误！');
	...
	function saveAsData(){
		...
		jQuery.ajax({
				   type: "POST",
				   url: '<%=request.getContextPath()%>/mx/sys/combinationApp/saveAs',
				   data: params,
				   dataType: 'json',
				   error: function(data){
					   alert('服务器处理错误！');
	...
	<tr>
		...
		<td align="left">
			<select id="srcDatabaseId" name="srcDatabaseId">
				...
				<option value="0">来源数据在各目标数据库</option>
