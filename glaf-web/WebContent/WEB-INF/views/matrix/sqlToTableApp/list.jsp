...
<script type="text/javascript">
   ...
   jQuery(function(){
		jQuery('#mydatagrid').datagrid({
				...
				striped: true,
				collapsible: true,
				url: '<%=request.getContextPath()%>/mx/sys/sqlToTableApp/json',
				remoteSort: false,
				singleSelect: true,
				idField: 'id',
				columns:[[
	...
	function columnList(id){
		var link = '<%=request.getContextPath()%>/mx/sys/tableMgr/extColumns?tableId=sqltotable_app_'+id;
	    jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function dataHandler(id){
        var link = '<%=request.getContextPath()%>/mx/sys/syntheticFlow/edit?currentStep=sqltotable_app_'+id+'&code=sqltotable_app';
	    jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
    ...
    function dropTable(id){
	  if(confirm("数据删除后不能恢复,确定重建目标表吗?")){
		var link = "<%=request.getContextPath()%>/mx/sys/sqlToTableApp/dropTable?id="+id;
		jQuery.ajax({
                       type: "POST",
                       url: link,
                       dataType: 'json',
    ...
    function syncList(syncId){
	    var link = '<%=request.getContextPath()%>/mx/sys/sqlToTableItem?syncId='+syncId;
	    jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function historyList(syncId){
	    var link = '<%=request.getContextPath()%>/mx/sys/sqlToTableHistory?syncId='+syncId;
	    jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function editRow(id){
	    var link="<%=request.getContextPath()%>/mx/sys/sqlToTableApp/edit?id="+id;
	    jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function addNew(){
	    var link="<%=request.getContextPath()%>/mx/sys/sqlToTableApp/edit";
	    jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function onRowClick(rowIndex, row){
	    var link = '<%=request.getContextPath()%>/mx/sys/sqlToTableApp/edit?id='+row.id;
	    jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function searchWin(){
	    jQuery('#dlg').dialog('open').dialog('setTitle','多源数据表查询');
	...
	function editSelected(){
	    ...
	    if (selected ){
		  var link = '<%=request.getContextPath()%>/mx/sys/sqlToTableApp/edit?id='+selected.id;
		  jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function deleteSelections(){
		...
		if(ids.length > 0 ){
		  if(confirm("数据删除后不能恢复，确定删除吗？")){
			...
			jQuery.ajax({
				   type: "POST",
				   url: '<%=request.getContextPath()%>/mx/sys/sqlToTableApp/delete?ids='+str,
				   dataType:  'json',
				   error: function(data){
					   alert('服务器处理错误！');
				   },
	...
	function execute(){
	    ...
	    if (selected && confirm("确定要在目标库上执行同步操作吗？")){
		    ...
		    jQuery.ajax({
				   type: "POST",
				   url: '<%=request.getContextPath()%>/mx/sys/sqlToTableApp/execute?syncId='+syncId,
				   dataType: 'json',
				   error: function(data){
					   alert('服务器处理错误！');
				   },
...
<div class="easyui-layout" data-options="fit:true">  
   <div data-options="region:'north',split:true,border:true"> 
	...
	&nbsp;<span class="x_content_title">多源数据表列表</span>
