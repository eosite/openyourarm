...
<%@ page import="com.glaf.matrix.data.domain.*"%>
...
<%
	    ...
	    List list = (List)request.getAttribute("sqlCriterias");
	    SqlCriteria sqlCriteria = (SqlCriteria)request.getAttribute("sqlCriteria");
...
<script type="text/javascript">
	...
	function saveData(){
		...
		jQuery.ajax({
				   type: "POST",
				   url: '<%=request.getContextPath()%>/mx/sys/sqlCriteria/saveSqlCriteria',
				   data: params,
				   dataType:  'json',
				   error: function(data){
					   alert('服务器处理错误！');
        ...
        <td>
          ...
          <%
			if(list!=null){
			  ...
			  while(iter.hasNext()){
				SqlCriteria bean2=(SqlCriteria)iter.next();
				if(sqlCriteria != null){
					if(sqlCriteria.getId() == bean2.getId() || sqlCriteria.getId() == bean2.getParentId()){
						continue;
					}
				}
			%>
	...
	<tr>
		...
		<td align="left">
            <input id="tableAlias" name="tableAlias" type="text" 
			       class="easyui-validatebox  x-text"  
				   value="${sqlCriteria.tableAlias}" size="50"/>
		   <br>
		</td>
	</tr>
	<tr>
		...
		<td align="left">
            <input id="columnName" name="columnName" type="text" 
			       class="easyui-validatebox  x-text"  
				   value="${sqlCriteria.columnName}" size="50"/>
		   <br>
		</td>
	</tr>
    <tr>
	...
	<tr>
		...
		<td align="left">
		  <input type="radio" name="requiredFlag" value="Y" 
		         <c:if test="${sqlCriteria.requiredFlag == 'Y'}">checked</c:if>>是&nbsp;&nbsp;
	      <input type="radio" name="requiredFlag" value="N" 
		         <c:if test="${sqlCriteria.requiredFlag == 'N'}">checked</c:if>>否
	...
	<tr>
		...
		<td align="left">
		  <input type="radio" name="collectionFlag" value="Y" onclick="javascript:dispLayer('Y');";
		         <c:if test="${sqlCriteria.collectionFlag == 'Y'}">checked</c:if>>是&nbsp;&nbsp;
	      <input type="radio" name="collectionFlag" value="N" onclick="javascript:dispLayer('N');"
		         <c:if test="${sqlCriteria.collectionFlag == 'N'}">checked</c:if>>否
  ...
  <tr id="layer_sql" style="display:none;">
    ...
    <td align="left">
        <textarea id="sql" name="sql" rows="12" cols="60" class="x-textarea"
				  style="width:320px;height:120px;" >${sqlCriteria.sql}</textarea>&nbsp;
	...
	<tr>
		...
		<td align="left">
		  <input type="radio" name="locked" value="0" <c:if test="${sqlCriteria.locked == 0}">checked</c:if>>启用&nbsp;&nbsp;
	      <input type="radio" name="locked" value="1" <c:if test="${sqlCriteria.locked == 1}">checked</c:if>>禁用
		</td>
	</tr>
...
<script type="text/javascript">
 <c:if test="${!empty sqlCriteria.sql}">
    jQuery("#layer_sql").show();
 </c:if>
</script>
</body>
