...
<script type="text/javascript">
   ...
   jQuery(function(){
		jQuery('#mydatagrid').datagrid({
				...
				striped: true,
				collapsible: true,
				url: '${contextPath}/mx/sys/tableCorrelation/json?tableId=${tableId}',
				remoteSort: false,
				singleSelect: true,
				idField: 'id',
				columns:[[
	...
	function addNew(){
	    var link="${contextPath}/mx/sys/tableCorrelation/edit?tableId=${tableId}";
		jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function editRow(id){
	    var link="${contextPath}/mx/sys/tableCorrelation/edit?tableId=${tableId}&id="+id;
		jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function onMyRowClick(rowIndex, row){
	    var link = '${contextPath}/mx/sys/tableCorrelation/edit?id='+row.id;
	    jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function editSelected(){
	    ...
	    if (selected ){
		  var link = '${contextPath}/mx/sys/tableCorrelation/edit?id='+selected.id;
		  jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function viewSelected(){
		...
		var selected = jQuery('#mydatagrid').datagrid('getSelected');
		if (selected ){
		    var link='${contextPath}/mx/sys/tableCorrelation/edit?readonly=true&id='+selected.id;
		    jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function deleteById(id){
		  if(confirm("数据删除后不能恢复，确定删除吗？")){
			jQuery.ajax({
				   type: "POST",
				   url: '${contextPath}/mx/sys/tableCorrelation/delete?id='+id,
				   dataType:  'json',
				   error: function(data){
					   alert('服务器处理错误！');
				   },
	...
	function deleteSelections(){
		...
		if(ids.length > 0 ){
		  if(confirm("数据删除后不能恢复，确定删除吗？")){
			...
			jQuery.ajax({
				   type: "POST",
				   url: '${contextPath}/mx/sys/tableCorrelation/delete?ids='+str,
				   dataType:  'json',
				   error: function(data){
					   alert('服务器处理错误！');
				   },
	...
	function searchData(){
        ...
        jQuery.ajax({
                    type: "POST",
                    url: '${contextPath}/mx/sys/tableCorrelation/json',
                    dataType:  'json',
                    data: params,
                    error: function(data){
                              alert('服务器处理错误！');
