...
<script type="text/javascript">
	...
	function saveData(){
		...
		jQuery.ajax({
				   type: "POST",
				   url: '${contextPath}/mx/sys/tableDataItem/saveTableDataItem',
				   data: params,
				   dataType:  'json',
				   error: function(data){
					   alert('服务器处理错误！');
	...
	function switchTable(){
	   ...
	   if(tableName != ""){
		   var link = "${contextPath}/mx/sys/tableMgr/columnsJsonArray?tableName="+tableName;
...
<div class="easyui-layout" data-options="fit:true">  
  ...
  <table class="easyui-form" style="width:600px;" align="center">
	...
	<tr>
		...
		<td align="left">
		    <select id="sortColumn" name="sortColumn">
				<c:forEach var="column" items="${columns}" >
				<option value="${column.sortColumn}">${column.title}</option>
				</c:forEach>
		    </select>
            <script type="text/javascript">
	...
	<tr>
		...
		<td align="left">
		  <input type="radio" name="locked" value="0" <c:if test="${tableDataItem.locked == 0}">checked</c:if>>是&nbsp;&nbsp;
	      <input type="radio" name="locked" value="1" <c:if test="${tableDataItem.locked == 1}">checked</c:if>>否&nbsp;&nbsp;
		</td>
	</tr>
    </tbody>
  </table>
