...
<script type="text/javascript">
   ...
   jQuery(function(){
		jQuery('#mydatagrid').datagrid({
				...
				striped: true,
				collapsible: true,
				url: '${contextPath}/mx/sys/tableDataItem/json',
				remoteSort: false,
				singleSelect: true,
				idField: 'id',
				columns:[[
	...
	function editRow(rowId){
	    var link = '${contextPath}/mx/sys/tableDataItem/edit?id='+rowId;
	    jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function deleteRow(rowId){
		 if(confirm("数据删除后不能恢复，确定删除吗？")){
			jQuery.ajax({
				   type: "POST",
				   url: '${contextPath}/mx/sys/tableDataItem/delete?id='+rowId,
				   dataType:  'json',
				   error: function(data){
					   alert('服务器处理错误！');
				   },
	...
	function addNew(){
	    var link="${contextPath}/mx/sys/tableDataItem/edit";
		jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function onRowClick(rowIndex, row){
	    var link = '${contextPath}/mx/sys/tableDataItem/edit?id='+row.id;
	    jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function editSelected(){
	    ...
	    if (selected ){
		  var link = '${contextPath}/mx/sys/tableDataItem/edit?id='+selected.id;
		  jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function viewSelected(){
		...
		var selected = jQuery('#mydatagrid').datagrid('getSelected');
		if (selected ){
		    var link='${contextPath}/mx/sys/tableDataItem/edit?readonly=true&id='+selected.id;
		    jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function deleteSelections(){
		...
		if(ids.length > 0 ){
		  if(confirm("数据删除后不能恢复，确定删除吗？")){
			...
			jQuery.ajax({
				   type: "POST",
				   url: '${contextPath}/mx/sys/tableDataItem/delete?ids='+str,
				   dataType:  'json',
				   error: function(data){
					   alert('服务器处理错误！');
				   },
