...
<link rel="stylesheet" type="text/css" href="${contextPath}/css/core.css">
...
<script type="text/javascript">
    jQuery(function(){
		jQuery('#taskTreeGrid').treegrid({
				...
				fitColumns:true,
				fit:true,
				url:'${contextPath}/mx/isdp/task/json?taskType=<%=taskType%>&databaseId=${databaseId}',
				idField:'indexId',
				treeField:'text',
	  ...
	  function onMyDblClickCell(index,field,value){
		   ...
		   if(index == "process"){
              ...
              window.open("${contextPath}/mx/isdp/wbs/check/list?databaseId=${databaseId}&activityId="+field.activityId);
		   }
	  }
	  ...
	  function openChkWbs(activityId){
		    window.open("${contextPath}/mx/isdp/wbs/check/list?databaseId=${databaseId}&activityId="+activityId)
	  }
	  ...
	  function searchTask(){
		   ...
		   var orderCondition = jQuery('#orderCondition').val();
		   jQuery.cookie('isdp_taskList_orderCondition', orderCondition);
		   var url = '${contextPath}/mx/isdp/task/json?databaseId=${databaseId}&taskType=<%=taskType%>&orderCondition='+orderCondition;
		   url = url+'&keyword='+keyword+'&page='+page;
           jQuery.get(url,{qq:'1'},function(data){
		      //var text = JSON.stringify(data); 
              //alert(text);
			  jQuery('#taskTreeGrid').treegrid('loadData', data);
		  },'json');
	  }
	  ...
	  function changeTaskOrder(){
		 var orderCondition = jQuery("#orderCondition").val();
		 jQuery.cookie('isdp_taskList_orderCondition', orderCondition);
         location.href="${contextPath}/mx/isdp/task/taskList?databaseId=${databaseId}&taskType=<%=taskType%>&orderCondition="+orderCondition;
	  }
   ...
   <div style="background:#fafafa;padding:2px;border:1px solid #ddd;font-size:12px"> 
	<table width="100%" height="99%">
		<tr>
			...
			<input type="text" id="keyword" name="keyword" size="50" class="x-searchtext">&nbsp;
			<input type="button" value="检索" class="btn btnGray" onclick="javascript:searchTask();">&nbsp;
			查看条件&nbsp;
