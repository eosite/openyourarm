...
var whereCondition={QUALITY_QUESTION_TYPE:"${param.type}"};
$(function() {
	//设置图表高度及宽度
	$("#chart").width(width/2-20);
	$("#chart").height(height/2-50);
	
	$("#table").width(width/2-20);
...
function createGrid(){
	...
	$("#grid").kendoGrid({
        dataSource: {
            ...
            transport: {
                ...
                type: "POST",
                read: "${pageContext.request.contextPath}/rs/isdp/collect/datalist?tableName=${param.tableName}&whereCondition="+encodeURI(JSON.stringify(whereCondition))
            },
            schema:{
            	total:"total",
            	data:"rows",
	...
	datasource.fetch(function(){
		...
		if(state==0){
			$("#noInput").val("未闭合");
			$("#noInput").attr("style","color:red;");
		}else{
			$("#noInput").val("闭合");
		}
		$("#allInput").val(all_total+"个");
...
function queryCharts(){
	...
	chart.setOptions({
		...
		dataSource: {
            transport: {
                read: {
                    url: "${pageContext.request.contextPath}/rs/isdp/collect/charts?dataType=list&tableName=${param.tableName}&whereCondition="+encodeURI(JSON.stringify(whereCondition)),
                    dataType: "json"
                }
            }
        }
