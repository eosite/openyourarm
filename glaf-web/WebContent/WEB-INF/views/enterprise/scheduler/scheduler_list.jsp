...
function createScheduler() {
    location.href="<%=request.getContextPath()%>/mx/enterprise/scheduler/showModify";
}

function modifyScheduler(taskId) {
    location.href="<%=request.getContextPath()%>/mx/enterprise/scheduler/showModify?taskId="+taskId;
}

function lockedScheduler(taskId, locked) {
	if(locked == "1"){
		if(confirm("锁定将无法进行任务业务调度，您真的要锁定吗？")) { 
			location.href="<%=request.getContextPath()%>/mx/enterprise/scheduler/locked?taskId="+taskId+"&locked="+locked;
		}
	} else {
			 location.href="<%=request.getContextPath()%>/mx/enterprise/scheduler/locked?taskId="+taskId+"&locked="+locked;
		}
...
function schedulerXY(taskId, startup) {
	if(startup == "false"){
	  if(confirm("您真的要停止任务业务调度吗？")) { 
		location.href="<%=request.getContextPath()%>/mx/enterprise/scheduler/startup?taskId="+taskId+"&startup=0";
	  }
	} else {
		location.href="<%=request.getContextPath()%>/mx/enterprise/scheduler/startup?taskId="+taskId+"&startup=1";
    }
}
...
<table align="center" class="table-border" cellspacing="0"
	...
	<tr>
		...
		<input type="button" value="新增任务业务调度" name="createScheduler" class="btn"
  ...
  %>
		<tr onmouseover="this.className='x-row-over';"
			...
			<td align="left" noWrap>&nbsp; <a
				href="<%=request.getContextPath()%>/mx/enterprise/scheduler/showModify?taskId=<c:out value="${scheduler.id}"/>">
