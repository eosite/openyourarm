...
<script type="text/javascript">
   ...
   jQuery(function(){
		jQuery('#mydatagrid').datagrid({
				...
				striped: true,
				collapsible:true,
				url:'<%=request.getContextPath()%>/mx/enterprise/schedulerLog/json?taskId=${taskId}',
				remoteSort: true,
				checkbox: true, 
				idField:'id',
				columns:[[
	...
	function deleteRows(){
		...
		if(ids.length > 0 ){
			if(confirm("确定删除这些业务调度历史吗？")){
			  jQuery.ajax({
					   type: "POST",
					   url: '<%=request.getContextPath()%>/mx/enterprise/schedulerLog/delete?ids='+ids,
					   dataType: 'json',
					   error: function(data){
						   alert('服务器处理错误！');
					   },
	...
	function deleteAll(){
	    if(confirm("确定删除该任务的全部业务调度历史吗？")){
			  jQuery.ajax({
					   type: "POST",
					   url: '<%=request.getContextPath()%>/mx/enterprise/schedulerLog/deleteAll?taskId=${taskId}',
					   dataType: 'json',
					   error: function(data){
						   alert('服务器处理错误！');
					   },
	...
	function onRowClick(rowIndex, row){
	    var link = '<%=request.getContextPath()%>/mx/enterprise/schedulerLog/view?q=1&id='+row.id;
	    art.dialog.open(link, { height: 425, width: 680, title: "业务调度详细信息", lock: true, scrollbars:"auto" }, false);
	}
	 ...
	 <form id="iForm" name="iForm" method="post" action="">  
      <table>
		   ...
		   <td>
	        ...
	        &nbsp;<span class="x_content_title">业务调度历史列表</span>
		   </td>
		   <td>
			   ...
			   <a href="#" class="easyui-linkbutton" data-options="plain:true, iconCls:'icon-back'"
				  onclick="javascript:location.href='<%=request.getContextPath()%>/mx/enterprise/scheduler';">返回</a>
		   </td>
			<td>运行状态&nbsp;</td>
			<td>
