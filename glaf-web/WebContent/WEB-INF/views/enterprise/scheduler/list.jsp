...
<script type="text/javascript">
   ...
   jQuery(function(){
		jQuery('#mydatagrid').datagrid({
				...
				striped: true,
				collapsible:true,
				url:'<%=request.getContextPath()%>/mx/enterprise/scheduler/json?taskType=${taskType}',
				remoteSort: true,
				checkbox: true, 
				idField:'id',
				columns:[[
   ...
   function formatterNextFireTime(val, row){
    ...
    function editRow(){
		 location.href ='<%=request.getContextPath()%>/mx/enterprise/scheduler/edit?taskType=${taskType}';
	}
	...
	function deleteRows(){
		...
		if(ids.length > 0){
		    ...
		    jQuery.ajax({
					   type: "POST",
					   url: '<%=request.getContextPath()%>/mx/enterprise/scheduler/delete?id='+taskId,
					   dataType: 'json',
					   error: function(data){
						   alert('服务器处理错误！');
					   },
	...
	function updateRow(){
		...
		if(rows.length ==1){
		   var taskId = rows[0].taskId;
		   location.href ='<%=request.getContextPath()%>/mx/enterprise/scheduler/edit?taskType=${taskType}&id='+taskId;
		} else {
		    alert("请选择其中一条记录。");
		}
	}
	...
	function enableRows(){
		...
		if(ids.length > 0 ){
			if(confirm("确定启用这些业务调度吗？")){
			  jQuery.ajax({
					   type: "POST",
					   url: '<%=request.getContextPath()%>/mx/enterprise/scheduler/enableRows?ids='+ids,
					   dataType: 'json',
					   error: function(data){
						   alert('服务器处理错误！');
					   },
	...
	function runIt(){
		...
		if(ids.length > 0 ){
		  if(confirm("确定立即运行这些业务调度吗？")){
		    jQuery.ajax({
				   type: "POST",
				   url: '<%=request.getContextPath()%>/mx/enterprise/scheduler/runIt?ids='+ids,
				   dataType:  'json',
				   error: function(data){
					   alert('服务器处理错误！');
				   },
    ...
    function restart(){
		...
		if(ids.length > 0 ){
		  if(confirm("确定重启这些业务调度吗？")){
		    jQuery.ajax({
				   type: "POST",
				   url: '<%=request.getContextPath()%>/mx/enterprise/scheduler/restart?ids='+ids,
				   dataType:  'json',
				   error: function(data){
					   alert('服务器处理错误！');
				   },
   ...
   function stop(){
		...
		if(ids.length > 0 ){
		  if(confirm("确定停止这些业务调度吗？")){
		    jQuery.ajax({
				   type: "POST",
				   url: '<%=request.getContextPath()%>/mx/enterprise/scheduler/stop?ids='+ids,
				   dataType:  'json',
				   error: function(data){
					   alert('服务器处理错误！');
				   },
	...
	function disableRows(){
		...
		if(ids.length > 0 ){
			if(confirm("确定禁用这些业务调度吗？")){
			  jQuery.ajax({
					   type: "POST",
					   url: '<%=request.getContextPath()%>/mx/enterprise/scheduler/disableRows?ids='+ids,
					   dataType: 'json',
					   error: function(data){
						   alert('服务器处理错误！');
					   },
	 ...
	 function showLogs(){
		...
		if(rows.length ==1){
		    var taskId = rows[0].taskId;
		    location.href='<%=request.getContextPath()%>/mx/enterprise/schedulerLog?taskId='+taskId;
		}else{
		    alert("请选择其中一条记录。");
		}
	 }
	 ...
	 function onRowClick(rowIndex, row){
	    var link = '<%=request.getContextPath()%>/mx/enterprise/scheduler/edit?taskType=${taskType}&id='+row.id;
	    art.dialog.open(link, { height: 425, width: 880, title: "业务调度详细信息", lock: true, scrollbars:"no" }, false);
	}
	...
	function search(){
         ...
         jQuery.ajax({
					   type: "POST",
					   url: '<%=request.getContextPath()%>/mx/enterprise/scheduler/json',
					   dataType: 'json',
					   data: params,
					   error: function(data){
						   alert('服务器处理错误！');
    ...
    <div class="toolbar-backgroud"  > 
      ...
      <table>
		   ...
		   <td>
	        ...
	        &nbsp;<span class="x_content_title">业务调度列表</span>
		   </td>
		   <td>
			   ...
			   <a href="#" class="easyui-linkbutton" data-options="plain:true, iconCls:'icon_stop'"
