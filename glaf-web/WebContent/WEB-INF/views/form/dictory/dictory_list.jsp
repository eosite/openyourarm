...
<%@page import="com.glaf.form.core.domain.FormDictory"%>
...
function checkOperation(form){
  ...
  if(num>0){
	...
	document.all.btn_delete.disabled=false;
  }else{
	document.all.btn_modify.disabled=true;
	document.all.btn_delete.disabled=true;
  }
}
function add(){
  var url="formDictory/prepareAdd?parent="+<%=parent%>;
...
function modify(form){
  ...
  var url="formDictory/prepareModify?id="+id+"&parent="+<%=parent%>;
  var link = "<%=request.getContextPath()%>/mx/form/"+url;
  var width=580;
  var height=420;
  var scroll="yes";
...
function sort(id, operate){  
    	jQuery.ajax({
			type: "POST",
			url: '<%=request.getContextPath()%>/mx/form/formDictory/sort?parent=<%=parent%>&id='+id+'&operate='+operate,
			dataType:  'json',
				error: function(data){
					alert('服务器处理错误！');
				},
...
function importJson(){
    var link='<%=request.getContextPath()%>/mx/form/formDictory/showImport';
    jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
...
function exportJson(){
	window.open('<%=request.getContextPath()%>/mx/form/formDictory/exportJson?parentId=<%=parent%>');
}
</script>
</head>
...
<html:form id="DictoryForm" name="DictoryForm" action="${contextPath}/mx/form/formDictory/batchDelete" method="post" target="_self">
...
if(list!=null){
  ...
  while(iter.hasNext()){
    FormDictory bean=(FormDictory)iter.next();	
%>
  ...
  <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
    <tr> 
      <td width="50%">
        ...
        <input name="btn_delete" type="button" value="删除" class="button" onClick="javascript:del();" disabled>
	  ...
	  <jsp:include page="/WEB-INF/views/inc/show_page.jsp" flush="true"> 
              ...
              <jsp:param name="url" value="showList"/>
