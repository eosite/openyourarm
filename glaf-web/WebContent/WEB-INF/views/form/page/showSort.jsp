...
<script type="text/javascript">
    ...
    function saveData(){
		...
		jQuery.ajax({
				   type: "POST",
				   url: "<%=request.getContextPath()%>/mx/form/page/saveSort?items="+str,
				   dataType:  'json',
				   error: function(data){
					   alert('服务器处理错误！');
				   },
...
<body>
        ...
        <div id="example">
            ...
            <div id="pages">
                <div id="pages-title"><span>分类排序</span></div>
                <div id="sortable">
				<c:forEach items="${pages}" var="page">
				  <c:if test="${page.locked == 0}">
                    <div class="item">
                        <span class="handler" title="选中后拖到指定位置">&nbsp;</span>
                        <label id="${page.id}" title="选中后拖到指定位置">${page.name}</label>
                    </div>
				  </c:if>
				</c:forEach> 
               </div>
