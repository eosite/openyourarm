...
<script type="text/javascript">
	...
	//转变字段数据
	function transformRows(rowsData,seq) {
		var rows = [];
	...
	$(document).ready(function() {
				...
				command : [ {
					...
					click : function(e) {
						...
						selectSource.fetch().then(function(){
							...
							//selectColumns.push();
							$.merge(selectColumns, transformRows(columns,data.id));
							rushSelectDatasourceGrid();
							//createWindow(data);
							$('#datasourceWindow').data('kendoWindow').close();
						})
					}
				} ]
			} ]
			...
			columns : [{
				...
				command : [ {
					...
					click : function(e) {
						...
						var index = jQuery.inArray(data, selectDatasource);
						selectDatasource.splice(index, 1);
		...
		var ctypes =  [{value:'xAxisName',text:'名称(x轴)'},{value:'yAxisName',text:'系列(y轴)'},{value:'axisName',text:'数值'}] ;
		//编辑字段
		...
		$("#commitToolbar").kendoToolBar({
			...
			items : [{
				...
				click : function(e) {
					...
					//$('#datasourceWindow').data("kendoWindow").open();
	...
	function createWindow(data){
		$("<div id='"+data.id+"'>我的</div>").appendTo("#container");
		$("#"+data.id).kendoWindow({
