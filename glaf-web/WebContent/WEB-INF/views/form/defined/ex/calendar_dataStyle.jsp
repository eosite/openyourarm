...
<body>
	...
	<div style="color: red;font-size: 18px;">
		注意 ：<br/>
		1、弹出页面，窗口名称，页面宽度，页面高度 适用于 弹出窗事件，如果不填写弹出页面，则当前不会弹出窗口
	</div>
	...
	zTree2 = jQuery.fn.zTree.init(jQuery("#selectPageTree2"), setting2);
		...
		$("#grid").kendoGrid({
			...
			columns : [{
				...
				field : 'bgcColor',
				title : '背景颜色',
				headerTemplate : '<span class="tooltip" title="背景颜色">背景颜色</span>',
				template : function(dataItem){
					return "<div style='width:50px;height:20px;background-color:"+dataItem.bgcColor+"'></div>";
				},
			...
			saveChanges : function(e) {
				...
				for (var i = 0; i < values.length; i++) {
					if (values[i].expression == undefined || values[i].expression == "" || values[i].name == undefined || values[i].name == "") {
						layer.alert("必须填写值", 3);
						return;
					}
					names += values[i].name + ",";
