...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		var ctypes =  [{value:'xAxisName',text:'名称(x轴)'},{value:'yAxisName',text:'系列(y轴)'},{value:'yAxisNameSpecial',text:'系列(y轴)(特殊)'},{value:'axisName',text:'数值'},
		               {value:'zAxisName',text:'气泡图大小'},{value:'elementsName',text:'地名'},{value:'longitute',text:'经度'},{value:'latitude',text:'纬度'},{value:'tColor',text:'矩形图颜色值'},
		              {value:'tid',text:'id'},{value:'tParent',text:'父节点'}] ;
		var fieldTypes = [
			{
				value:'string',
				text:'字符串'
		...
		$("#fieldColumnsGrid").kendoGrid({
			...
			columns : [ {
				...
				field : 'FieldType',
				template : function(dataItem) {
					var filedType = dataItem.FieldType;
					if (filedType) {
						var size = fieldTypes.length;
						for (var i = 0; i < size; i++) {
							var c = fieldTypes[i] ;
							if (c.value == dataItem.FieldType) {
								return c.text;
							}
						}
					}
			...
			items : [{
				...
				click : function(e) {
					...
					for(var i=0;i<selectDatasource.length;i++){
						...
						datasetIds.push(selectDatasource[i].datasetId);
					}
					...
					result.datasetId = datasetIds.join(",");
					results.push(result);
