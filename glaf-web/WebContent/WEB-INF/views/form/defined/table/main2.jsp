...
<script type="text/javascript">
	...
	//刷新 字段信息
	function rushColumnsGrid(){
		var columnsGrid = $("#columnsGrid").data("kendoGrid");
		columnsGrid.dataSource.read();
	}
	//（非查询刷新） 用于选择字段后
	function refreshColumnsGrid(){
		var columnsGrid = $("#columnsGrid").data("kendoGrid");
		columnsGrid.refresh();
	}
	//刷新 选择字段
					...
					parse: function(response){
						...
						if(response && response.length > 0){
							$.each(selectColumns,function(i,item){
								$.each(response,function(k,ktem){
									if(item.columnLabel == ktem.columnLabel){
										response.splice(k,1);								
										return false;	
									}
								})
				...
				command : [ {
					...
					click : function(e) {
						...
						if (flag) {
							var that = this;
							//删除已选字段
							var dataAry = that.dataItems();
							var indexNum = $.inArray(data, dataAry);
							if(indexNum != -1){
		...
		$("#widgetToolbar").kendoToolBar({
			...
			items:[
			   { type: "button", text: "保存", icon: "k-icon k-i-tick" ,click:function(e){
				   ...
				   column.widgetSourceType = widgetSourceType ;
				   column.widgetSource = widgetSource ;
				   var defaultIndex = $("#defaultIndex").val();
				   if(defaultIndex){
				   		column.defaultIndex = defaultIndex;
				   }
	...
	function copyDataSet(e){
		var linkImgGrid = $("#linkImgGrid").data("kendoGrid").dataSource;
		/* linkImgGrid.push({
			expression : 1,
			htmlExpression :1 ,
		}); */
		var tr = e.target.closest("tr");
		var uuid = $(tr).attr("data-uid");
		var el = {};
		$.each(linkImgGrid.data(),function(i,data){
			if(data.uid == uuid){
				el = {
						htmlExpression : data.htmlExpression,
...
<body>
	...
	<div id="tabstrip">
		...
		<div>
			<div id="columnsSplitter" style="height: 570px;">
				<div>
					<div id="datasourceDropdownlist"></div>
					<button id="batch_add">批量添加</button>
					<button id="refresh_columns" style="float:right;">刷新</button>
