...
<script type="text/javascript">
	...
	function init(){
		...
		if('${param.columnTemplateId}' != ''){
			var sourceModel = parent.document.getElementById('${param.columnTemplateId}').value;
			var array = eval(sourceModel);
			if(array){
	...
	$(document).ready(function() {
		...
		var ctypes =  [{value:'name',text:'名称'},{value:'stake',text:'桩号'},{value:'type',text:'类型'},{value:'point',text:'经纬度'}] ;
		if(uploadtype && uploadtype == "bim"){
			ctypes = [{value:'name',text:'文件名称'},{value:'urn',text:'唯一码'},{value:'formId',text:'表单ID'},{value:'attrId',text:'文件ID'}] ;
		}
		if(uploadtype && uploadtype == "excel"){
			ctypes = modelData;
		}
		if(uploadtype && uploadtype == 'bmapheat'){
			ctypes =  [{value:'lng',text:'经度'},{value:'lat',text:'纬度'},{value:'count',text:'密集度'}] ;
		}
		if(uploadtype && uploadtype == 'bmapclusterer'){
			ctypes =  [{value:'lng',text:'经度'},{value:'lat',text:'纬度'}] ;	
		}
		if(uploadtype && uploadtype == 'bmapmarker'){
			ctypes =  [{value:'lng',text:'经度'},{value:'lat',text:'纬度'}] ;	
		}
		//编辑字段
			...
			items : [{
				...
				click : function(e) {
					...
					for(var j=0;j<selectColumns.length;j++){
						...
						if(ctypes && column.ctype){
							var flag = false;
							for (var k = 0; k < ctypes.length; k++) {
								var c = ctypes[k] ;
								if (c.value == column.ctype) {
									flag = true;
									break;
								}
							}
