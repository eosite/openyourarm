...
<script type="text/javascript">
    ...
    function checkForm(){
        ...
        var datasetIds = jQuery("#datasetIds").val();
        var querySQL = jQuery("#querySQL").val();

        if( queryIds == '' && datasetIds=='' && querySQL=='' ){
    ...
    function viewChart(){
        ...
        window.open('<%=request.getContextPath()%>/mx/bi/chart/showChart?chartId=${chart.id}&databaseId='+databaseId);
    }

    function viewHighCharts(){
        var databaseId = jQuery("#databaseId").val();
        window.open('<%=request.getContextPath()%>/mx/bi/chart/highcharts/showChart?chartId=${chart.id}&chooseThemes=true&databaseId='+databaseId);
    }

    function viewECharts(){
       var databaseId = jQuery("#databaseId").val();
        window.open('<%=request.getContextPath()%>/mx/bi/chart/echarts/showChart?chartId=${chart.id}&chooseThemes=true&databaseId='+databaseId);
    }

    function viewKendoChart(){
       var databaseId = jQuery("#databaseId").val();
       window.open('<%=request.getContextPath()%>/mx/bi/chart/kendo/showChart?chartId=${chart.id}&chooseThemes=true&databaseId='+databaseId);
    }
    ...
    function openDataSet(){
        var link = '<%=request.getContextPath()%>/mx/bi/chart/chartDatasetList?elementId=datasetIds&elementName=datasetNames&chartId=${chart.id}&xtype=1';
        var x=100;
        var y=100;
        if(is_ie) {
    ...
    function openDataSet2(){
        var link = '<%=request.getContextPath()%>/mx/bi/chart/chartDatasetList?elementId=secondDatasetIds&elementName=dataset2Names&chartId=${chart.id}&xtype=2';
        var x=100;
        var y=100;
        if(is_ie) {
        	x=document.body.scrollLeft+event.clientX-event.offsetX-100;
 ...
 <br/>
	 <form id="iForm" name="iForm" method="post">
        ...
        <tr>
        	 ...
        	 <br><%out.println("#{input_yyyymmdd}");%>代表输入的日期，如<%=com.glaf.core.config.SystemConfig.getInputYYYYMMDD()%>
              <br><%out.println("#{input_yyyymm}");%>代表输入的月份，如<%=com.glaf.core.config.SystemConfig.getInputYYYYMM()%>
