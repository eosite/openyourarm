...
<title>${transferTable.tableName} 数据列表</title>
...
<script type="text/javascript">
    ...
    jQuery(function(){
		jQuery('#easyui_data_grid').datagrid({
				width: ${transferTable.columns.size()*190},
				height:400,
				fit:false,
				fitColumns: true,
				nowrap: false,
				...
				remoteSort: false,
				singleSelect: true,
				idField: '${transferTable.idColumn.columnName}',
				columns:[[
				        {title:'序号', field:'startIndex', width:80, sortable:false}
                        <c:forEach items="${transferTable.columns}" var="column">
						  <c:choose>
	...
	function onMyRowClick(rowIndex, row){
	    var link = '<%=request.getContextPath()%>/mx/sys/table/edit?tableName_enc=${tableName_enc}&businessKey='+row.${transferTable.idColumn.columnName};
	    var x=50;
        var y=50;
        if(is_ie) {
