...
<script type="text/javascript">
   ...
   var setting = {
    ...
    function zTreeOnClick(event, treeId, treeNode, clickFlag) {
		...
		loadMxData('<%=request.getContextPath()%>/mx/dts/transferTable/json?nodeId='+treeNode.id);
	}
	...
	function reLoadData(nodeId){
		loadData('<%=request.getContextPath()%>/mx/dts/transferTable/json?nodeId='+nodeId);
	}
   ...
   jQuery(function(){
		jQuery('#easyui_data_grid').datagrid({
				...
				striped: true,
				collapsible: true,
				url: '<%=request.getContextPath()%>/mx/dts/transferTable/json',
				remoteSort: false,
				singleSelect: true,
				idField: 'id',
				columns:[[
	...
	function formatterKeys(val, row){
		var str = "<a href='javascript:editRow(\""+row.id+"\");'>修改</a>&nbsp;<a href='javascript:removeRow(\""+row.id+"\");'>删除</a>";        
	    return str;
	}

	function editRow(id){
		var nodeId = jQuery("#nodeId").val();
	    var link = '<%=request.getContextPath()%>/mx/dts/transferTable/edit?id='+id+"&nodeId="+nodeId;
	...
	function removeRow(id){
      if(confirm("数据删除后不能恢复，确定删除吗？")){
			jQuery.ajax({
				   type: "POST",
				   url: '<%=request.getContextPath()%>/mx/dts/transferTable/delete?id='+id,
				   dataType:  'json',
				   error: function(data){
					   alert('服务器处理错误！');
				   },
	...
	function addNew(){
	    ...
	    var link="<%=request.getContextPath()%>/mx/dts/transferTable/edit?nodeId="+nodeId;
	...
	function onRowClick(rowIndex, row){
		var nodeId = jQuery("#nodeId").val();
	    var link = '<%=request.getContextPath()%>/mx/dts/transferTable/edit?id='+row.id+"&nodeId="+nodeId;
		window.open(link);
	    //art.dialog.open(link, { height: 430, width: 820, title: "修改记录", lock: true, scrollbars:"no" }, false);
		/**
		jQuery.layer({
	...
	function editSelected(){
	    ...
	    if (selected ){
		  var nodeId = jQuery("#nodeId").val();
		  var link = "<%=request.getContextPath()%>/mx/dts/transferTable/edit?id="+selected.id+"&nodeId="+nodeId;
		  //art.dialog.open(link, { height: 430, width: 820, title: "修改记录", lock: true, scrollbars:"no" }, false);
		  jQuery.layer({
					type: 2,
					maxmin: true,
	...
	function viewSelected(){
		...
		if (selected ){
		  var nodeId = jQuery("#nodeId").val();
		  location.href="<%=request.getContextPath()%>/mx/dts/transferTable/edit?readonly=true&id="+selected.id+"&nodeId="+nodeId;
		}
	}
	...
	function deleteSelections(){
		...
		if(ids.length > 0 && confirm("数据删除后不能恢复，确定删除吗？")){
			...
			jQuery.ajax({
				   type: "POST",
				   url: '<%=request.getContextPath()%>/mx/dts/transferTable/delete?ids='+str,
				   dataType:  'json',
				   error: function(data){
					   alert('服务器处理错误！');
				   },
	...
	function deployNew(){
        ...
        var link = "<%=request.getContextPath()%>/mx/dts/transferTable/showDeploy?nodeId="+nodeId;
	...
	function importData(){
		...
		var selected = jQuery('#easyui_data_grid').datagrid('getSelected');
		if (selected ){
		  var link = "<%=request.getContextPath()%>/mx/dts/transferTable/showImport?transferId="+selected.id;
	...
	function exportXml(){
		...
		var selected = jQuery('#easyui_data_grid').datagrid('getSelected');
		if (selected ){
		  var link = "<%=request.getContextPath()%>/mx/dts/transferTable/exportXml?transferId="+selected.id;
		  window.open(link);
		}
	}
   ...
   <div data-options="region:'center'">  
	<div class="easyui-layout" data-options="fit:true">  
	   <div data-options="region:'north',split:true,border:true" style="height:40px"> 
		...
		&nbsp;<span class="x_content_title">目标表列表</span>
