...
<script type="text/javascript">
   ...
   var setting = {
	...
	function editRow(id){
		var nodeId = jQuery("#nodeId").val();
		var link = '<%=request.getContextPath()%>/mx/dts/table/edit?tableName_enc='+id+"&nodeId="+nodeId;
		jQuery.layer({
					type: 2,
					maxmin: true,
					shadeClose: true,
    ...
    function transformTable(tableName_enc){
		if(confirm("确定重新获取数据吗？")){
			...
			jQuery.ajax({
					   type: "POST",
					   url: '<%=request.getContextPath()%>/rs/dts/table/transformTable?tableName_enc='+tableName_enc,
					   data: params,
					   dataType:  'json',
					   error: function(data){
						   alert('服务器处理错误！');
	  ...
	  function rebuild(tableName_enc, type){
	   ...
	   function deleteTbl(tableName_enc){
