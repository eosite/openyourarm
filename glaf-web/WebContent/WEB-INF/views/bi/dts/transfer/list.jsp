...
<script type="text/javascript">
   ...
   jQuery(function(){
	...
	function formatterKeys(val, row){
		var str = "<a href='javascript:editRow(\""+row.id+"\");'>修改</a>&nbsp;<a href='javascript:removeRow(\""+row.id+"\");'>删除</a>&nbsp;<a href='javascript:showDataList(\""+row.id+"\");'>数据</a>";        
	    return str;
	}

	function editRow(id){
		var nodeId = jQuery("#nodeId").val();
	    var link = '<%=request.getContextPath()%>/mx/dts/dataTransfer/edit?id='+id+"&nodeId="+nodeId;
		jQuery.layer({
					type: 2,
					maxmin: true,
					shadeClose: true,
	...
	function showDataList(id){
		var link = '<%=request.getContextPath()%>/mx/dts/dataTransfer/dataList?transferId='+id;
		jQuery.layer({
					type: 2,
					maxmin: true,
					shadeClose: true,
					...
					offset: ['20px',''],
					fadeIn: 100,
					area: ['980px', (jQuery(window).height() - 60) +'px'],
					iframe: {src: link}
				});
	}
	...
	function removeRow(id){
      if(confirm("数据删除后不能恢复，确定删除吗？")){
			jQuery.ajax({
				   type: "POST",
				   url: '<%=request.getContextPath()%>/mx/dts/dataTransfer/delete?id='+id,
				   dataType:  'json',
				   error: function(data){
					   alert('服务器处理错误！');
				   },
	...
	function searchWin(){
	    jQuery('#dlg').dialog('open').dialog('setTitle','数据抓取查询');
   ...
   <div data-options="region:'center'">  
	<div class="easyui-layout" data-options="fit:true">  
	   <div data-options="region:'north',split:true,border:true" style="height:40px"> 
		...
		&nbsp;<span class="x_content_title">数据抓取列表</span>
