...
<%
        ...
        if(unselecteds != null && unselecteds.size() > 0){
		    for(int j=0;j<unselecteds.size();j++){
				TableDefinition u = (TableDefinition) unselecteds.get(j);
				if(selecteds != null && selecteds.contains(u.getTableName())){
                    buffery.append("\n<option value=\"").append(u.getTableName()).append("\">")
						   .append(u.getTableName()).append(" [").append(u.getTitle()).append("]")
						   .append("</option>");
				} else {
				    bufferx.append("\n<option value=\"").append(u.getTableName()).append("\">")
						   .append(u.getTableName()).append(" [").append(u.getTitle()).append("]")
...
<script language="javascript">
 ...
 var contextPath = "${contextPath}";
	...
	function doSelect() {
		...
		var x_names = parent_window.document.getElementById("${elementName}");
