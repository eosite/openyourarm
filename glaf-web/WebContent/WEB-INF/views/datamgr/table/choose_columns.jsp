...
<%
	    ...
	    List selectedIds = (List)request.getAttribute("selectedIds");
        if(columns != null && columns.size() > 0){
			for(int j=0;j<columns.size();j++){
				ColumnDefinition c = (ColumnDefinition) columns.get(j);
				if(selectedIds != null && selectedIds.contains(c.getColumnName())){
                    buffery.append("\n<option value=\"").append(c.getColumnName()).append("\" selected>")
						   .append(c.getTitle()).append(" [")
						   .append(c.getColumnName()).append("]</option>");
				} else {
				    bufferx.append("\n<option value=\"").append(c.getColumnName()).append("\">")
						   .append(c.getTitle()).append(" [")
						   .append(c.getColumnName()).append("]</option>");
				}
			}
...
<div class="x_content_title"><img
	src="<%=request.getContextPath()%>/images/window.png" alt="选择字段">&nbsp;
选择字段</div>
	...
	<tbody>
		...
		<tr>
			...
			<div align="center"><select class="list"
				style="width: 260px; height: 350px;" multiple="multiple" size="12"
