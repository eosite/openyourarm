...
//事件定义器中的回调方法
	...
	var command = [{
                ...
                "click": function showDetails(e) {
							      ...
							      //alert(dataItem.id);
								  var link = "<%=request.getContextPath()%>/mx/sys/tableTransform/edit?tableName="+dataItem.id;
								  editRow(link);
								}
                },
				{
                ...
                "click": function deleteRow(e) {
									 ...
									 if(confirm("数据删除后不能恢复，确定删除吗？")){
										 var link = "<%=request.getContextPath()%>/mx/sys/tableTransform/delete?tableName="+dataItem.id;
                ...
                "click": function showColumns(e) {
								  ...
								  //alert(JSON.stringify(dataItem));
							      //alert(dataItem.id);
								  var link = "<%=request.getContextPath()%>/mx/sys/columnTransform?tableName="+dataItem.id;
								  editColumns(link);
								}
                },
				{
                ...
                "click": function executeRow(e) {
								     ...
								     //alert(JSON.stringify(dataItem));
							         //alert(dataItem.id);
								     var link = "<%=request.getContextPath()%>/mx/sys/tableTransform/execute?tableName="+dataItem.id;
									 var params = jQuery("#iForm").formSerialize();
									 jQuery.ajax({
											   type: "POST",
											   url: link,
                ...
                "click": function emptyColumnsRow(e) {
					             if(confirm("确定重置目标字段的值为空吗？")){
								     ...
								     //alert(JSON.stringify(dataItem));
							         //alert(dataItem.id);
								     var link = "<%=request.getContextPath()%>/mx/sys/tableTransform/emptyColumns?tableName="+dataItem.id;
									 var params = jQuery("#iForm").formSerialize();
									 jQuery.ajax({
											   type: "POST",
											   url: link,
                ...
                "click": function handler(e) {
								    var dataItem = this.dataItem(jQuery(e.currentTarget).closest("tr"));
								    var link = "<%=request.getContextPath()%>/mx/sys/syntheticFlow/edit?currentStep=table_transform_"+dataItem.id+"&code=table_transform";
								    showHandler(link);
								}
                },
				{
                ...
                "click": function showLogging(e) {
									...
									var link = "<%=request.getContextPath()%>/mx/sys/executionLog?type=table_transform&businessKey="+dataItem.id;
								    showLog(link);
								}
                }
				],
  ...
  jQuery(function() {
        ...
        "dataSource": {
            ...
            "transport": {
                ...
                "read": {
                    ...
                    "type": "POST",
                    "url": "<%=request.getContextPath()%>/rs/sys/tableTransform/data"
                }
            },
    ...
    function addRow(){
         editRow('<%=request.getContextPath()%>/mx/sys/tableTransform/edit');
    }
