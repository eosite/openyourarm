		...
		if (array != null && array.size() > 0) {
			...
			if (users != null && users.size() > 0) {
				for (int j = 0; j < users.size(); j++) {
					...
					bufferx.append("\n<option value=\"").append(user.getActorId()).append("\">")
						   .append(user.getName() != null ? user.getName() : user.getActorId()).append(" [").append(user.getActorId()).append("]")
						   .append("</option>");
				}
			}
			...
			if (roles != null && roles.size() > 0) {
				for (int j = 0; j < roles.size(); j++) {
					...
					buffery.append("\n<option value=\"").append(role.getId()).append("\">")
						   .append(role.getName()).append(" [").append(role.getId()).append("]")
						   .append("</option>");
				}
			}
	}
...
var contextPath="<%=request.getContextPath()%>";
  ...
  function doSelection() {
	...
	if(confirm("确定按选中记录设置数据权限吗？")){
		  document.getElementById("userIds").value=result;
		  document.getElementById("roleIds").value=result2;
		  document.getElementById("objectIds").value=result3;
		  var link = "<%=request.getContextPath()%>/mx/sys/dataPermission/saveAll?businessType=${businessType}";
...
function viewByUser(){
	location.href="<%=request.getContextPath()%>/mx/sys/dataPermission/viewByUser?businessType=${businessType}";
}
		...
		<form id="iForm" name="iForm" class="x-form" method="post" >
			...
			<div class="content-block" style="width: 100%;">
				...
				<div align="center">
					...
					<input value="按用户设置" class="btn btnGray" name="btn" type="button"
						onclick="javacsript:viewByUser();"> 
					<br />
					<br />
					<br />
					<br />
