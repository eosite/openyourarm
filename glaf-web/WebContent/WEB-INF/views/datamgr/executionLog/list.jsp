...
<script type="text/javascript">
   ...
   jQuery(function(){
		jQuery('#mydatagrid').datagrid({
				...
				striped: true,
				collapsible: true,
				url: '<%=request.getContextPath()%>/mx/sys/executionLog/json?type=${type}&businessKey=${businessKey}&dateAfter=${dateAfter}',
				remoteSort: false,
				singleSelect: true,
				idField: 'id',
				columns:[[
				        {title:'序号', field:'startIndex', width:50, sortable:false},
						{title:'用户',field:'createBy', width:60, formatter:formatterUser},
					    {title:'编号',field:'jobNo', width:285, formatter:formatterNo},
	...
	function viewDataset(id){
		window.open("<%=request.getContextPath()%>/mx/dataset/sqlbuilder?id="+id);
	}
	...
	function removeToday(){
		if(confirm("确定要删除当天的执行日志吗？")){
			var link = "<%=request.getContextPath()%>/mx/sys/executionLog/delete?type=${type}&businessKey=${businessKey}";
			jQuery.ajax({
					   type: "POST",
					   url: link,
					   dataType:  'json',
	...
	function removeOverdue(){
		if(confirm("确定要删除已经过期的执行日志吗？")){
			var link = "<%=request.getContextPath()%>/mx/sys/executionLog/deleteOverdue";
			jQuery.ajax({
					   type: "POST",
					   url: link,
					   dataType:  'json',
	...
	function paramlogs(){
		location.href="<%=request.getContextPath()%>/mx/sys/parameterLog?businessKey=${type}_${businessKey}";
	}
...
<div class="easyui-layout" data-options="fit:true">  
   <div data-options="region:'north',split:true,border:true" style="height:40px"> 
	...
	<form id="iForm" name="iForm" method="post" action="<%=request.getContextPath()%>/mx/sys/executionLog">
	<img src="<%=request.getContextPath()%>/images/window.png">
	&nbsp;<span class="x_content_title">操作日志</span>
	&nbsp;
	<input name="btn0" type="button" value="参数日志" class="btnGray" onclick="javascript:paramlogs();">
	&nbsp;
