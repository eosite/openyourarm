...
<%
		...
		List queries = (List)request.getAttribute("unselected");
		List selecteds = (List)request.getAttribute("selected");
        if(queries != null && queries.size() > 0){
		    for(int j=0;j<queries.size();j++){
				ColumnDefinition u = (ColumnDefinition) queries.get(j);
				if(selecteds != null && selecteds.contains(u.getColumnName())){
                    buffery.append("\n<option value=\"").append(u.getColumnName().toLowerCase()).append("\">")
						   .append(u.getColumnName().toLowerCase()).append(" [").append(u.getColumnName().toLowerCase()).append("]")
						   .append("</option>");
				} else {
				    bufferx.append("\n<option value=\"").append(u.getColumnName().toLowerCase()).append("\">")
						   .append(u.getColumnName().toLowerCase()).append(" [").append(u.getColumnName().toLowerCase()).append("]")
...
<div class="content-block" style="width: 885px;"> 
		...
		<tr>
			...
			<div align="center">
				<select class="list" id="noselected" name="noselected" 
					style="width: 420px; height: 280px;" multiple="multiple" size="12"
