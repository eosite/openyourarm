...
<script type="text/javascript">
     ...
     function saveAccessor(actorId, projectId){
		 ...
		 jQuery.ajax({
				   type: "POST",
				   url: '<%=request.getContextPath()%>/mx/datamgr/project/saveAccessor?actorId='+actorId+'&projectId='+projectId+'&operation='+operation,
				   dataType:  'json',
				   error: function(data){
					   alert('服务器处理错误！');
				   },
	...
	function switchView(){
		<c:choose>
		   <c:when test="${op_view eq 'project'}">
		       location.href="<%=request.getContextPath()%>/mx/datamgr/project/permission?op_view=user";
		   </c:when>
		   <c:otherwise>
			   location.href="<%=request.getContextPath()%>/mx/datamgr/project/permission?op_view=project";
		   </c:otherwise>
		</c:choose>
	}

function searchProjects(){
      document.iForm.action="<%=request.getContextPath()%>/mx/datamgr/project/permission?op_view=project";
	  document.iForm.submit();
}

function searchUsers(){
      document.iForm.action="<%=request.getContextPath()%>/mx/datamgr/project/permission?op_view=user";
	  document.iForm.submit();
}
  ...
  <tr>
	...
	<td width="15%" align="right">
	   <c:choose>
		 ...
		 <c:otherwise>
		  <input type="button" class="btnGray" value="切换项目视图" onclick="javascript:switchView();">
		 </c:otherwise>
		</c:choose>
	</td>
  </tr>
		  ...
		  <tr>
			<td colspan="10" class="list-title">
			  <table width="100%" cellspacing="0" cellpadding="0">
			   <tr>
				<td align="left">
				<font color="#0066ff"><b>项目：<%=project.getTitle()%> [<%=project.getName()%>]</b></font>
				</td>
			   </tr>
			  </table>
			</td>
