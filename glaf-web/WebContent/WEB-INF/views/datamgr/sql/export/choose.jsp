...
<%
		...
		List selectedSqlDefIds = (List) request.getAttribute("selectedSqlDefIds");
		List sqlDefinitions = (List) request.getAttribute("sqlDefs");
		if(sqlDefinitions != null && sqlDefinitions.size() > 0){
			for(int j=0;j<sqlDefinitions.size();j++){
				...
				if(selectedSqlDefIds != null && selectedSqlDefIds.contains(r.getId())){
					bufferx.append("\n<option value=\"").append(r.getId()).append("\">")
					   .append(r.getTitle()).append(" [")
					   .append(r.getId()).append("]</option>");	
				}
				buffery.append("\n<option value=\"").append(r.getId()).append("\">")
					   .append(r.getTitle()).append(" [")
					   .append(r.getId()).append("]</option>");		
