...
<script>
	...
	$(document).ready(function() {
		...
		if(dataSet.id){
			$.ajax({
				url : contextPath + "/mx/datasrc/getLastestJson",
				type : 'POST',
				dataType : 'json',
...
<body>
			...
			<table style="width: 100%;">
				<tr>
					<td style="width:500px;text-align: left;vertical-align: middle; " ><img
						...
						style="font-family:Lucida Calligraphy;font-size: 20px;font-weight: bolder;">数据源输入</span>
					...
					<td style="text-align:right;">
						...
						<button id="button3" type="button">预览</button>
					</td>
				</tr>
			</table>
		</div>
		<div>
			<div id="horizontal" style="height:100%; width: 100%;">
				<div
						     ...
						     <script type="text/javascript">
						     	var $select_biaoduan =  $("#select-biaoduan").kendoDropDownList({
			                        ...
			                        change: function(e){
										...
										var selectType = $("#select-type").data('kendoDropDownList').dataItem();
			                        	//$("#select-biaoduan").data("select",e.sender.dataItem());
						...
						<hr>
						<ul id="fieldTree" class="ztree" style="height:80%;overflow:auto">
						</ul>
				</div>
				<div>
				<div id="tabstrip" class='' style="border:0px;">
					...
					<div class='tabstrips' style=" width: 96%;overflow-y:auto;overflow-x: hidden;">
							<div id="sql" style="width: 100%; height: 99%;"></div>
							<script type="text/x-kendo-template" id="sql-toolbar-template">
                			<div class="toolbar">
                    			<button class='k-button' id='sql-toolbar-create'>增加</button>
                			</div>
            				</script>
							<script type="text/javascript">
								
								var $sql = $("#sql").kendoGrid({
									toolbar : kendo.template($("#sql-toolbar-template").html()),
									editable : true,
									selectable:true,
									columns : [ {
										field : 'sql',
										...
										editor : function(container, options) {
											...
											var $input = $("<input/>",{name : options.field,class : 'k-textbox',placeholder:'请在此输入自定义sql语句',readonly : true}).appendTo(container).change(function(e){
									...
									{ 
									  ...
									  width: "120px",
									  template : function(){
									    return "<button class='k-button' onclick='deleteRow(this)' >删除</button>";
									    } 
									}]
								}).data("kendoGrid");
							
								var grid = $('#sql').data('kendoGrid');
								grid.dataSource.insert({sql : ''});
								//grid.hideColumn(1);
								//grid.refresh();
								$("#sql").find("#sql-toolbar-create").kendoButton({
									click : function(e){
										// alert("22");
	...
	<script type="text/javascript">
		...
		$("#button1").kendoButton({
			...
			click : function(e){
				saveDataSet(true,contextPath + "/mx/datasrc/toSql");
			}
		});
		...
		$("#button2").kendoButton({
			...
			click : function(e){
				saveDataSet(true,contextPath + "/mx/datasrc/save");
			}
		});
		...
		$("#button3").kendoButton({
			...
			click : function(e){
				...
				if(dataSet.id){
					window.open("${contextPath}/mx/etl/model/preview?id=" + dataSet.id);
				} else {
					alert('请保存');
				}
		...
		function saveDataSet(save,url){
			...
			var database ={};
			database.databaseId = dataItem.id;
			database.code = dataItem.code;
			database.host = dataItem.host;
			database.databaseName = dataItem.text;
			
			if(database.databaseName=="默认"){
				database.databaseId = 14;
				database.host = "192.168.1.27";
				database.databaseName = "glafDB";
			}
			var databaseStr = JSON.stringify(database);

			$.ajax({
				url : url,
				data : {datasetId:dataSet.id,database:databaseStr,jsonResult : jsonStr},
				type : 'post',
				dataType : 'json',
				success : function(data){
					if(data){
				...
				var sql = JSON.stringify($sql.dataSource.data());
				$.ajax({
					url : url,
					data : {datasetId:dataSet.id,database:databaseStr,sql:sql},
					type : 'post',
					dataType : 'json',
					success : function(data){
						if(data){
		...
		function onSelect(e)
		{
		   ...
		   //saveDataSet(false,contextPath + "/mx/dataset/handle/saveHistory");
