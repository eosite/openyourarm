...
<%
		...
		List executions = (List)request.getAttribute("executions");
        if(executions != null && executions.size() > 0){
		    for(int j=0; j<executions.size(); j++){
				TableExecution d = (TableExecution) executions.get(j);
				if(selecteds != null && selecteds.contains(d.getId())){
                    buffery.append("\n<option value=\"").append(d.getId()).append("\">")
						   .append(d.getTitle()).append(" [").append(d.getTableName()).append("]")
						   .append("</option>");
				} else {
				    bufferx.append("\n<option value=\"").append(d.getId()).append("\">")
						   .append(d.getTitle()).append(" [").append(d.getTableName()).append("]")
...
<table class="table-border" align="center" cellpadding="4"
	...
	<tbody>		 
		<tr>
			...
			<div align="center">
			   <div align="center">可选操作</div>
			</div>
			</td>
			<td></td>
			<td colspan="2" width="212">
			<div align="center">已选操作</div>
			</td>
		</tr>
		<tr>
			<td width="2">&nbsp;</td>
...
<div align="center"><br />
 ...
 <input value=" 操作列表 " class=" btn" name="button" type="button"
		onclick="javacsript:window.open('<%=request.getContextPath()%>/mx/sys/tableExecution');">
