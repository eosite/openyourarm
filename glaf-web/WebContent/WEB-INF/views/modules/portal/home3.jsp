...
<script type="text/javascript">
   ...
   function changeTheme(){
	...
	function createNode(node){
		var n = $("<a href='#' class='easyui-menubutton' data-options=\"menu:'#"+node.id+"'\" style='color: black;'>"+node.text+"</a>");
		var d = $("<div id='"+node.id+"'></div>");
		var nodeChilds = node.children ;
		for (var i = 0; i < nodeChilds.length; i++) {
			var child = nodeChilds[i];
			var c = $("<div onclick=\"addTab('"+child.text+"', '"+child.url+"')\" data-options=\"iconCls:'icon-gears'\">"+child.text+"</div>");
			d.append(c);
		}
		$("#horizontal-menu").append(n).append(d);
	}

	function initMenu(){
		var nodes = _mxm_.children ;
...
<div region="north" split="false"  border="false" style="overflow: hidden; height: 52px;
        background: url(${contextPath}/themes/${theme}${userTheme.background}) #7f99be repeat-x center 50%;
        background-size:83px 100%;
        line-height: 52px;color: #fff; font-family: Verdana, 微软雅黑,黑体">
        ...
        <span style="padding-left:10px; font-size: 24px;">
			<img src="<%=request.getContextPath()%><%=SystemConfig.getString("res_logo","/images/logo.png")%>" border="0" align="absmiddle" /> 
			<span class="sys_name" style="padding-left:10px; font-size: 24px; ">
			<%=SystemConfig.getString("res_system_name")%>
			</span>&nbsp;
...
<div id="mainPanle" region="center" border="false" style="background: #eee; overflow-y:hidden">
		<div class="easyui-panel" id="horizontal-menu" style="padding:0px;height: 28px;line-height: 24px;border: 0px; 
      		margin: 0px;background-color:rgba(223, 235, 254, 0.8);font-size: 16px;font-weight: bold;">
      	</div>
...
<!--views\modules\portal\home3.jsp-->