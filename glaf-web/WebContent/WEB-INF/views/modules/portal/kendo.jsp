...
#panelBar > .k-item > .k-link{
	padding-left: 0.1em;
	background-color: rgba(63, 150, 200, 1) ;
	
}
#panelBar > .k-item > .k-link >.k-image{
	margin-top: 7px;
}
...
.k-panel>li.k-item{
	background-color:rgba(95, 201, 253, 0.5);
}
#tree{
	background-color:rgba(95, 201, 253, 0.5);
}
...
<body>
	<div id="banner"   class="k-header k-widget" 
		...
		">
		<table style="width: 100%; height: 100%; border: 0px;background-image: url('${contextPath}/images/top.jpg');" >
			<tr>
				<!-- <td id="imgside" style="width: 60px;"><img src="${contextPath}/images/logo.png" height="25"
					style="margin-left: 5px; vertical-align: middle;"></td> -->
				<td id="imgside2" style="width: 520px; padding-left: 2px;">
				...
				<td>
					<ul id="menu-images" class="menuCss" style="background:no-repeat scroll 0 0 rgba(0, 0, 0, 0) "></ul>
				</td>
	...
	<script type="text/javascript">
		...
		function initMenuData(){
			nMenuJson+="{\"text\": \"<img src='${contextPath}/images/home_32.png'><span class='menuSpanCss'>首  页</span>\", \"cssClass\":\"firstMenuCss\",  \"encoded\": false, \"url\":\"javascript:initHome(\\\"${webPath}<%=com.glaf.base.utils.LoginContextUtil.getIndexUrl(request)%>\\\");\"},";
		...
		function createNode(node){
			...
			if(node.text!="")
			{	
				var imagePath="${contextPath}/images/apple_corp.png";
				if(node.icon!=null&&node.icon!=""&&node.icon!=undefined){
					imagePath="${contextPath}"+node.icon;
				}
				nMenuJson+="\"text\":\"<img src='"+imagePath+"'><span class='menuSpanCss'>"+node.text+"</span>\"," ;
					//"\"text\":\""+node.text+"\",";
			}
			
			nMenuJson+="\"cssClass\":\"firstMenuCss\",\"encoded\": false,";
			/* if(node.icon!=null&&node.icon!=""&&node.icon!=undefined)
			{
		...
		function initLeftTree(nodeId){
			//创建左侧
			if(!$("#tree").html()){
				$("#horizontal").data("kendoSplitter").insertBefore({collapsed: false ,collapsible: true, collapsedSize: "0px",max:"300px",resizable: false,size:"250px",scrollable: true},
					$("#content")).html("<div id='panelBar' style='background:no-repeat scroll 0 0 rgba(0, 0, 0, 0)'>无下级菜单</div>").attr("id","tree");
			}
		...
		function creatPanelBarDatasource(nodes,dataSource,bol,count){
			...
			for(var i=0;i<nodes.length;i++){
				...
				var obj = {} ;
				obj.text = node.text ;
				var imagePath = "${contextPath}/images/left_default.png" ;
				if(node.icon){
					imagePath ="${contextPath}"+node.icon;
				}
...
<!--views\modules\portal\kendo.jsp-->