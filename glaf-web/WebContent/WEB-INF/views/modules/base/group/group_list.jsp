...
function add(){
  var url="group/prepareAdd?type=${type}";
  var link = "<%=request.getContextPath()%>/mx/base/"+url;
  var width=450;
  var height=250;
  var scroll="no";
...
function modify(form){
  ...
  var url="group/prepareModify?groupId="+id;
  var link = "<%=request.getContextPath()%>/mx/base/"+url;
  var width=450;
  var height=250;
  var scroll="no";
...
function users(form){
  ...
  var url="group/groupUsers?groupId="+id;
  var link = "<%=request.getContextPath()%>/mx/base/"+url;
  var width=450;
  var height=480;
  var scroll="yes";
...
function sortGroup(id, operate){  
  ...
  //alert('groupId='+id+'&operate='+operate);
    	jQuery.ajax({
			type: "POST",
			url: '<%=request.getContextPath()%>/mx/base/group/sort?groupId='+id+'&operate='+operate,
			dataType:  'json',
				error: function(data){
					alert('服务器处理错误！');
				},
...
<html:form action="${contextPath}/mx/base/group/batchDelete" method="post" target="_self"> 
  ...
  <tr> 
    ...
    <td width="50%"> 
      ...
      <jsp:include page="/WEB-INF/views/inc/show_page.jsp" flush="true"> 
              ...
              <jsp:param name="url" value="group"/>
