...
<script type="text/javascript">
	...
	function filter(treeId, parentNode, childNodes) {
			...
			for (var i=0, l=childNodes.length; i<l; i++) {
			    ...
			    //     childNodes[i].icon="<%=request.getContextPath()%>/images/user.png";
		        //}
			}
			return childNodes;
	}
	...
	function chooseMyFormData(){
        ...
        for(var i=0; i<selectedNodes.length; i++){ 
			...
			if(selectedNodes[i].actorId){
			  ...
			  //alert(selectedNodes[i].name);
			  if(selectedNodes[i].name != null && selectedNodes[i].name != 'undefined'){
                  sx_name += selectedNodes[i].name; 
			  } else { 
				  sx_name += selectedNodes[i].actorId; 
			  }
			}
        }  
