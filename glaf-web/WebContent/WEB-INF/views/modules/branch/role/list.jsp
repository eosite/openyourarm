...
<script type="text/javascript">
   ...
   jQuery(function(){
		jQuery('#mydatagrid').datagrid({
				...
				striped: true,
				collapsible:true,
				url:'<%=request.getContextPath()%>/mx/branch/role/json',
				remoteSort: false,
				singleSelect: true,
				idField:'id',
				columns:[[
	...
	function addNew(){
		var url="<%=request.getContextPath()%>/mx/branch/role/prepareAdd";
	    var width=450;
	    var height=280;
	    var scroll="no";
	...
	function onMyRowClick(rowIndex, row){
		...
		var url="<%=request.getContextPath()%>/mx/branch/role/prepareModify?id="+row.id;
	    var width=450;
	    var height=280;
	    var scroll="no";
	    //openWindow(url, width, height, scroll);
		jQuery('#edit_dlg').dialog('open').dialog('setTitle','角色编辑');
		jQuery('#editForm').form('load', '<%=request.getContextPath()%>/rs/branch/role/detail?id='+row.id);
	}
	...
	function editSelected(){
		...
		if (selected ){
			 jQuery('#edit_dlg').dialog('open').dialog('setTitle','角色编辑');
			 jQuery('#editForm').form('load', '<%=request.getContextPath()%>/rs/branch/role/detail?id='+selected.id);
		}
	}
	...
	function roleUsers(){
		...
		var selected = jQuery('#mydatagrid').datagrid('getSelected');
		if (selected ){
			 var url='<%=request.getContextPath()%>/mx/branch/role/roleUsers?id='+selected.id;
	...
	function editSelected2(){
		...
		var selected = jQuery('#mydatagrid').datagrid('getSelected');
		if (selected ){
			var url="<%=request.getContextPath()%>/mx/branch/role/prepareModify?id="+selected.id;
			var width=450;
			var height=280;
			var scroll="no";
			openWindow(url, width, height, scroll);
	...
	function saveData(){
		...
		jQuery.ajax({
				   type: "POST",
				   url: '<%=request.getContextPath()%>/rs/branch/role/'+res,
				   data: params,
				   dataType:  'json',
				   error: function(data){
					   //alert('服务器处理错误！');
	...
	function deleteSelections(){
		...
		if(ids.length > 0 && confirm("数据删除后不能恢复，确定删除吗？")){
			...
			jQuery.ajax({
				   type: "POST",
				   url: '<%=request.getContextPath()%>/rs/branch/role/batchDelete?rowIds='+rowIds,
				   dataType:  'json',
				   error: function(data){
					   if(data != null && data.message != null){
						   //alert(data.message);
