...
function add(){
  ...
  var link = "<%=request.getContextPath()%>/mx/branch/"+url;
  var width=520;
  var height=320;
  var scroll="no";
...
function roleUsers(form){
  ...
  var link = "<%=request.getContextPath()%>/mx/branch/role/roleUsers?id="+id;
  var width=520;
  var height=425;
  var scroll="no";
  //openWindow(url, width, height, scroll);
...
function roleMenus(form){
  ...
  var link = "<%=request.getContextPath()%>/mx/branch/role/roleMenus?roleId="+id;
  var width=520;
  var height=425;
  var scroll="no";
  //openWindow(url, width, height, scroll);
...
function del(){
  if(confirm("数据删除后不能恢复，确定删除吗？")){
	var form = document.all.iForm;
    form.target="hiddenFrame";
    form.action="<%=request.getContextPath()%>/mx/branch/role/batchDelete";
	form.submit();
  }
}
...
function sort(id, operate){  
  //SysRoleAjaxService.sort(id, operate, {callback:function (){reloadPage();}});
    	jQuery.ajax({
			type: "POST",
			url: '<%=request.getContextPath()%>/mx/branch/role/sort?id='+id+'&operate='+operate,
			dataType:  'json',
				error: function(data){
					alert('服务器处理错误！');
				},
...
function queryForm(){
	...
	iForm.method="post";
    iForm.action="<%=request.getContextPath()%>/mx/branch/role/showList?_rq_=1";
	iForm.submit();
}
...
<body style="padding-top:0px;padding-left:2px;padding-right:20px;">
