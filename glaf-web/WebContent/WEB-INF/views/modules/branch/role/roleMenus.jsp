...
<script type="text/javascript">
	 ...
	 var setting = {
			async: {
				enable: true,
				url: "<%=request.getContextPath()%>/rs/branch/role/roleMenusJson?roleId=${role.id}",
				dataFilter: filter
			},
			check: {
				enable: true
	...
	function saveRoleSiteMenus(){
		...
		jQuery.ajax({
				   type: "POST",
				   url: '<%=request.getContextPath()%>/rs/branch/role/saveRoleMenus?roleId=${role.id}',
				   dataType:  'json',
				   data: params,
				   error: function(data){
					   alert('服务器处理错误！');
	...
	function exportSiteMenus(){
        ...
        $("#nodeIds").val(sx);
        document.iForm.action="<%=request.getContextPath()%>/mx/branch/application/exportMenus";
...
&nbsp;
<input type="button" name="save" value=" 导 出 " class="button" onclick="javascript:exportSiteMenus();"> 
</div> 
<div>
