...
function searchRoles(){
      document.iForm.action="<%=request.getContextPath()%>/mx/branch/user/permission?op_view=role&parentId=${parentId}";
	  document.iForm.submit();
}

function searchUsers(){
      document.iForm.action="<%=request.getContextPath()%>/mx/branch/user/permission?op_view=user&parentId=${parentId}";
	  document.iForm.submit();
}
...
<table width="95%" height="30" border="0" cellspacing="1" cellpadding="0" class="mainTable">
  <tr>
    <td width="85%" align="right">
		...
		<c:choose>
         <c:when test="${op_view eq 'role'}">
	      <input type="button" value="查找" class="btnGray" onclick="javascript:searchRoles();">
		 </c:when>
		 <c:otherwise>
