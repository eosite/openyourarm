...
<script type="text/javascript">
  ...
  var setting = {
    ...
    function zTreeOnClick(event, treeId, treeNode, clickFlag) {
		...
		loadData('<%=request.getContextPath()%>/mx/branch/user/json?nodeId='+treeNode.id);
	}
   ...
   jQuery(function(){
	...
	function addNew(){
	    ...
	    var link="<%=request.getContextPath()%>/mx/branch/user/prepareAdd?nodeId="+nodeId+"&deptId=${deptId}";
	...
	function onRowClick(rowIndex, row){
		...
		var nodeId = jQuery("#nodeId").val();
	    var link = '<%=request.getContextPath()%>/mx/branch/user/prepareModify?nodeId='+nodeId+'&deptId=${deptId}&id='+row.id;
	...
	function resetPwd(){
	    ...
	    if (selected ){
		  //location.href="<%=request.getContextPath()%>/branch/user.do?method=edit&id="+selected.id;
		  var link = "<%=request.getContextPath()%>/mx/branch/user/prepareResetPwd?parent=${parent}&deptId=${deptId}&id="+selected.id;
	...
	function deleteSelections(){
		...
		if(ids.length > 0 && confirm("数据删除后不能恢复，确定删除吗？")){
			...
			jQuery.ajax({
				   type: "POST",
				   url: '<%=request.getContextPath()%>/mx/branch/user/delete?ids='+ids,
				   dataType:  'json',
				   error: function(data){
					   alert('服务器处理错误！');
				   },
