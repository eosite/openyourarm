...
<script type="text/javascript">
    ...
    function saveData(){
		...
		jQuery.ajax({
				   type: "POST",
				   url: "<%=request.getContextPath()%>/mx/sys/database/saveSort?items="+str,
				   dataType:  'json',
				   error: function(data){
					   alert('服务器处理错误！');
				   },
...
<body>
        ...
        <div id="example">
            ...
            <div id="pages">
                <div id="pages-title"><span>数据库列表</span></div>
                <div id="sortable">
				<c:forEach items="${list}" var="db">
				  <c:if test="${db.active == '1'}">
                    <div class="item">
                        <span class="handler" title="选中后拖到指定位置">&nbsp;</span>
                        <label id="${db.id}" title="选中后拖到指定位置">${db.title}[${db.dbname}]</label>
                    </div>
				  </c:if>
				</c:forEach> 
               </div>
