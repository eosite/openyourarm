      ...
      <tr >
		...
		<td valign="middle">
		  <c:choose>
			 ...
			 <c:when test="${p.inputType == 'imgbox'}">
			 	<input type="text" id="<c:out value="${p.name}"/>" name="<c:out value="${p.name}"/>" class="span3 x-text" 
			 	  value="<c:out value="${p.value}"/>"	onclick="javascript:openWindow('/glaf/mx/image/choose?elementId=<c:out value="${p.name}"/>', self, 50, 50, 1098, 520);" />
			 </c:when>
			 <c:otherwise>
