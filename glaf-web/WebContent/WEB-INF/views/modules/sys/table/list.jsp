   ...
   jQuery(function(){
	...
	function exportSQLite(){
		...
		if(ids.length > 0 ){
			...
			//var dbType = jQuery('#dbType').val();
			//jQuery("#tables").val(ids);
			window.open("<%=request.getContextPath()%>/mx/sys/tableMgr/exportDB?systemName=default&tables="+tables);
		} else {
			alert("请选择至少一条记录。");
		}
	}
	...
	function exportDB(){
		...
		if(ids.length > 0 ){
			...
			document.iForm.action="<%=request.getContextPath()%>/mx/sys/export/exportSQlite?tables="+tables;
			document.iForm.submit();
		} else {
			alert("请选择至少一条记录。");
		}
...
<div class="easyui-layout" data-options="fit:true">  
   <div data-options="region:'north',split:true,border:true" style="height:40px"> 
	...
	&nbsp;<span class="x_content_title">数据表</span>
	   ...
	   <a href="#" class="easyui-linkbutton" data-options="plain:true, iconCls:'icon-save'"
	      onclick="javascript:exportSQLite();">生成SQLite数据</a> 
