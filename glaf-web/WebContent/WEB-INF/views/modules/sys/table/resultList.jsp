...
<script type="text/javascript">
	var type_ = '${type}';
	var retFn_ = '${retFn}';
	var idColumnName_ = '${idColumn.columnName}';
	...
	function onMyRowClick(rowIndex, row){
	    var link = '<%=request.getContextPath()%>/mx/sys/table/edit?tableName_enc=${tableName_enc}&systemName=${systemName}&businessKey='+row.${idColumn.columnName};
	    var x=50;
        var y=50;
        if(is_ie) {
...
<template id="table_column_template">
...
<template id="table_template">
	<table id="easyui_data_grid" class="easyui-datagrid" style="width:1000px;height:445px"
			url="<%=request.getContextPath()%>/rs/system/table/resultList?tableName_enc=${tableName_enc}&gridType=easyui&systemName=${systemName}"
...
<template id="table_template_sqlite">
	<table id="easyui_data_grid" class="easyui-datagrid" style="width:1000px;height:445px"
			url="<%=request.getContextPath()%>/rs/system/table/resultList?tableName_enc=${tableName_enc}&gridType=easyui&systemName=${systemName}"
			title="${tableName} 数据列表 " iconCls="icon-list" remoteSort="true"  
			method="post" pageSize="15" pageList="[10,15,20,30,40,50,100,200,500]"
	...
	<div id="tb" style="padding:5px;height:auto">
		<a href="#" class="easyui-linkbutton" onclick="sureDataAndBack()" iconCls="icon-add" plain="true">确定返回</a>
	</div>
</template>
</head>
...
<div class="easyui-layout" data-options="fit:true">  
  <div data-options="region:'center',border:false" id="easyui_data_grid_panel">
...
<script type="text/javascript">
	var $table = "";
	if(type_ && type_ == 'sqlite'){
		$table = $($("#table_template_sqlite").html());
	}else{
		$table = $($("#table_template").html());
	}
	$table.find(">thead>tr").append($("#table_column_template").html());
	$("#easyui_data_grid_panel").append($table);
	
	function sureDataAndBack(){
		var selectedRows = $('#easyui_data_grid').datagrid('getSelections');
		var datalist = "";
		$.each(selectedRows,function(i,item){
			if(datalist){
				datalist += ",";
			}
			datalist += item[idColumnName_];
			// datalist.push(item[idColumnName_]);
		})
		var parent = window.parent || window;
		if(retFn_ && $.isFunction(parent[retFn_])){
			parent[retFn_]({
				idColumnName : idColumnName_,
				datalist : datalist,
			});
			return;
		}
