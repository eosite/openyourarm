...
<script type="text/javascript">
   ...
   jQuery(function(){
		jQuery('#mydatagrid').datagrid({
				...
				striped: true,
				collapsible:true,
				url:'<%=request.getContextPath()%>/mx/sys/application/json',
				remoteSort: false,
				singleSelect:true,
				idField:'id',
				columns:[[
	...
	function addNew(){
	    //location.href="<%=request.getContextPath()%>/mx/sys/application/edit";
		var nodeId = jQuery("#nodeId").val();
		var link = "<%=request.getContextPath()%>/mx/sys/application/prepareAdd?parent="+nodeId;
	    art.dialog.open(link, { height: 420, width: 600, title: "添加记录", lock: true, scrollbars:"yes" }, false);
	}

	function onRowClick(rowIndex, row){
        //window.open('<%=request.getContextPath()%>/mx/sys/application/edit&rowId='+row.id);
	    var link = '<%=request.getContextPath()%>/mx/sys/application/prepareModify?id='+row.id;
	...
	function editSelected(){
	    ...
	    if (selected ){
		var link = "<%=request.getContextPath()%>/mx/sys/application/prepareModify?id="+selected.id;
		art.dialog.open(link, { height: 480, width: 600, title: "修改记录", lock: true, scrollbars:"yes" }, false);
	    }
	}
	...
	function functionSettings(){
	    ...
	    var selected = jQuery('#mydatagrid').datagrid('getSelected');
	    if (selected ){
		  var link = "<%=request.getContextPath()%>/mx/sys/function/showFuncList?parent="+selected.id;
	...
	function viewSelected(){
		...
		var selected = jQuery('#mydatagrid').datagrid('getSelected');
		if (selected ){
		    location.href="<%=request.getContextPath()%>/mx/sys/application/prepareModify?id="+selected.id;
		}
	}
	...
	function deleteSelections(){
		...
		if(ids.length > 0 && confirm("数据删除后不能恢复，确定删除吗？")){
			...
			jQuery.ajax({
				   type: "POST",
				   url: '<%=request.getContextPath()%>/mx/sys/application/delete?rowIds='+rowIds,
				   dataType:  'json',
				   error: function(data){
					   alert('服务器处理错误！');
				   },
