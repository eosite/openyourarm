...
<input type="hidden" name="jobDataMap">
...
<table border="0" cellpadding="0" cellspacing="0"  class="table table-striped table-bordered table-condensed">
    ...
    <tr>
		<td width="25%" height="24">任务类型&nbsp;</td>
		<td height="24">
		 <select id="taskType" name="taskType" class="x-select" chname="任务类型">
			<option value="SYS">系统</option>
			<option value="eim">服务数据导入</option>
		 </select>
		</td>
	</tr>
	<tr id="eimServerInst" style="display:none;">
		<td width="25%" height="24">服务实例&nbsp;</td>
		<td height="24">
		 <select id="attribute" name="attribute" class="x-select"
			chname="服务实例">
   ...
   <tr>
	...
	<tr>
	 ...
	 <tr>
		<td width="25%" height="60">参数定义&nbsp;</td>
		<td height="50" >
			<table id="param-table" style="width: 700px; margin: 0px; padding: 0px; border-spacing: 0px;">
	...
	<div align="center">
	    <input name="btn_save" type="submit" value="保存" class="btn btn-primary">
		 ...
		 <input id="btn_return" type="button" value="返回" class="btn btn-primary">
	</div>
...
<tr>
				   ...
				   <td><input type="text" class="" field="keyName" /></td>
				   <td><input type="text" class="" field="stringVal" /></td>
...
<script type="text/javascript">
    ...
    $("#taskType").change(function(){
		  ...
		  if(val=="eim"){
			  $("#eimServerInst").show();
			  $("#jobClass_tr").hide();
			  $("#jobClass").val("com.glaf.teim.job.DataImpJob");
		  }else{
			  $("#eimServerInst").val("");
			  $("#attribute").val("");
			  $("#eimServerInst").hide();
	...
	$("#btn_return").click(function(){
		 if(confirm("确认返回吗？")){
			 window.location="${contextPath}/mx/system/scheduler";
		 }
	});
	$(function() {
		     //初始化任务类型
			 ...
			 //初始化服务实例
			 var attribute="${scheduler.attribute}";
			 ...
			 $.ajax({
				 type: "POST",
					   url: "<%=request.getContextPath()%>/rs/teim/dataimp/all",
					   dataType:  'json',
					   error: function(data){
						   console.log('获取数据错误！');
					   },
	...
	var ParamTable = (function(){
	   ...
	   <c:if test="${scheduler.jobDataMap != null  && fn:length(scheduler.jobDataMap) > 0}">
		 <c:forEach items="${scheduler.jobDataMap}" var="v">
			funcs.append(${v.value.toJsonObject()});
		 </c:forEach>
	   </c:if>
	...
	window.verifyAll = function(form){
		...
		form.jobDataMap.value = JSON.stringify(data);
