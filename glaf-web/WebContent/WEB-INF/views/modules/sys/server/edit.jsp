...
<script type="text/javascript">
	...
	function saveData(){
		...
		jQuery.ajax({
				   type: "POST",
				   url: '<%=request.getContextPath()%>/mx/sys/server/saveServer',
				   data: params,
				   dataType:  'json',
				   error: function(data){
					   alert('服务器处理错误！');
	...
	function verify(){
		...
		jQuery.ajax({
				   type: "POST",
				   url: '<%=request.getContextPath()%>/mx/sys/server/verify',
				   data: params,
				   dataType:  'json',
				   error: function(data){
					   alert('服务器处理错误！');
	...
	<tr>
		...
		<td align="left">
            <input id="password" name="password" type="password" 
			       class="easyui-validatebox  x-text" style="width:425px;" 
				   <c:if test="${!empty serverEntity.password}"> value="88888888" </c:if>  size="50"/>
		</td>
	</tr>
