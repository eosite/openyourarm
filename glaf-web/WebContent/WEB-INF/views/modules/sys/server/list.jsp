...
<script type="text/javascript">
   ...
   jQuery(function(){
		jQuery('#mydatagrid').datagrid({
				...
				striped: true,
				collapsible: true,
				url: '<%=request.getContextPath()%>/mx/sys/server/json?nodeId=${nodeId}',
				remoteSort: false,
				singleSelect: true,
				idField: 'id',
				columns:[[
	...
	function addNew(){
	    //var link="<%=request.getContextPath()%>/mx/sys/server/edit?fromUrl=${fromUrl}";
	    //art.dialog.open(link, { height: 420, width: 680, title: "添加记录", lock: true, scrollbars:"no" }, false);
		//location.href=link;
		link="<%=request.getContextPath()%>/mx/sys/server/edit?nodeId=${nodeId}";
		jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function onRowClick(rowIndex, row){
	    var link = '<%=request.getContextPath()%>/mx/sys/server/edit?id='+row.id+'&fromUrl=${fromUrl}';
	...
	function formatterKeys(val, row){
		var str = "<a href='javascript:editRow(\""+row.id+"\");'>修改</a>&nbsp;<a href='javascript:deleteRow(\""+row.id+"\");'>删除</a>&nbsp;<a href='javascript:verify(\""+row.id+"\");'>验证</a>&nbsp;<c:if test="${showIni == 'true'}"><a href='javascript:initDB(\""+row.id+"\");'>初始化</a>&nbsp;</c:if>";
	    return str;
	}
	...
	function initDB(id){
		if(confirm("初始化要执行很长一段时间，确定要初始化该库吗？")){
		  jQuery.ajax({
				   type: "POST",
				   url: '<%=request.getContextPath()%>/mx/sys/server/initServer?id='+id,
				   dataType:  'json',
				   error: function(data){
					   alert('服务器处理错误！');
				   },
	...
	function dataList(id){
       var link="<%=request.getContextPath()%>/mx/server/data?serverEntityId="+id;
	   location.href=link;
	}

	function editRow(id){
		link="<%=request.getContextPath()%>/mx/sys/server/edit?id="+id;
		jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	...
	function deleteRow(id){
		if(confirm("删除服务器配置将同时删除历史统计结果，删除后数据不能恢复，确定删除吗？")){
			jQuery.ajax({
				   type: "POST",
				   url: '<%=request.getContextPath()%>/rs/sys/server/delete?id='+id,
				   dataType:  'json',
				   error: function(data){
					   alert('服务器处理错误！');
				   },
	...
	function editSelected(){
	    ...
	    if (selected ){
		  var link = '<%=request.getContextPath()%>/mx/sys/server/edit?id='+selected.id+'&fromUrl=${fromUrl}';
		  //art.dialog.open(link, { height: 420, width: 680, title: "修改记录", lock: true, scrollbars:"no" }, false);
		  //location.href=link;
		  editRow(selected.id);
	    }
	...
	function viewSelected(){
		...
		var selected = jQuery('#mydatagrid').datagrid('getSelected');
		if (selected ){
		    location.href='<%=request.getContextPath()%>/mx/sys/server/edit?readonly=true&id='+selected.id+'&fromUrl=${fromUrl}';
		}
	}
	...
	function deleteSelections(){
		...
		if(ids.length > 0 ){
		  if(confirm("数据删除后不能恢复，确定删除吗？")){
			...
			jQuery.ajax({
				   type: "POST",
				   url: '<%=request.getContextPath()%>/mx/sys/server/delete?ids='+str,
				   dataType:  'json',
				   error: function(data){
					   alert('服务器处理错误！');
				   },
    ...
    function verify(id){
		jQuery.ajax({
				   type: "POST",
				   url: '<%=request.getContextPath()%>/mx/sys/server/verify2?id='+id,
				   dataType:  'json',
				   error: function(data){
					   alert('服务器处理错误！');
				   },
	...
	function reloadDB(){
	  if(confirm("确定重载配置服务器吗？")){
         jQuery.ajax({
				   type: "POST",
				   url: '<%=request.getContextPath()%>/mx/sys/server/reloadServer',
				   dataType:  'json',
				   error: function(data){
					   alert('服务器处理错误！');
				   },
	...
	function perm(){
		location.href="<%=request.getContextPath()%>/mx/sys/server/permission";
	}
