...
<script type="text/javascript">
	 ...
	 var setting = {
			async: {
				enable: true,
				url: "<%=request.getContextPath()%>/rs/sys/role/authorityUsersJson?roleId=${sysRole.id}",
				dataFilter: filter
			},
			check: {
				enable: true
	...
	function saveAuthorityUsers(){
		...
		jQuery.ajax({
				   type: "POST",
				   url: '<%=request.getContextPath()%>/rs/sys/role/saveAuthorityUsers?roleId=${sysRole.id}',
				   dataType:  'json',
				   data: params,
				   error: function(data){
					   alert('服务器处理错误！');
...
<span class="x_content_title">授权角色【${sysRole.name}】给用户</span>
&nbsp;
<input type="button" name="save" value=" 保 存 " class="button" onclick="javascript:saveAuthorityUsers();"> 
</div> 
<div>
