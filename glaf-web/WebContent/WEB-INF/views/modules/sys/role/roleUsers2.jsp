...
<script type="text/javascript">
	...
	function saveMyFormData(){
		...
		jQuery.ajax({
				   type: "POST",
				   url: '<%=request.getContextPath()%>/mx/sys/role/saveRoleUsers',
				   data: params,
				   dataType:  'json',
				   error: function(data){
					   alert('服务器处理错误！');
...
<div class="easyui-layout" data-options="fit:true">  
  ...
  <form id="iForm" name="iForm" method="post">
    ...
    <ul id="tt" class="easyui-tree" data-options="url:'<%=request.getContextPath()%>/mx/sys/role/roleUsersJson?roleCode=${sysRole.code}',animate:true,checkbox:true,onlyLeafCheck:true"></ul>  
  </form>
</div>
</div>
<script type="text/javascript">
