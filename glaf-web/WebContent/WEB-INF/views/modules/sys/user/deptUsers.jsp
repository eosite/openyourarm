...
var setting = {
    ...
    function zTreeOnClick(event, treeId, treeNode, clickFlag) {
		...
		loadData('<%=request.getContextPath()%>/mx/sys/user/json?nodeId='+treeNode.id);
	}
   ...
   jQuery(function(){
		jQuery('#mydatagrid').datagrid({
				...
				striped: true,
				collapsible:true,
				url:'<%=request.getContextPath()%>/mx/sys/user/json?parent=${parent}&deptId=${deptId}',
				sortName: 'id',
				sortOrder: 'desc',
				remoteSort: false,
				singleSelect:true,
	...
	function addNew(){
	    //location.href="<%=request.getContextPath()%>/mx/sys/user/edit";
		var nodeId = jQuery("#nodeId").val();
	    var link="<%=request.getContextPath()%>/mx/sys/user/prepareAdd?nodeId="+nodeId+"&deptId=${deptId}";
	...
	function onRowClick(rowIndex, row){
		...
		var nodeId = jQuery("#nodeId").val();
	    var link = '<%=request.getContextPath()%>/mx/sys/user/prepareModify?nodeId='+nodeId+'&deptId=${deptId}&id='+row.actorId_enc;
	...
	function deleteSelections(){
		...
		if(ids.length > 0 && confirm("数据删除后不能恢复，确定删除吗？")){
			...
			jQuery.ajax({
				   type: "POST",
				   url: '<%=request.getContextPath()%>/mx/sys/user/delete?ids='+ids,
				   dataType:  'json',
				   error: function(data){
					   alert('服务器处理错误！');
				   },
