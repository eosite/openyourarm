...
<script type="text/javascript">
    ...
    function zTreeOnClick(event, treeId, treeNode, clickFlag) {
		...
		loadData('<%=request.getContextPath()%>/mx/sys/dictory/json?nodeId='+treeNode.id);
	}
   ...
   jQuery(function(){
	...
	function addNew(){
		...
		var link = "<%=request.getContextPath()%>/mx/sys/dictory/prepareAdd?parent="+nodeId;
	...
	function onRowClick(rowIndex, row){
		var nodeId = jQuery("#nodeId").val();
	    var link = '<%=request.getContextPath()%>/mx/sys/dictory/prepareModify?id='+row.id;
	...
	function editSelected(){
	    ...
	    if (selected ){
		  //location.href="<%=request.getContextPath()%>/mx/sys/dictory/edit&rowId="+selected.id;
		  var link = "<%=request.getContextPath()%>/mx/sys/dictory/prepareModify?id="+selected.id;
		  art.dialog.open(link, { height: 400, width: 480, title: "修改字典", lock: true, scrollbars:"yes" }, false);
		  //openWindow(link, 400, 480, "yes");
	    }
	}
	...
	function viewSelected(){
		...
		var selected = jQuery('#mydatagrid').datagrid('getSelected');
		if (selected ){
		    location.href="<%=request.getContextPath()%>/mx/sys/dictory/prepareModify?id="+selected.id;
		}
	}
	...
	function deleteSelections(){
		...
		if(ids.length > 0 && confirm("数据删除后不能恢复，确定删除吗？")){
			...
			jQuery.ajax({
				   type: "POST",
				   url: '<%=request.getContextPath()%>/mx/sys/dictory/delete?rowIds='+rowIds,
				   dataType:  'json',
				   error: function(data){
					   alert('服务器处理错误！');
				   },
