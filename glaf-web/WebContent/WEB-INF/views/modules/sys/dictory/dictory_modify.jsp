...
<html:form id="editForm" action="${contextPath}/mx/sys/dictory/saveModify" method="post" 
...
<div class="easyui-panel" title="修改数据字典" style="width:550px;padding:10px">  
 <table width="95%" align="center" border="0" cellspacing="0" cellpadding="5">
	  ...
	  <tr>
        ...
        <td>
        <textarea id="desc" name="desc"  class="k-textbox" data-bind="value: desc" 
		          style="height:60px; width:375px;">${bean.getDesc() != null ? bean.getDesc() : ""}</textarea>
        ...
        <td>
		   <c:choose>
		     <c:when test="${a.type=='String'}">
			    <input type="text" name="${a.name}"  datatype="string" nullable="${a.nullable}" maxsize="${a.length}" chname="${a.title}" value="${a.value}" size="50"
				class="easyui-validatebox"
				<c:if test="${a.required==1}">data-options="required:true"</c:if>
				>
             </c:when>
			 <c:when test="${a.type=='Date'}">
			    <input type="text" name="${a.name}" class="easyui-datebox" datatype="datetime" nullable="${a.nullable}" maxsize="${a.length}" chname="${a.title}"  size="50"
			 ...
			 <c:when test="${a.type=='Long'}">
			    <input type="text" name="${a.name}" datatype="integer" nullable="${a.nullable}" maxsize="12" chname="${a.title}" value="${a.value}" size="50"
				class="easyui-validatebox"
				<c:if test="${a.required==1}">data-options="required:true"</c:if>
				>
             </c:when>
			 <c:when test="${a.type=='Double'}">
			    <input type="text" name="${a.name}" datatype="double" nullable="${a.nullable}" maxsize="20" chname="${a.title}" value="${a.value}" size="50"
				class="easyui-validatebox"
				<c:if test="${a.required==1}">data-options="required:true"</c:if>
				>
             </c:when>
