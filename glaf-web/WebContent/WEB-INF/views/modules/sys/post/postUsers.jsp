...
<script type="text/javascript">
	 ...
	 var setting = {
			async: {
				enable: true,
				url: "<%=request.getContextPath()%>/rs/sys/post/usersJson?postId=${post.id}",
				dataFilter: filter
			},
			check: {
				enable: true
	...
	function saveRoleUsers(){
		...
		jQuery.ajax({
				   type: "POST",
				   url: '<%=request.getContextPath()%>/rs/sys/post/saveUsers?postId=${post.id}',
				   dataType:  'json',
				   data: params,
				   error: function(data){
					   alert('服务器处理错误！');
...
<span class="x_content_title">岗位【${post.name}】的用户</span>
&nbsp;
