...
function del(form){
  if(confirmDelete(form)){
    ...
    form.action="function/batchDelete";
	form.submit();
  }
}
...
function sort(id, operate){  
 // SysFunctionAjaxService.sort(id, operate, {callback:function (){reloadPage();}});
       jQuery.ajax({
			type: "POST",
			url: '<%=request.getContextPath()%>/sys/function/sort?id='+id+'&operate='+operate,
			dataType:  'json',
				error: function(data){
					alert('服务器处理错误！');
				},
...
<body>
<div class="nav-title"><label class="Title">模块管理</label><label class="Title">&gt;&gt;功能列表</label></div>
<html:form action="${contextPath}/mx/sys/function/batchDelete" method="post" target="_self"> 
<input type="hidden" name="parent" value="<%=parent%>">
<input type="hidden" name="id" value="0">
<input type="hidden" name="funcName" value="">
<input type="hidden" name="funcMethod" value="">
<input type="hidden" name="code" value="">
<table style="width: 80%;" border="12" cellspacing="2" cellpadding="10" class="table table-bordered" align="center">
<thead>
  <tr style='background-color:#e6f2f8;font-size:15px;'> 
    ...
    <th style="text-align:center;width: 15%">功能名称</th>
    <th style="text-align:center;width: 65%">调用方法</th>
    <th style="text-align:center;width: 15%">权限代码</th> 
  </tr>
  </thead>
<%
int i=0;
...
<table width="80%" border="0" align="center" cellpadding="0" cellspacing="0">
  <tr> 
    <td width="50%"><input name="btn_new" type="button" value="新增" class="button btn-primary" onclick="javascript:create();">
      <input name="btn_del" type="button" value="删除" class="button btn-danger" onclick="javascript:del(this.form);" disabled>
...
<html:form action="${contextPath}/mx/sys/function/saveAdd" method="post" target="hiddenFrame" onsubmit="return verifyAll(this);" > 
<input type="hidden" name="parent" value="<%=parent%>">
<table width="80%" border="0" align="center" cellpadding="0" cellspacing="0" class="box">
  ...
  <tr>
    <td> 
       ...
       <label class="col-xs-3 " for="funcName" style="text-align: right;">功能名称*</label>
          <div class="col-xs-9">
          <input name="funcName" type="text" size="80" class="form-control" datatype="string" nullable="no" maxsize="80" chname="功能名称" style="width:300px;display: inline-block;">
          </div>
         </div>
		  
		  <div class="form-group row">
       <label class="col-xs-3 " for="funcName" style="text-align: right;">调用方法*</label>
          <div class="col-xs-9">
          <input name="funcMethod" type="text" size="80" class="form-control" datatype="string" nullable="no" maxsize="200" chname="调用方法" style="width:300px;display: inline-block;">
          </div>
         </div>
		  
		  <div class="form-group row">
           <label class="col-xs-3 " for="funcName" style="text-align: right;">权限代码*</label>
          <div class="col-xs-9">
          <input name="code" type="text" size="80" class="form-control" datatype="string" nullable="no" maxsize="50" chname="权限代码" style="width:300px;display: inline-block;">
          </div>
         </div>
		  
		  <tr>
			<td><input name="btn_save" type="submit" value="保存" class="button btn-success"></td>
			<td>&nbsp;</td>
		  </tr>
		</table>
	</td>
...
<form name="modifyForm" action="${contextPath}/mx/sys/function/saveModify" method="post" target="hiddenFrame"
