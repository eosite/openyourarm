   ...
   jQuery(function(){
	...
	function viewSelected(){
		...
		if (selected ){
			...
			//alert('<%=request.getContextPath()%>/mx/sys/cacheMgr/detail?key='+selected.name+'&region='+selected.region);
            jQuery.ajax({
				   type: "GET",
				   url: '<%=request.getContextPath()%>/mx/sys/cacheMgr/detail?key='+selected.name+'&region='+selected.region,
				   dataType:  'json',
				   error: function(data){
					   alert('服务器处理错误！');
				   },
