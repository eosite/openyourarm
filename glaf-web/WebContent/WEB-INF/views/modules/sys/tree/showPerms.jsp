...
<script language="javascript">
    ...
    function saveForm(){
		...
		jQuery.ajax({
				   type: "POST",
				   url: '<%=request.getContextPath()%>/mx/sys/tree/savePerms',
				   dataType: 'json',
				   data: params,
				   error: function(data){
					   alert('服务器处理错误！');
	...
	function switchPerms(){
		...
		location.href="<%=request.getContextPath()%>/mx/sys/tree/showPerms?nodeId=<%=bean.getId()%>&entryKey="+entryKey;
	}
</script>
</head>
...
<html:form id="iForm" name="iForm" action="${contextPath}/mx/sys/tree/savePerms" method="post"  > 
