...
<script type="text/javascript">
   ...
   $(function(){
	$('#myTreeGrid').treegrid({
       ...
       //autoRowHeight:true,
       //resizable:true,
       url:'${contextPath}/mx/sys/department/treegridJson?parentId=5',
       idField:'id',
       treeField:'deptName',
	...
	$('#userGrid').datagrid({
       url: '${contextPath}/mx/sys/user/json?parent=${parent}&deptId=${deptId}',
       width: 700,
       height: 'auto',
       fitColumns:true,
       fit:true,
	   ...
	   $('#roleGrid').datagrid({
       url: '${contextPath}/mx/sys/department/deptRolesJson?nodeId=5',
       width: 700,
       height: 'auto',
	   fit:true,
       fitColumns:true,
  ...
  function onUserRowClick(rowIndex, row){
      var link = '${contextPath}/mx/sys/user/showUser?parent=${parent}&deptId=${deptId}&userId='+row.id;
	  art.dialog.open(link, { height: 420, width: 520, title: "查看用户信息", lock: true, scrollbars:"no" }, false);
  }

  function onRoleRowClick(rowIndex, row){
      var link = '${contextPath}/mx/sys/department/showDeptRoleUsers?nodeId='+row.nodeId+'&roleCode='+row.code;
	...
	function onTreeGridClick(row){
       //alert("点击了\n"+row.indexName+"\nid="+row.indexId);
	    var url = '${contextPath}/mx/sys/department/treegridJson?parentId='+row.id;
        loadData(row, url);
		updateUserGrid(row);
		updateRoleGrid(row);
	}
	...
	function updateUserGrid(row){
		//alert("点击了\n"+row.indexName+"\nid="+row.indexId);
        var url = '${contextPath}/mx/sys/user/json?parent='+row.deptId;
	...
	function updateRoleGrid(row){
		//alert("点击了\n"+row.indexName+"\nid="+row.indexId);
        var url = '${contextPath}/mx/sys/department/deptRolesJson?nodeId='+row.id;
	    $.post(url,{qq:'mike'},function(data){
        //var text = JSON.stringify(data); 
        //alert(text);
