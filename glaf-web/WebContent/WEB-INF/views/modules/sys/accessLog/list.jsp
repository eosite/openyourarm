...
<script type="text/javascript">
   ...
   jQuery(function(){
		jQuery('#mydatagrid').datagrid({
				...
				striped: true,
				collapsible: true,
				url: '<%=request.getContextPath()%>/mx/sys/accessLog/json?startTime=${startTime}&endTime=${endTime}&searchWords_enc=${searchWords_enc}',
				remoteSort: false,
				singleSelect: true,
				idField: 'id',
				columns:[[
    ...
    function searchData2(){
        ...
        jQuery.ajax({
                    type: "POST",
                    url: '<%=request.getContextPath()%>/mx/sys/accessLog/json',
                    dataType:  'json',
				    data: params,
                    error: function(data){
                            alert('服务器处理错误！');
	...
	function deleteYesterday(){
		if(confirm("数据删除不能恢复，确定删除一天以前的访问日志吗？")){
          jQuery.ajax({
                    type: "POST",
                    url: '<%=request.getContextPath()%>/rs/sys/accessLog/deleteYesterday',
                    dataType:  'json',
                    error: function(data){
                            alert('服务器处理错误！');
                    },
	...
	function deleteLastWeek(){
		if(confirm("数据删除不能恢复，确定删除一周以前的访问日志吗？")){
          jQuery.ajax({
                    type: "POST",
                    url: '<%=request.getContextPath()%>/rs/sys/accessLog/deleteLastWeek',
                    dataType:  'json',
                    error: function(data){
                            alert('服务器处理错误！');
                    },
    ...
    function viewDenyList(){
		var link="<%=request.getContextPath()%>/mx/sys/accessLog/denyList";
		jQuery.layer({
			type: 2,
			maxmin: true,
			shadeClose: true,
	 ...
	 <tr>
		...
		<td>
	     &nbsp;<a href="#" class="easyui-linkbutton" data-options="plain:true, iconCls:'icon-user'"
	              onclick="javascript:viewDenyList();">黑名单列表</a>
		</td>
		<td>
	     &nbsp;<a href="#" class="easyui-linkbutton" data-options="plain:true, iconCls:'icon-remove'"
	              onclick="javascript:deleteLastWeek();">删除一周以前的日志</a>
	    </td>
		<td>
         &nbsp;<a href="#" class="easyui-linkbutton" data-options="plain:true, iconCls:'icon-remove'"
	         onclick="javascript:deleteYesterday();">删除一天以前的日志</a>
		</td>
        <td>
