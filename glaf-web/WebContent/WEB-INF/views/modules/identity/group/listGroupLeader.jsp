...
<script type="text/javascript">
   ...
   jQuery(function(){
		jQuery('#mydatagrid').datagrid({
				...
				striped: true,
				collapsible:true,
				url:'<%=request.getContextPath()%>/mx/identity/group/listGroupLeaderJson?groupId=${groupId}',
				remoteSort: false,
				//singleSelect:true,
				idField:'userId',
				nowrap:true,
				columns:[[
					{title:'', field:'xstartIndex',checkbox:true, width:40},
	                {title:'序号',field:'startIndex',width:40,sortable:true,align:'center'},
					...
					{title:'部门',field:'dname', width:120,sortable:true,halign:'center'}
				]],
				rownumbers:false,
				pagination:false,
				pageSize:15,
	...
	function deleteSelections(){
		...
		if(confirm("数据删除后不能恢复，确定删除吗？")){
			...
			jQuery.ajax({
				   type: "POST",
				   url: '<%=request.getContextPath()%>/mx/identity/group/batchDeleteGroupLeader?groupId=${groupId}&userIds='+rowIds,
				   dataType:  'json',
				   error: function(data){
	...
	function addUser(){
		  var link = "<%=request.getContextPath()%>/mx/base/identityChoose/chooseUsersInsert?actionUrl=/mx/identity/group/saveGroupLeaders?type=groupLeaderAddUser&rowId=${groupId}";
		  var width=700;
		  var height=400;
		  var scroll="yes";
