 ...
 function clearRow2(prefix){
  ...
  function submitRequest(){
		if(confirm("确定要保存数据吗？")){
			  ...
			  jQuery.ajax({
				   type: "POST",
				   url: '<%=request.getContextPath()%>/mx/identity/agent/saveAgents',
				   data: params,
				   error: function(data){
					   alert('服务器处理错误！');
				   },
...
<input type="hidden" id="assignFrom" name="assignFrom" value="<c:out value="${assignFrom}" />"> 
<input type="hidden" id="checkFlag" name="checkFlag" value="1">
<div align="right" style="width: 90%">
<input type="button" value="保存设置" class="button" id="confirm" name="confirm"
	   onclick="javacsript:submitRequest();"> 
</div>
<br>
<table class="treeTable" width="90%" cellspacing="1" cellpadding="4">
	<thead>
		<tr class="x-title">
	...
	<tbody>
		<tr id="node_0" class="x-node-content">
			<td noWrap><img
				src="<%=request.getContextPath()%>/images/darrow.gif">&nbsp;
			<label title="点击展开">全局代理</label></td>
			...
			<input type="text" id="global_agentNames" name="global_agentNames"
				...
				onclick="javacsript:selectUsers('iForm', 'global_agentIds','global_agentNames', '', '');"
				size="15" style="cursor: pointer;" class="x-text" readonly>
			&nbsp; <img
			...
			<td noWrap><input type="text" id="global_startDate"
				name="global_startDate" style="cursor: pointer;" class="x-text"
				readonly
			...
			<td noWrap><input type="text" id="global_endDate"
				name="global_endDate" style="cursor: pointer;" class="x-text"
				readonly
 ...
 <%
   ...
   if(processDefinitions != null && processDefinitions.size() > 0){
	   ...
	   while(iter99.hasNext()){
		   TreeModel pd = (TreeModel) iter99.next();
  ...
  %>
		<tr id="node_<%=pd.getId()%>" class="x-node-content">
			<td noWrap><img
				src="<%=request.getContextPath()%>/images/darrow.gif">&nbsp;
			<label title="点击展开"><%=pd.getDescription() != null ? pd.getDescription() : pd.getName()%></label></td>
			...
			<input type="text" id="<%=pd.getName()%>_name"
				...
				class="x-text" readonly
				value="<%=RequestUtils.getAttribute(request, "x_" + pd.getName() + "_agentNames")%>"
				onclick="javacsript:selectUser('iForm', '<%=pd.getName()%>','<%=pd.getName()%>_name');"
		...
		<%
		   if(dataMap != null && dataMap.get(pd.getName()) != null){
			 ...
			 if(tasks != null && tasks.size() > 0) {
				 ...
				 while(iter9.hasNext()){
					 TreeModel task = (TreeModel) iter9.next();
					 pageContext.removeAttribute("startDate");
					 pageContext.removeAttribute("endDate");
					 pageContext.removeAttribute("locked");
	  ...
	  %>
		<tr id="node_<%=pd.getId()%>_<%=task.getId()%>" class="child-of-node_<%=pd.getId()%> x-node-content">
			...
			<input type="text" id="<%=pd.getName()%>_<%=task.getName()%>_name"
				...
				style="cursor: pointer;" class="x-text" readonly
				value="<%=RequestUtils.getAttribute(request, "x_" + pd.getName()+ "_"+task.getName() + "_agentNames")%>"
				onclick="javacsript:selectUser('iForm', '<%=pd.getName()%>_<%=task.getName()%>','<%=pd.getName()%>_<%=task.getName()%>_name');"
