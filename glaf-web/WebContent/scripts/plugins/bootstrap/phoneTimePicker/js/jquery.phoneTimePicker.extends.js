(function($, window, document, undefined) {
	var plugin = "phoneTimePicker", optionKey = plugin + ".options",
	...
	$.fn[plugin] = function(options, param) {
		if (typeof options == 'string' && (options in $.fn[plugin].methods)) {
			...
			if (!plugin) {
				this.data("phoneTimePicker", new Plugin(this, params));
            } 
...
//				var jq = methods.getOject.call(this);
				if($(this.target).data("phoneTimePicker.options")&&val!=""){
//					var opts = $(this).data("datetimepickerbt").options;
...
//					var dateStr = dateFormat(val,format);
					...
					if(val){
						var time,
						    options = $(this.target).data("phoneTimePicker.options").options;
						options.dateTimeStr = val;
//						if(pageParameters){
//							time = JSON.parse(pageParameters).sys_date||options.time;
//						}
						this.target.find("input[id=time]").val(this.dateFormat(val,options.format));
					}else{
...
//					var date = dateFormat(options.time,format);
			...
			tdisabled : function(){
				this.target.find("input[id=time]").datetimePicker("disable");
				this.target.find("input[id=time]").attr("readonly","readonly");
