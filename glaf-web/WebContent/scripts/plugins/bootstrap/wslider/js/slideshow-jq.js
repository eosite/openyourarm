...
function generateHTML(elementID, rawOptions) {
	...
	return "<div id='" + elementID + "' style='height:1000px'></div>"; // to maintain scroll state
}
...
function updatePhotos(elementID, photos) { // TODO: rename to updateImages
	if (slideshows[elementID]) {
		...
		slideshows[elementID] = new Slideshow($('#' + elementID + ''), options);
...
function render(rawOptions) {
	...
	var slideshow = new Slideshow($('#' + elementID + ""), options);
	slideshows[elementID] = slideshow;
	slideshow.render();
}
...
function Slideshow(element, options) {
	...
	function renderLink(photo, i) {
		...
		var a = td.find('a');
		var j = td.find('.wslide-link-inner1');
		j.css("width",photo.photoWidth);
		j.css("height",photo.photoHeight);
		if (isTouch) {
	...
	function sizeImage(i) {
		...
		img.width(options.images[0].width);
		img.height(options.images[0].height);
		imgWrap.css('width', options.images[0].width);
		imgWrap.css('height', options.images[0].height);
		imgWrap.css('left', -Math.round(options.images[0].width / 2 - 50) );
