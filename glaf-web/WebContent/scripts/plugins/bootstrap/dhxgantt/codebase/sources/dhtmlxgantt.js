...
gantt._render_grid_header = function () {
	...
	for (var i = 0; i < columns.length; i++) {
		...
		if(col.width && typeof col.width == 'string' && col.width.indexOf("%")){
...
gantt._render_bg_canvas = function(){
	...
	gantt.$task_bg.appendChild(fragment);
	...
	function getUrlFromCache(key){
		if(!gantt._canvas_bg_cache) {
			gantt._canvas_bg_cache = {};
			return null;
		}

		return gantt._canvas_bg_cache[key] || null;
	}
	...
	function getStyleElement(){
		if(!gantt._staticBgStyleId){
			gantt._staticBgStyleId = "gantt-static-bg-styles-" + gantt.uid();
		}

		var style = document.getElementById(gantt._staticBgStyleId);

		if(!style){
			style = document.createElement("style");
			style.id = gantt._staticBgStyleId;
	...
	function cacheUrl(key, url){
		...
		gantt._canvas_bg_cache[key] = url;
	}
	...
	function createBgDivs(bgTiles){
		...
		for (var i = 0; i < widths.length; i++){
			...
			if ((cell != prevCell && prevCell !== undefined) || (i == widths.length - 1) || (prevWidth > maxTileSize)){
				...
				var totalHeight = gantt.config.row_height * gantt._order.length,
					currentTop = 0,
					rowHeight = Math.floor(maxTileSize / gantt.config.row_height) * gantt.config.row_height;
