		...
		return this.each(function(){
			...
			this.createAgoCommentHtml = function(){
				...
				$.each(para.agoComment, function(k, v){
					...
					var componentID = v.componentID;
					var option = $("#"+v.componentID).data("reviewArea").option;
					var item = '';
					item += '<div id="comment'+v.id+'" class="comment">';
					item += '	<a class="avatar">';
					if(v.flag){
						if(v.datasetId != undefined){
							item += '		<img src="'+contextPath +'/mx/form/imageUpload?method=downloadById&id='+v.datasetId+'">';
						}
						else{
							if(v.imgUrl != undefined){
								item += '		<img src="'+contextPath + imgeUrl+'">';
							}
							else{
								item += '		<img src="'+contextPath+'/scripts/plugins/bootstrap/reviewArea/image/foot.png">';
							}
						}
					}		
					item += '	</a>';
					...
					item += '			<a class="reply"  selfID="'+v.id+'" >回复</a>';
					item += '		</div>';
					item += '	</div>';
					item += '</div>';
			...
			this.addNewComment = function(param){
				...
				item += '		<div class="actions">';
				item += '			<a class="reply" href="'+this.option.event.reply(this)+'" selfID="'+param.id+'" >回复</a>';
				item += '		</div>';
				item += '	</div>';
				item += '</div>';
