(function($, window, document, undefined) {
	var plugin = "photoSwipe", optionKey = plugin + ".options",
	...
	$.fn[plugin] = function(options, param) {
		if (typeof options == 'string' && (options in $.fn[plugin].methods)) {
			...
			if (!plugin) {
				this.data("photoSwipe", new Plugin(this, params));
            } 
	...
	$.fn[plugin].methods = {
		...
		__renderTemplate : function($target,$opt,ret){
			...
			$($target[0]).find(".demo-gallery").empty();
			
			$.each(ret[0].data,function(i,data){	
				       
					if(pldataSource[0].imgUrl != undefined){
						var element = "<a href=\""+contextPath + data[pldataSource[0].imgUrl.en]+"\" " +
			    		"data-size=\"1600x1067\" data-med=\""+data[pldataSource[0].imgName[0].en]+"\" data-med-size=\"1024x683\"> " +
			    		"<img style=\"width:"+$opt.width+";height:"+$opt.height+"\" src=\""+contextPath + data[pldataSource[0].imgUrl.en]+"\" alt=\"\" />" +
			    		"<figure>"+data[pldataSource[0].imgName[0].en]+"</figure></a>";
					} else{
						var thumbPath = contextPath +"/mx/form/imageUpload?method=downloadById&id="+data[pldataSource[0].datasetId[0].en];
						var name = pldataSource[0].imgName != undefined ? data[pldataSource[0].imgName[0].en] : undefined;
						var element = "<a href=\""+thumbPath+"\" " +
			    		"data-size=\"1600x1067\" data-med=\""+ name +"\" data-med-size=\"1024x683\"> " +
			    		"<img style=\"width:"+$opt.width+";height:"+$opt.height+"\" src=\""+thumbPath+"\" alt=\"\" />" +
			    		"<figure>"+ name +"</figure></a>";
					}
					$($target[0]).find(".demo-gallery").append(element);
		     });
		},
		__linkageControl : function(that,$target,param){
			var $opt = that.option;
			$opt.read.data = JSON.stringify({
				rid : $opt.rid,
