...
$.fn.rangeCalendar = function(options) {
	...
	this.each(function(i, el) {
		...
		obj.didSelectMonth = function(e) {
			...
			obj._placeElement(obj.calendarObj,monthPosition,currentMonthId);
			obj._dispatchEvent(obj.changeRangeCallback,obj.range(),obj);	
		...
		obj._initCalendar = function() {
		    ...
		    obj.calendarObj.draggable({ 
		      	...
		      	cursor: "move",
			  		...
			  		start: function(e, ui) {
			  			if(obj.calendarObj.data("isSlidering")){
			  				return false;
			  			}
			  			xpos = ui.position.left;
			  			obj.calendarObj.data("startleft",xpos);
			  		  	$(window).unbind("resize"); //Prevents window.resize event triggering
			  		  	obj.isDragging = true;
		...
		obj._getCalendarHTML = function(startDate,endDate) {
			...
			for (var index = 1; (date.isBefore(endDate) || date.isSame(endDate)) ; index++){
				...
				var style = "style='";
				if(obj.cellWidth){
					style += "width:"+obj.cellWidth+"px;";
				}
				style += "'";
					
				cell = '<div class="cal-cell cell" '+style+' date-id="'+fullYear+monthNumber+dayNumber+'" month-id="'+fullYear+''+monthNumber+'" month="'+monthNumber+'">';
		...
		obj._generateView = function () {
			...
			(obj.hideMonths ? obj.monthsObj.addClass("hide-months") : '');
		...
		obj._init = function( element,options ) {
			...
			obj.hideMonths = obj.options.hideMonths;
			obj.cellWidth = obj.options.cellWidth || "";
		...
		if(obj.options.onSuccessFunc){
			obj.options.onSuccessFunc(obj);
		}
	});
