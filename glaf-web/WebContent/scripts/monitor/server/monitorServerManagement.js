...
function PageManageFunc() {
    this.pageManage = {
        ...
        _gobalParam: {
            ...
            treeQueryUrl: contextPath + '/mx/monitor/monitorCategory/treeData',
        ...
        _openPublishTerminate : function(title,data){
            ...
            var dialog = BootstrapDialog.show({
                ...
                onshown: function(dialog) {
                    ...
                    var $serverUrl = $templateHtml.find("[name='serverUrl']");
                    var $clientUrl = $templateHtml.find("[name='clientUrl']");

                    $serverUrl.val("http://192.168.1.122:8080/glaf/website/monitor/Server/setInfo");
                    $clientUrl.val("http://192.168.1.55:18001/glaf/website/monitor/client/buildConnection");

                    $templateHtml.find("#sureBtn").click(function(event){
                        var $btn = $(this);

                        var serverUrl = $templateHtml.find("[name='serverUrl']").val();
                        var clientUrl = $templateHtml.find("[name='clientUrl']").val();
                        if(serverUrl && serverUrl){
                            ...
                            var url = contextPath + "/mx/momitor/server/buildConnection";
                            var option = {
                                url: url,
                                async: false,
                                type: 'post',
        ...
        _openSetWarningItemDialog : function(title,data,type){
            ...
            var dialog = BootstrapDialog.show({
                ...
                onshown: function(dialog) {
                    ...
                    // var params = "{}";
                    var params = JSON.stringify({"terminalId" : data.id,"type":type});
                    var gridSetting = $.extend(true,{},that.gridSetting);
                    ...
                    $modalBody.find(".addBtn").click(function(event){
                        ...
                        if(type == 2){
                            that._openEditWarningItemDialog("新增基础属性",selectedNode,scallback,type);
                        }else{
                            that._openEditWarningItemDialog("新增预警项",selectedNode,scallback,type);
                        }
                    })
                    $modalBody.find(".editBtn").click(function(event){
                        //修改预警项
                        ...
                        selectedNode.terminalId = data.id;
                        if(type == 2){
                            that._openEditWarningItemDialog("修改基础属性",selectedNode,scallback,type);
                        }else{
                            that._openEditWarningItemDialog("修改预警项",selectedNode,scallback,type);                          
                        }
                    })
                    $modalBody.find(".deleteBtn").click(function(event){
                        //删除预警项
         ...
         * @param  {[type]} data      [渲染数据]
         * @param  {[type]} scallback [编辑成功回调，用于重载表格]
         * @param  {[type]} type [预警项或基础属性,1为预警项，2为基础属性]
         * @return {[type]}           [description]
         */
        _openEditWarningItemDialog : function(title,data,scallback,type){
            var that = this;
            var data = data || {};
            var scallback = scallback;
            //编辑预警项
            ...
            var dialog = BootstrapDialog.show({
                ...
                onshown: function(dialog) {
                    ...
                    ztreeSetting.callback.onAsyncSuccess = function(event, treeId, treeNode, msg) {
                        ...
                        $.each(expandNodes,function(i,item){
                            that.dialog.treeObj.expandNode(item, true,false);  
                        })
                    }
