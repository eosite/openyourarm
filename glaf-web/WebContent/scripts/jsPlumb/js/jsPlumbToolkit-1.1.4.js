...
// if not an inline if, parse a normal attribute.
    ...
    _e(RotorsInstance.prototype, {
        ...
        update:function(el, data) {
            ...
            el.style.backgroundColor = data.color == '' ? '#3E7E9C' : '#'+data.color; 
...
// ---------- bind to events in the toolkit ---------------------
        ...
        if (this.bindToolkitEvents !== false) {
            ...
            _toolkit.bind(_e.nodeAdded, function (params) {
                ...
                nodeEl.style.backgroundColor = params.data.color == "" ? "#3E7E9C" : "#"+params.data.color
                if (nodeEl != null) {
            ...
            if (v) {
                ...
                if (existing) {
                    if (!jsPlumbUtil.isArray(existing)){
                    	_rotors.data(out, a, existing = existing );// TODO klaus.wang
                    	//_rotors.data(out, a, existing = [ existing ]);
                    }
                    if (jsPlumbUtil.isArray(existing))
                    	existing.push(v);
                }
