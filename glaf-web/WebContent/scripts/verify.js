...
function  formatNumber(Dight,How)  
{          
   if(How==null||How==''||How=='undefined'){
  		 How=2;
   }    
	var digitNumber = Math.round(Dight*Math.pow(10,How))/Math.pow(10,How);
    var numberString = digitNumber.toString()
    var dIndex = numberString.indexOf(".");
	if(dIndex == -1) {
	   numberString = numberString + ".";
       dIndex = numberString.indexOf(".");
	}
	while (dIndex!=-1 && numberString.length <dIndex+How+1) {
...
/* 
校验是否为double型的数据,返回的值为真和假,true和false;
*/
...
function verifyInput2(input){
   if(input != null){
	  if (input.getAttribute("required") == "true"){
		  if(input.getAttribute("dataType") == "DATE"){
              
		  } else  if(input.getAttribute("dataType") == "TIMESTAMP"){

		  } else {
			if( isNull(input.value) && input.getAttribute("displayName") !=null){
			  alert(input.displayName+"不能为空值");
			  return true;
		      }
		  }
	  }
  ...
  switch(input.datatype){
	...
	case "INTEGER": 
	  if (checkInteger(input.value)==false){
		alert(input.displayName+" 应该为整数类型");
		return true;
	  }
	  break;
	case "double": 
	case "DOUBLE": 
	  if (checkDouble(input.value)==false){
		alert(input.displayName+" 应该为数值类型");
		return true;
	  }
	  break;  
	default : 
...
*检测指定单选按钮输入是否合法
*/
function verifyRadio(input){
   if(input != null && input.getAttribute("type") == "radio" && input.getAttribute("required") == "true"){
 		var d = document.getElementsByName(input.getAttribute("name"));
		var ix = false;
		...
		if(!ix){
			if( input.getAttribute("displayName") !=null){
			  alert(input.displayName+"不能为空值");
			}
			return true;
		}
...
function verifyCheckBox(input){
   if(input != null && input.getAttribute("type") == "checkbox" && input.getAttribute("required") == "true"){
 		var d = document.getElementsByName(input.getAttribute("name"));
		var ix = false;
		for(var i = 0; i < d.length; i++){
		 if(d[i].checked){
