...
var TreeGridExtend = function() {
	...
	var lineTemp = ["<tr class='{0} {1}'>", //列模板
		"<td><input  type='text' class='k-textbox tg-name'/></td>",
		"<td><input  type='text' class='k-textbox tg-type'/></td>",
		"<td><input  type='text' class='k-textbox tg-button tg-content'/></td>",
		"<td><input  type='text' class='k-textbox tg-button tg-execFn'/></td>",
	...
	obj.isClick = true;
	obj.bindData = function($tr, nodeObj) { //绑定数据
		if (nodeObj) {
			$tr.find(".tg-name").val(nodeObj.name);

			$tr.find("span .tg-type").data("kendoDropDownList").value(nodeObj.input);

			$tr.find(".tg-execFn").val(nodeObj.execFn);
			$tr.find(".tg-execFn").data("idata", nodeObj.execFnData);

			$tr.find(".tg-content").val(nodeObj.content);
			$tr.find(".tg-content").data("idata", nodeObj.contentData);
		}
	};
	...
	obj.click = function(e, $tr) {
		...
		var $trigger = $tr.find(".tg-execFn"),
			triggerData = $trigger.data("idata");
		nodes = [];
		if (triggerData) {
			var idatas = triggerData;
			nodes = idatas.nodes;
		}
	...
	obj._initControl = function($tr, nodeDatas) {
		$tr.find(".tg-type").kendoDropDownList({
			dataSource: obj.permissionTypeDataSource,
			dataTextField: "name",
			dataValueField: "code",
			select:function(e){
				var $cont = $(this.wrapper).parents("tr").find(".tg-content");
				$cont.val("") && $cont.removeData('idata');
			}
		});
	...
	obj.init = function(datas) {
		obj.permissionTypeDataSource = getDictByCode("permission_type");
...
function retInputData(data) {
	var $select = mtxx.select;
	$select.val(data.names).data("idata", data);
...
var opWin = function() {
	var url = mtxx.contextPath,
		$this = $(this);
	mtxx.select = $this;
	if ($this.hasClass("tg-execFn")) { //执行验证匹配项（前提）
		url += "/mx/form/defined/ex/permissionDefinedSelect?getFn=getInputData&retFn=retInputData&pageId=" + pageId;
	} else if ($this.hasClass("tg-content")) { // 根据角色、用户、部门 弹出不同的页面
		...
		var $drop = $this.parents("tr").find(".tg-type[data-role=dropdownlist]").data("kendoDropDownList"),
			value = $drop.value(),
			name = $drop.text();
			url += "/mx/form/defined/param/"+value;
		$.layer({
			type: 2,
			maxmin: true,
			shadeClose: false,
...
function saveValidate() {
	...
	function nodeToObj($node) {
		...
		nodeObj.input = $node.find("span .tg-type").data("kendoDropDownList").value();

		nodeObj.content = $node.find(".tg-content").val();
		nodeObj.contentData = $node.find(".tg-content").data("idata");

		nodeObj.execFn = $node.find(".tg-execFn").val();
		nodeObj.execFnData = $node.find(".tg-execFn").data("idata");
