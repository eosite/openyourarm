	...
	bindMouseUp: function(e) {
		...
		if (len) {
			...
			$.each(selectNodes, function(i, selectNode) {
					...
					if(isGroup){
						if(val.drole == role){
							$.each(val.children, function(cindex, cval) {
								if (selectNodeId == (cval.eleId || cval.id)) {
									$.each(cval.children, function(i, v) {
										if (v.isEvn) {
											item = {
												encoded: false
											};
											item.text = "<div class=\"li-span\" onclick=\"selectNodeById('" + selectNodeId + "','" + v.eName + "')\">" + v.name + "</div>";
											items.push(item);
										}
									})
								}
				...
				items.push({
					encoded: false,
					text: "<div class=\"li-span\" onclick=\"removeNodeById('" + selectNodeId + "')\">移除</div>",
				});
				//return false;
				allItems.push({
					cssClass: "kmeau",
					text: $node.attr("cname"),
					items: items
				});
			});
			$menu.insertBefore(allItems, "li:first-child");
			/*for (var i = 0; i < len; i++) {
