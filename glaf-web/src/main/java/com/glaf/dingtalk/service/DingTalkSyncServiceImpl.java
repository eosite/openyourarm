...
@Service("com.glaf.dingtalk.service.dingTalkSyncService")
...
public class DingTalkSyncServiceImpl implements DingTalkSyncService{
	...
	public void sync(String baseId, String tmpId, String targetDatabase, String targetTableName,
		...
		try {
			...
			// 创建插入语句
			String insertSQL = createInsertSQL(targetTableName, columnMapping, metaData, returnDataColumns,extColumnStr);
	...
	public void sync(EimBaseInfo eimBaseInfo, EimServerTmp eimServerTmp, String targetDatabase, String targetTableName,
		...
		try {
			...
			List<String[]> returnDataColumns = new ArrayList<String[]>();
			// 创建插入语句
			String insertSQL = createInsertSQL(targetTableName, columnMapping, metaData, returnDataColumns,null);
			cmd = conn.prepareStatement(insertSQL);
			logger.info(
	...
	public void importData(EimBaseInfo eimBaseInfo, EimServerTmp eimServerTmp, JSONObject pagingContentJson,
		...
		if (currentPage > 0) {
			...
			paramsMap.put("offset", (currentPage-1)*100);
			paramsMap.put("size", 100);
		}
		JSONObject returnJSON = dingTalkService.callService(eimServerTmp.getTmpId(), eimBaseInfo.getId(), paramsMap);
		...
		if (statusCode == 200) {
			...
			if (returnDataJson != null) {
				...
				if (errcode == 0) {
					if (currentPage >0) {
						boolean hasMore=returnDataJson.getBoolean("hasMore");
						if(hasMore) {
							JSONObject paramsMapClone = new JSONObject();
							paramsMapClone.putAll(paramsMap);
							importData(eimBaseInfo, eimServerTmp, pagingContentJson, recursiveContentMap, currentPage+1,
						...
						for (int j = 0; j < data.size(); j++) {
							...
							for (String[] column : returnDataColumns) {
								...
								if (dType.equals("string")) {
									...
									//加密处理
									//if(encryFlag.equals("true")&&colVal!=null&&StringUtils.isNotEmpty((String) colVal)){
										//colVal=FlkWebCryptUtils.getInstance().encrypt((String) colVal, FlkWebCryptUtils.TYPE_CODER_BASE64);
									//}
	...
	@javax.annotation.Resource(name = "com.glaf.dingtalk.service.dingTalkService")
