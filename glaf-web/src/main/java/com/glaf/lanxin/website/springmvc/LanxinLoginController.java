...
@Controller("/lanxin/login")
@RequestMapping("/lanxin/login")
public class LanxinLoginController {
	...
	public ModelAndView login(HttpServletRequest request, HttpServletResponse response, ModelMap modelMap) {
		...
		//获取session中的accessTokenJson并检测code是否一致
		JSONObject accessTokenJson=null;
		if(request.getSession().getAttribute("accessTokenJson")!=null){
			accessTokenJson=(JSONObject)request.getSession().getAttribute("accessTokenJson");
			if(!code.equals(accessTokenJson.getString("code"))){
				accessTokenJson=null;
			}
		}
		JSONObject returnJson = lanxinService.getAccessTokenByBaseIdAndCode(baseId, code, accessTokenJson);
		if (returnJson == null) {
			return this.prepareLogin(request, response, modelMap);
		}
		...
		//从returnJson中获取身份信息openId
		String openId=returnJson.getString("openid");
		if(StringUtils.isEmpty(openId)){
			return this.prepareLogin(request, response, modelMap);
		}
		SysUser bean =sysUserService.findByAccount(openId);
		if(bean==null)
		bean = sysUserService.findByMobile(openId);
		if(bean==null){
			return this.prepareLogin(request, response, modelMap);
		}
		/**
		...
		if (bean != null) {
			...
			//访问令牌写入session
			request.getSession().setAttribute("accessTokenJson", returnJson);
			//同时写入缓存用于后台接口调用
