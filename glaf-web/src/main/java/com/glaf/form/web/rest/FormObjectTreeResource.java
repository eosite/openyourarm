...
import com.glaf.form.core.domain.FormObjectTree;
import com.glaf.form.core.query.FormObjectTreeQuery;
import com.glaf.form.core.service.FormObjectTreeService;
...
@Controller
@Path("/rs/form/formObjectTree")
public class FormObjectTreeResource {
	protected static final Log logger = LogFactory.getLog(FormObjectTreeResource.class);
	...
	public byte[] deleteAll(@Context HttpServletRequest request)
		...
		if (rowIds != null) {
			...
			if (ids != null && !ids.isEmpty()) {
				formObjectTreeService.deleteByIds(ids);
			}
		}
	...
	public byte[] deleteById(@Context HttpServletRequest request) throws IOException {
                formObjectTreeService.deleteById(request.getParameter("id"));
		return ResponseUtils.responseJsonResult(true);
	}
	...
	public byte[] data(@Context HttpServletRequest request, @RequestBody DataRequest dataRequest) throws IOException {
		...
		if (total > 0) {
			...
			List<FormObjectTree> list = formObjectTreeService.getFormObjectTreesByQueryCriteria(start, limit,
					query);
	...
	@POST
	@Path("/saveFormObjectTree")
	@ResponseBody
	@Produces({ MediaType.APPLICATION_JSON })
	public byte[] saveFormObjectTree(@Context HttpServletRequest request) {
	        Map<String, Object> params = RequestUtils.getParameterMap(request);
	...
	public byte[] view(@Context HttpServletRequest request) throws IOException {
		FormObjectTree formObjectTree = null;
		if (StringUtils.isNotEmpty(request.getParameter("id"))) {
                  formObjectTree = formObjectTreeService.getFormObjectTree(request.getParameter("id"));
		}
		JSONObject result = new JSONObject();
		if (formObjectTree != null) {
		    result =  formObjectTree.toJsonObject();
		    Map<String, User> userMap = IdentityFactory.getUserMap();
		    result.put("id", formObjectTree.getId());
		    result.put("formObjectTreeId", formObjectTree.getId());
		}
		return result.toJSONString().getBytes("UTF-8");
	}
}
