...
import com.glaf.form.core.util.FormTaskDomainFactory;
...
@Controller
@Path("/rs/form/formTask")
public class FormTaskResource {
	protected static final Log logger = LogFactory.getLog(FormTaskResource.class);
	...
	public byte[] deleteAll(@Context HttpServletRequest request)
		...
		if (rowIds != null) {
			...
			if (ids != null && !ids.isEmpty()) {
				formTaskService.deleteByIds(ids);
			}
		}
	...
	public byte[] deleteById(@Context HttpServletRequest request) throws IOException {
                formTaskService.deleteById(RequestUtils.getLong(request, "id"));
		return ResponseUtils.responseJsonResult(true);
	}
	...
	@ResponseBody
	@Produces({ MediaType.APPLICATION_JSON })
	public byte[] saveFormTask(@Context HttpServletRequest request) {
	        Map<String, Object> params = RequestUtils.getParameterMap(request);
	...
	public byte[] view(@Context HttpServletRequest request) throws IOException {
		FormTask formTask = null;
		if (StringUtils.isNotEmpty(request.getParameter("id"))) {
                  formTask = formTaskService.getFormTask(RequestUtils.getLong(request, "id"));
		}
		JSONObject result = new JSONObject();
		if (formTask != null) {
		    result =  formTask.toJsonObject();
		    Map<String, User> userMap = IdentityFactory.getUserMap();
		    result.put("id", formTask.getId());
		    result.put("formTaskId", formTask.getId());
		}
		return result.toJSONString().getBytes("UTF-8");
	}
}
