...
@Controller
@Path("/rs/form/formEvent")
public class FormEventResource {
	protected static final Log logger = LogFactory.getLog(FormEventResource.class);
	...
	public byte[] deleteAll(@Context HttpServletRequest request) throws IOException {
		...
		if (rowIds != null) {
			...
			if (ids != null && !ids.isEmpty()) {
				formEventService.deleteByIds(ids);
			}
		}
	...
	public byte[] deleteById(@Context HttpServletRequest request) throws IOException {
		formEventService.deleteById(request.getParameter("id"));
		return ResponseUtils.responseJsonResult(true);
	}
	...
	@ResponseBody
	@Produces({ MediaType.APPLICATION_JSON })
	public byte[] saveFormEvent(@Context HttpServletRequest request) {
		Map<String, Object> params = RequestUtils.getParameterMap(request);
	...
	public byte[] view(@Context HttpServletRequest request) throws IOException {
		...
		if (StringUtils.isNotEmpty(request.getParameter("id"))) {
			formEvent = formEventService.getFormEvent(request.getParameter("id"));
		}
		JSONObject result = new JSONObject();
		if (formEvent != null) {
			result = formEvent.toJsonObject();
			Map<String, User> userMap = IdentityFactory.getUserMap();
			result.put("id", formEvent.getId());
			result.put("formEventId", formEvent.getId());
		}
		return result.toJSONString().getBytes("UTF-8");
	}
}
