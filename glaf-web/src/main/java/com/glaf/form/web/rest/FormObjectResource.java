...
import com.glaf.form.core.domain.FormObject;
import com.glaf.form.core.query.FormObjectQuery;
import com.glaf.form.core.service.FormObjectService;
...
@Controller
@Path("/rs/form/formObject")
public class FormObjectResource {
	protected static final Log logger = LogFactory.getLog(FormObjectResource.class);
	...
	public byte[] deleteAll(@Context HttpServletRequest request)
		...
		if (rowIds != null) {
			...
			if (ids != null && !ids.isEmpty()) {
				formObjectService.deleteByIds(ids);
			}
		}
	...
	public byte[] deleteById(@Context HttpServletRequest request) throws IOException {
                formObjectService.deleteById(request.getParameter("id"));
		return ResponseUtils.responseJsonResult(true);
	}
	...
	public byte[] data(@Context HttpServletRequest request, @RequestBody DataRequest dataRequest) throws IOException {
		...
		if (total > 0) {
			...
			List<FormObject> list = formObjectService.getFormObjectsByQueryCriteria(start, limit,
					query);
	...
	@ResponseBody
	@Produces({ MediaType.APPLICATION_JSON })
	public byte[] saveFormObject(@Context HttpServletRequest request) {
	        Map<String, Object> params = RequestUtils.getParameterMap(request);
	...
	public byte[] view(@Context HttpServletRequest request) throws IOException {
		...
		if (StringUtils.isNotEmpty(request.getParameter("id"))) {
                  formObject = formObjectService.getFormObject(request.getParameter("id"));
		}
		JSONObject result = new JSONObject();
		if (formObject != null) {
		    result =  formObject.toJsonObject();
		    Map<String, User> userMap = IdentityFactory.getUserMap();
		    result.put("id", formObject.getId());
		    result.put("formObjectId", formObject.getId());
		}
		return result.toJSONString().getBytes("UTF-8");
	}
}
