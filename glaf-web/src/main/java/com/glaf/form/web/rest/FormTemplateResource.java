...
@Controller
@Path("/rs/form/template")
public class FormTemplateResource {
	protected static final Log logger = LogFactory
			.getLog(FormTemplateResource.class);
	...
	public byte[] deleteAll(@Context HttpServletRequest request)
		...
		if (rowIds != null) {
			...
			if (ids != null && !ids.isEmpty()) {
				formTemplateService.deleteByIds(ids);
			}
		}
	...
	public byte[] deleteById(@Context HttpServletRequest request)
			throws IOException {
		formTemplateService.deleteById(RequestUtils.getInt(request, "id"));
		return ResponseUtils.responseJsonResult(true);
	}
	...
	@ResponseBody
	@Produces({ MediaType.APPLICATION_JSON })
	public byte[] saveFormTemplate(@Context HttpServletRequest request) {
		Map<String, Object> params = RequestUtils.getParameterMap(request);
		...
		try {
			...
			formTemplate.setComponentId(RequestUtils.getInt(request,
					"componentId"));
			formTemplate.setCreateBy(request.getParameter("createBy"));
			formTemplate.setCreateDate(RequestUtils.getDate(request,
					"createDate"));
	...
	public byte[] view(@Context HttpServletRequest request) throws IOException {
		...
		if (StringUtils.isNotEmpty(request.getParameter("id"))) {
			formTemplate = formTemplateService.getFormTemplate(RequestUtils
					.getInt(request, "id"));
		}
		JSONObject result = new JSONObject();
		if (formTemplate != null) {
			result = formTemplate.toJsonObject();
			// Map<String, User> userMap = IdentityFactory.getUserMap();
			result.put("id", formTemplate.getId());
			result.put("formTemplateId", formTemplate.getId());
		}
		return result.toJSONString().getBytes("UTF-8");
	}
}
