 ...
 * 
 */
@Controller("/form/charts")
@RequestMapping("/form/charts")
public class FormChartsController {
	protected static final Log logger = LogFactory.getLog(FormChartsController.class);
	...
	public @ResponseBody byte[] getChartsDatas(HttpServletRequest request,
		...
		if (!StringUtils.isBlank(__ret)) {
			logger.error("--------------------------图表查询的返回数据---------------");
			logger.error(__ret);
			JSONObject retDataJSON = JSON.parseObject(__ret);
			JSONArray retDataAry = retDataJSON.getJSONArray("data")
					,rowsJSON = new JSONArray()
					,retArray = new JSONArray();
			JSONObject retData,result;
			for(Object obj : retDataAry){
				retData = (JSONObject)obj;
				
				Set<String> keys = retData.keySet();
				Iterator<String> iterator = keys.iterator();
				JSONObject josnObj = new JSONObject();
				while (iterator.hasNext()) {
					String key = (String) iterator.next();
					if (!key.toLowerCase().equals(key)) {
						josnObj.put(key, retData.get(key) != null ? retData.get(key).toString() : "");
					}
					josnObj.put(key.toLowerCase(), retData.get(key) != null ? retData.get(key).toString() : "");
				}
				rowsJSON.add(josnObj);
			}
			logger.error(rowsJSON.toJSONString());
