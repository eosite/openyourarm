...
public class ZTreeModel extends CommonModel implements IRule, CssRule, AttrRule {
	...
	public String getTemplateScript() {
		String script = "<link rel='stylesheet' type='text/css' href='${contextPath}/scripts/ztree/css/zTreeStyle/zTreeStyle.css'>"
	...
	public String getElementHtml() {
		...
		sb.append("<div id='" + this.getId() + "_div' style='width:100%;height:100%;overflow: hidden;" + display + "' >");
		...
		sb.append(
				"<ul ztreestyle='"+ele.attr("ztreestyle")+"' id='" + this.getId() + "' class='ztree' data-role='ztree' style='width:" + this.getWidth() + ";height:" + this.getHeight() + ";overflow: auto;' ></ul></div>");
		return sb.toString();
	}
	 ...
	 * @return
	 */
	public String getsynccheckc(){
		return source.getOrDefault("syncCheckC", "false");
	}
	
	//获取父子关系的联动
	public String getChildAndParent(){
	   String string=source.get("childAndParent");
		com.alibaba.fastjson.JSONArray ary = JSON.parseArray(string);
		JSONObject returnjson=new JSONObject();
		if(null!=ary){
			//选中两个对象
			JSONObject object = (JSONObject) ary.get(0);
			JSONObject object1 = (JSONObject) ary.get(1);
			
			if(!object.get("NY").equals("")){
				 String Ystring="";
				 if(!object.get("p").equals("")){
					 Ystring=Ystring+object.get("p");
				 }
				 if(!object.get("s").equals("")){
					 Ystring=Ystring+object.get("s");
				 }
				 returnjson.put("Y",Ystring);
			}
			if(!object1.get("NY").equals("")){
				 String Nstring="";
				 if(!object1.get("p").equals("")){
					 Nstring=Nstring+object1.get("p");
				 }
				 if(!object1.get("s").equals("")){
					 Nstring=Nstring+object1.get("s");
				 }
				 returnjson.put("N",Nstring);
			}
