...
@Controller
@Path("/rs/dep/report/depReportTemplate")
public class DepReportTemplateResource {
	protected static final Log logger = LogFactory.getLog(DepReportTemplateResource.class);
	...
	public byte[] deleteAll(@Context HttpServletRequest request)
		...
		if (rowIds != null) {
			...
			if (ids != null && !ids.isEmpty()) {
				depReportTemplateService.deleteByIds(ids);
			}
		}
	...
	public byte[] deleteById(@Context HttpServletRequest request) throws IOException {
                depReportTemplateService.deleteById(RequestUtils.getLong(request, "id"));
		return ResponseUtils.responseJsonResult(true);
	}
	...
	@POST
	@Path("/saveDepReportTemplate")
	@ResponseBody
	@Produces({ MediaType.APPLICATION_JSON })
	public byte[] saveDepReportTemplate(@Context HttpServletRequest request) {
	        Map<String, Object> params = RequestUtils.getParameterMap(request);
	...
	public byte[] view(@Context HttpServletRequest request) throws IOException {
		...
		if (StringUtils.isNotEmpty(request.getParameter("id"))) {
                  depReportTemplate = depReportTemplateService.getDepReportTemplate(RequestUtils.getLong(request, "id"));
		}
		JSONObject result = new JSONObject();
		if (depReportTemplate != null) {
		    result =  depReportTemplate.toJsonObject();
		    Map<String, User> userMap = IdentityFactory.getUserMap();
		    result.put("id", depReportTemplate.getId());
		    result.put("depReportTemplateId", depReportTemplate.getId());
		}
		return result.toJSONString().getBytes("UTF-8");
	}
}
