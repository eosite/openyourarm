...
import com.glaf.dep.report.query.DepReportTmpCategoryQuery;
...
@Controller
@Path("/rs/dep/report/depReportTmpCategory")
public class DepReportTmpCategoryResource {
	protected static final Log logger = LogFactory.getLog(DepReportTmpCategoryResource.class);
	...
	public byte[] deleteAll(@Context HttpServletRequest request)
			throws IOException {
		String rowIds = request.getParameter("depIds");
		if (rowIds != null) {
			List<Long> ids = StringTools.splitToLong(rowIds);
			if (ids != null && !ids.isEmpty()) {
				depReportTmpCategoryService.deleteByIds(ids);
			}
		}
	...
	public byte[] deleteById(@Context HttpServletRequest request) throws IOException {
                depReportTmpCategoryService.deleteById(RequestUtils.getLong(request, "depId"));
		return ResponseUtils.responseJsonResult(true);
	}
	...
	@POST
	@Path("/saveDepReportTmpCategory")
	@ResponseBody
	@Produces({ MediaType.APPLICATION_JSON })
	public byte[] saveDepReportTmpCategory(@Context HttpServletRequest request) {
	        Map<String, Object> params = RequestUtils.getParameterMap(request);
	...
	public byte[] view(@Context HttpServletRequest request) throws IOException {
		DepReportTmpCategory depReportTmpCategory = null;
		if (StringUtils.isNotEmpty(request.getParameter("depId"))) {
                  depReportTmpCategory = depReportTmpCategoryService.getDepReportTmpCategory(RequestUtils.getLong(request, "depId"));
		}
		JSONObject result = new JSONObject();
		if (depReportTmpCategory != null) {
		    result =  depReportTmpCategory.toJsonObject();
		    Map<String, User> userMap = IdentityFactory.getUserMap();
		    result.put("id", depReportTmpCategory.getId());
		    result.put("depReportTmpCategoryId", depReportTmpCategory.getId());
		}
		return result.toJSONString().getBytes("UTF-8");
	}
}
