...
import com.glaf.report.core.domain.ReportTmpCol;
import com.glaf.report.core.query.ReportTmpColQuery;
import com.glaf.report.core.service.ReportTmpColService;
...
@Controller
@Path("/rs/report/reportTmpCol")
public class ReportTmpColResource {
	protected static final Log logger = LogFactory.getLog(ReportTmpColResource.class);
	...
	public byte[] deleteAll(@Context HttpServletRequest request)
		...
		if (rowIds != null) {
			...
			if (ids != null && !ids.isEmpty()) {
				reportTmpColService.deleteByIds(ids);
			}
		}
	...
	public byte[] deleteById(@Context HttpServletRequest request) throws IOException {
                reportTmpColService.deleteById(request.getParameter("id"));
		return ResponseUtils.responseJsonResult(true);
	}
	...
	public byte[] data(@Context HttpServletRequest request, @RequestBody DataRequest dataRequest) throws IOException {
		...
		if (total > 0) {
			...
			List<ReportTmpCol> list = reportTmpColService.getReportTmpColsByQueryCriteria(start, limit,
					query);
	...
	@POST
	@Path("/saveReportTmpCol")
	@ResponseBody
	@Produces({ MediaType.APPLICATION_JSON })
	public byte[] saveReportTmpCol(@Context HttpServletRequest request) {
	        Map<String, Object> params = RequestUtils.getParameterMap(request);
	...
	public byte[] view(@Context HttpServletRequest request) throws IOException {
		ReportTmpCol reportTmpCol = null;
		if (StringUtils.isNotEmpty(request.getParameter("id"))) {
                  reportTmpCol = reportTmpColService.getReportTmpCol(request.getParameter("id"));
		}
		JSONObject result = new JSONObject();
		if (reportTmpCol != null) {
		    result =  reportTmpCol.toJsonObject();
		    Map<String, User> userMap = IdentityFactory.getUserMap();
		    result.put("id", reportTmpCol.getId());
		    result.put("reportTmpColId", reportTmpCol.getId());
		}
		return result.toJSONString().getBytes("UTF-8");
	}
}
