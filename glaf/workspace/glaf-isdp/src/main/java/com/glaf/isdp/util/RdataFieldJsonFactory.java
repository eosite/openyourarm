 ...
 *
 */
public class RdataFieldJsonFactory {

	public static RdataField jsonToObject(JSONObject jsonObject) {
		RdataField model = new RdataField();
		...
		if (jsonObject.containsKey("lgcexpress")) {
			model.setLgcexpress(jsonObject.getString("lgcexpress"));
		}

		return model;
	}

	public static JSONObject toJsonObject(RdataField model) {
		...
		if (model.getLgcexpress() != null) {
			jsonObject.put("lgcexpress", model.getLgcexpress());
		}
		return jsonObject;
	}

	public static ObjectNode toObjectNode(RdataField model) {
	...
	public static JSONArray listToArray(java.util.List<RdataField> list) {
		JSONArray array = new JSONArray();
		if (list != null && !list.isEmpty()) {
			for (RdataField model : list) {
	...
	public static java.util.List<RdataField> arrayToList(JSONArray array) {
		java.util.List<RdataField> list = new java.util.ArrayList<RdataField>();
		for (int i = 0, len = array.size(); i < len; i++) {
			JSONObject jsonObject = array.getJSONObject(i);
			RdataField model = jsonToObject(jsonObject);
			list.add(model);
		}
		return list;
	}

	private RdataFieldJsonFactory() {
