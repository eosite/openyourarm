...
public class TableTransformBean implements java.lang.Runnable {
	...
	public TableTransformBean(long databaseId, String tableName, Map<String, Object> parameter) {
	...
	public boolean execute(long databaseId, String tableName, Map<String, Object> params) {
		...
		try {
			...
			sqlExecutor2 = sqlHelper.buildSql("table_transform", tableName, params);
							...
							for (ColumnTransform cm : cols) {
								...
								if (StringUtils.isNotEmpty(condition)) {
									...
									try {
										...
										logger.error(condition + " : " + dataMap);
									}
								}
								if (boolValue) {
			...
			ExceptionUtils.addMsg("table_transform_" + tableName, ex.getMessage());
	...
	public void run() {
		result = this.execute(databaseId, tableName, parameter);
	}
