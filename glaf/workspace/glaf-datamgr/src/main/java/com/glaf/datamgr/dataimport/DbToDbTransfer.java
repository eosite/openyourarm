...
package com.glaf.datamgr.dataimport;
...
public class DbToDbTransfer {
	...
	public static void main(String[] args) {
		...
		List<String> error_tables = new java.util.ArrayList<String>();
		...
		try {
			...
			if ("oracle".equals(dbtype)) {
				rs = dbmd.getTables(null, dbmd.getUserName(), null, new String[] { "TABLE" });
			} else {
		...
		if (!tables.isEmpty()) {
			for (String tableName : tables) {
				try {
					List<String> pks = DBUtils.getPrimaryKeys(srcSystemName, tableName);
					if (pks != null && pks.size() == 1) {
						transfer.transferTable(srcSystemName, destSystemName, tableName, pks.get(0), true);
					} else {
						transfer.transferTable(srcSystemName, destSystemName, tableName, null, true);
					}
				} catch (Exception ex) {
	 ...
	 * @param rebuildTargartTable
	 */
	public void transferQueryToTable(Connection srcCon, Connection destCon, String sql, String tableName,
		...
		List<ColumnDefinition> columns1 = new ArrayList<ColumnDefinition>();
		...
		try {
			...
			for (int i = 1; i <= count; i++) {
				...
				column.setOrdinal(i);
				columns2.add(column);
				if (StringUtils.equalsIgnoreCase(primaryKeyColumn, column.getColumnName())) {
			...
			if (rebuildTargartTable) {
				tableDefinition.setColumns(columns1);
		...
		for (int i = 0, len = columns2.size(); i < len; i++) {
			sb.append(" ? ");
			...
			while (rs.next()) {
				...
				for (ColumnDefinition column : columns2) {
					...
					if ("Integer".equals(javaType)) {
						try {
							psmt2.setInt(column.getOrdinal(), rs.getInt(column.getOrdinal()));
						} catch (Exception ex) {
							logger.error(column.getColumnName() + " datatype error");
							psmt2.setString(column.getOrdinal(), rs.getString(column.getOrdinal()));
						}
					} else if ("Long".equals(javaType)) {
						try {
							psmt2.setLong(column.getOrdinal(), rs.getLong(column.getOrdinal()));
						} catch (Exception ex) {
							logger.error(column.getColumnName() + " datatype error");
							psmt2.setString(column.getOrdinal(), rs.getString(column.getOrdinal()));
						}
						...
						try {
							psmt2.setDouble(column.getOrdinal(), rs.getDouble(column.getOrdinal()));
						} catch (Exception ex) {
							logger.error(column.getColumnName() + " datatype error");
							psmt2.setString(column.getOrdinal(), rs.getString(column.getOrdinal()));
						}
						...
						if ("sqlite".equals(dbType)) {
							...
							if (t != null) {
								psmt2.setString(column.getOrdinal(), DateUtils.getDate(t));
							} else {
								psmt2.setString(column.getOrdinal(), null);
							}
						} else {
							psmt2.setTimestamp(column.getOrdinal(), rs.getTimestamp(column.getOrdinal()));
						}
					} else if ("Boolean".equals(javaType)) {
						try {
							psmt2.setBoolean(column.getOrdinal(), rs.getBoolean(column.getOrdinal()));
						} catch (Exception ex) {
							logger.error(column.getColumnName() + " datatype error");
							psmt2.setString(column.getOrdinal(), rs.getString(column.getOrdinal()));
						}
						...
						try {
							psmt2.setObject(column.getOrdinal(), rs.getObject(column.getOrdinal()));
						} catch (Exception ex) {
							ex.printStackTrace();
	...
	public void transferQueryToTable(String srcSystemName, String destSystemName, String sql, String tableName,
			String primaryKeyColumn, boolean rebuildTargartTable) {
		...
		try {
			...
			this.transferQueryToTable(srcCon, destCon, sql, tableName, primaryKeyColumn, rebuildTargartTable);
		} finally {
	...
	public void transferTable(Connection srcCon, Connection destCon, String tableName, String primaryKeyColumn,
			boolean rebuildTargartTable) {
		String sql = " select * from " + tableName;
		this.transferQueryToTable(srcCon, destCon, sql, tableName, primaryKeyColumn, rebuildTargartTable);
	}

	public void transferTable(String srcSystemName, String destSystemName, String tableName, String primaryKeyColumn,
			boolean rebuildTargartTable) {
		String sql = " select * from " + tableName;
		this.transferQueryToTable(srcSystemName, destSystemName, sql, tableName, primaryKeyColumn, rebuildTargartTable);
	}
