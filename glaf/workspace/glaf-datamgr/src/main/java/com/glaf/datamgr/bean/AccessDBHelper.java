...
public class AccessDBHelper {
	...
	protected static final Log logger = LogFactory.getLog(AccessDBHelper.class);

	public HikariDataSource getDataSource(String sqliteDB) {
		String dbpath = SystemProperties.getConfigRootPath() + "/db/" + sqliteDB;
		String jdbcUrl = "jdbc:access:" + dbpath;
		String jdbcDriverClass = "com.hxtt.sql.access.AccessDriver";
		...
		logger.info("access file path : " + dbpath);
		return ds;
	}
	...
	public HikariDataSource getTempDataSource(String sqliteDB) {
		...
		config.setMaxLifetime(1000L * 3600 * 8);
		HikariDataSource ds = new HikariDataSource(config);
		logger.info("access file temp path : " + dbpath);
		return ds;
	}
