...
import com.glaf.datamgr.query.ParameterLogQuery;
...
@Controller
@Path("/rs/sys/parameterLog")
public class ParameterLogResource {
	protected static final Log logger = LogFactory.getLog(ParameterLogResource.class);
	...
	public byte[] view(@Context HttpServletRequest request) throws IOException {
		ParameterLog parameterLog = null;
		if (StringUtils.isNotEmpty(request.getParameter("id"))) {
			parameterLog = parameterLogService.getParameterLog(RequestUtils.getLong(request, "id"));
		}
		JSONObject result = new JSONObject();
		if (parameterLog != null) {
			result = parameterLog.toJsonObject();

			result.put("id", parameterLog.getId());
			result.put("parameterLogId", parameterLog.getId());
		}
		return result.toJSONString().getBytes("UTF-8");
	}
}
