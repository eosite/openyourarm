...
@Controller("/rs/datamgr/entity")
@Path("/rs/datamgr/entity")
public class EntitySegmentResource {
	protected static final Log logger = LogFactory.getLog(EntitySegmentResource.class);
	...
	public byte[] deleteAll(@Context HttpServletRequest request) throws IOException {
		...
		if (rowIds != null) {
			...
			if (ids != null && !ids.isEmpty()) {
				entitySegmentService.deleteByIds(ids);
			}
		}
	...
	public byte[] deleteById(@Context HttpServletRequest request) throws IOException {
		entitySegmentService.deleteById(request.getParameter("id"));
		return ResponseUtils.responseJsonResult(true);
	}
	...
	public byte[] data(@Context HttpServletRequest request, @RequestBody DataRequest dataRequest) throws IOException {
		...
		EntitySegmentDomainFactory.processDataRequest(dataRequest);
	...
	@POST
	@Path("/saveEntitySegment")
	@ResponseBody
	@Produces({ MediaType.APPLICATION_JSON })
	public byte[] saveEntitySegment(@Context HttpServletRequest request) {
	...
	public byte[] view(@Context HttpServletRequest request) throws IOException {
		EntitySegment entitySegment = null;
		if (StringUtils.isNotEmpty(request.getParameter("id"))) {
			entitySegment = entitySegmentService.getEntitySegment(request.getParameter("id"));
		}
		JSONObject result = new JSONObject();
		if (entitySegment != null) {
			result = entitySegment.toJsonObject();
			result.put("id", entitySegment.getId());
			result.put("entitySegmentId", entitySegment.getId());
		}
		return result.toJSONString().getBytes("UTF-8");
	}
}
