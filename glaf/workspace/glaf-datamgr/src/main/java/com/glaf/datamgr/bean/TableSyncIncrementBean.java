...
/**
 * 表增量同步，只是插入数据，不做删除和更新
 *
 */
public class TableSyncIncrementBean implements java.lang.Runnable {
	...
	public TableSyncIncrementBean() {

	}

	public TableSyncIncrementBean(long sourceDatabaseId, long targetDatabaseId, long tableSyncId) {
	...
	public boolean execute(long sourceDatabaseId, long targetDatabaseId, long tableSyncId) {
		...
		try {
			...
			if (tableSync != null && StringUtils.equals(tableSync.getInsertOnly(), "Y")) {
				...
				for (String syncColumn : syncColumns) {
					...
					if (col != null) {
						 ...
						 */
						if (tableSync.getPrimaryKey() != null && StringUtils
								.equalsIgnoreCase(col.getColumnName().trim(), tableSync.getPrimaryKey().trim())) {
				...
				if (idColumn == null) {
					if (StringUtils.isNotEmpty(tableSync.getPrimaryKey())) {
						idColumn = columnMap.get(tableSync.getPrimaryKey().trim().toLowerCase());
	...
	public boolean execute(long sourceDatabaseId, long targetDatabaseId, TableSync tableSync) {
		...
		logger.debug("----------------------------------------------------------------");
		logger.debug("----------------------TableSyncIncrementBean execute------------");
		logger.debug("----------------------------------------------------------------");
		...
		int batchSize = conf.getInt("table_sync_batchSize", 10000);
			...
			if (skipResult) {
				...
				while (firstResult <= totalResult) {
					...
					if (source_list != null && !source_list.isEmpty()) {
						...
						sqlBuffer.append(" select ").append(selectBuffer.toString()).append(" from ")
								...
								.append(targetDatabaseId);

						if (idColumn != null) {
							if ((StringUtils.equalsIgnoreCase(idColumn.getJavaType(), "Integer")
									|| StringUtils.equalsIgnoreCase(idColumn.getJavaType(), "Long"))) {
								sqlBuffer.append(
										QueryUtils.getNumSQLCondition(sourceRowKeys, "E", idColumn.getColumnName()));
							} else {
								sqlBuffer.append(
										QueryUtils.getSQLCondition(sourceRowKeys, "E", idColumn.getColumnName()));
							}
						} else {
						...
						if (target_list != null && !target_list.isEmpty()) {
							...
							for (Map<String, Object> dataMap : target_list) {
								String shaid_ = ParamUtils.getString(dataMap, "shaid_");
								if (StringUtils.isNotEmpty(shaid_)) {
									keys.add(shaid_);
									targetRowKeys.add(shaid_);
								}
								buffer.delete(0, buffer.length());
								for (String syncColumn : syncColumns) {
									value = dataMap.get(syncColumn);
				...
				for (int i = 0; i < maxForEach; i++) {
					...
					if (source_list != null && !source_list.isEmpty()) {
						...
						if (maxForEach == 1) {
							sql = " select " + selectBuffer.toString() + " from " + tableSync.getTargetTableName()
									+ " where EX_DATABASEID_ = " + targetDatabaseId;
						}
						...
						for (Map<String, Object> dataMap : source_list) {
							...
							if (StringUtils.isNotEmpty(key)) {
								...
								if (StringUtils.isNotEmpty(targetMD5) && !StringUtils.equals(sourceMD5, targetMD5)) {// 记录存在MD5不相等
									...
									dataMap.put("ex_databaseid_", targetDatabaseId);
								} else {// 记录不存在
									dataMap.put("_jobno_", jobNo);
									dataMap.put("ex_jobno_", jobNo);
									dataMap.put("ex_databaseid_", targetDatabaseId);
