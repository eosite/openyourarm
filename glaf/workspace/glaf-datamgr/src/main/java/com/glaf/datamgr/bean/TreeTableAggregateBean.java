...
import com.glaf.datamgr.domain.ParameterLog;
...
import com.glaf.datamgr.util.ParameterLogFactory;
...
public class TreeTableAggregateBean implements java.lang.Runnable {
			...
			if (actorId != null) {
				...
				if (treeTableAggregate != null) {
					...
					if (database != null) {
						...
						if (DBUtils.ORACLE.equals(database.getType())) {
	...
	public boolean execute(Database database, TreeTableAggregate treeTableAggregate, String splitColumn, int splitValue,
		...
		try {
						...
						for (Map<String, Object> dataMap : source_list) {
							...
							if (StringUtils.isNotEmpty(key)) {
								if (StringUtils.isNumeric(key)) {
									if (key.length() < 8) {
										dataMap.put("_syntagmatic_", Double.parseDouble(database.getIntToken()
												+ StringTools.getDigit7Id(Integer.parseInt(key))));
										dataMap.put("ex_syntagmatic_", dataMap.get("_syntagmatic_"));
									} else {
										dataMap.put("_syntagmatic_", Double.parseDouble(database.getIntToken() + key));
										dataMap.put("ex_syntagmatic_", dataMap.get("_syntagmatic_"));
									}
								}
			...
			if (!insert_list.isEmpty()) {
				...
				bulkInsertBean.bulkInsert(targetConn, tableDefinition, list);
				insert_list.clear();
				list.clear();
			}
			...
			if (!update_list.isEmpty()) {
				...
				batchUpdateBean.executeBatch(targetConn, tableDefinition, list);
				update_list.clear();
				list.clear();
			}

			if (StringUtils.isNotEmpty(treeTableAggregate.getSourceTableExecutionIds())) {
				TableExecutionBean tableExecutionBean = new TableExecutionBean();
				List<String> executionIds = StringTools.split(treeTableAggregate.getSourceTableExecutionIds());
			...
			if (StringUtils.isNotEmpty(treeTableAggregate.getTargetTableExecutionIds())) {
				parameter.put("ex_jobno_", jobNo);
				parameter.put("_jobno_", jobNo);
				parameter.put("jobno_", jobNo);
				TableExecutionBean tableExecutionBean = new TableExecutionBean();
				List<String> executionIds = StringTools.split(treeTableAggregate.getTargetTableExecutionIds());
				List<TableExecution> executions = tableExecutionBean.getTableExecutions(executionIds);
				if (executions != null && !executions.isEmpty()) {
				...
				if (tableDefinition.getIdColumn() != null) {
						...
						if (years != null && !years.isEmpty()) {
							for (Integer year : years) {
								try {
									...
									// TreeTableAggregateThread command = new
									// TreeTableAggregateThread(database,
									// treeTableAggregate, "__year__", year);
									// com.glaf.core.util.ThreadFactory.execute(command);
									if (DBUtils.ORACLE.equals(database.getType())) {
										this.execute(database, treeTableAggregate, "ex_year_", year, parameter);
									} else {
										this.execute(database, treeTableAggregate, "__year__", year, parameter);
									}
									Thread.sleep(200);
								} catch (Exception ex) {
								}
						...
						if (yearMonths != null && !yearMonths.isEmpty()) {
							for (Integer yearMonth : yearMonths) {
								try {
									...
									// TreeTableAggregateThread command = new
									// TreeTableAggregateThread(database,
									// treeTableAggregate, "__yearmonth__",
									// yearMonth);
									// com.glaf.core.util.ThreadFactory.execute(command);
									if (DBUtils.ORACLE.equals(database.getType())) {
										this.execute(database, treeTableAggregate, "ex_yearmonth_", yearMonth,
												parameter);
									} else {
										this.execute(database, treeTableAggregate, "__yearmonth__", yearMonth,
												parameter);
									}
									Thread.sleep(200);
								} catch (Exception ex) {
					...
					if (ret) {
						...
						if (StringUtils.equalsIgnoreCase(treeTableAggregate.getGenByMonth(), "D") || (isLastDayOfMonth
							...
							try {
								columns = DBUtils.getColumnDefinitions(database.getName(),
										treeTableAggregate.getTargetTableName());
								...
								tableSynthetic.setSourceTableName(treeTableAggregate.getTargetTableName());
								...
								tableCopyBean.execute(database, database, tableSynthetic);

								treeTableAggregate.setTargetTableName(monthTargetTable);
								TreeTableAggregateMetaBean metaBean = new TreeTableAggregateMetaBean();
								metaBean.updateTreeTableAggregateMeta(treeTableAggregate, rules);
							} catch (Exception ex) {
	...
	protected void splitData(Database database, String tableName, String columnName, ColumnDefinition idColumn) {
		...
		String updateSql = " update " + tableName + " set __YEAR__ = ?, __YEARMONTH__ = ? where "
				+ idColumn.getColumnName() + " = ? ";
		if (DBUtils.ORACLE.equals(database.getType())) {
			updateSql = " update " + tableName + " set EX_YEAR_ = ?, EX_YEARMONTH_ = ? where "
					+ idColumn.getColumnName() + " = ? ";
		}
		...
		try {
			...
			yearCol.setColumnName("__YEAR__");
			if (DBUtils.ORACLE.equals(database.getType())) {
			...
			yearMonthCol.setColumnName("__YEARMONTH__");
			if (DBUtils.ORACLE.equals(database.getType())) {
	...
	protected void splitYearData(Database database, String tableName, int year) {
		String sql = " select TREEID_ from " + tableName + " where __YEAR__ = " + year;
		if (DBUtils.ORACLE.equals(database.getType())) {
			sql = " select TREEID_ from " + tableName + " where EX_YEAR_ = " + year;
		}
		long start = System.currentTimeMillis();
	...
	protected void splitYearMonthData(Database database, String tableName, int yearMonth) {
		String sql = " select TREEID_ from " + tableName + " where __YEARMONTH__ = " + yearMonth;
		if (DBUtils.ORACLE.equals(database.getType())) {
			sql = " select TREEID_ from " + tableName + " where EX_YEARMONTH_ = " + yearMonth;
		}
		List<Map<String, Object>> resultList = new ArrayList<Map<String, Object>>();
		Map<String, Object> dataMap = null;
		...
		try {
			...
			if (DBUtils.ORACLE.equals(database.getType())) {
				...
				buffer.append("  set __YEARMONTH__ = ");
			}
