...
package com.glaf.datamgr.dataexport;
...
public class XmlExporter {
	...
	public void exportXml(String systemName, String rootDir, int pageSize) {
		...
		try {
			...
			while (rs.next()) {
				...
				this.exportXml(systemName, rootDir, tableName, tableName.toLowerCase(), pageSize);
			}
		} catch (Exception ex) {
			ex.printStackTrace();
	...
	public void exportXml(String systemName, String rootDir, String sql, Map<String, Object> paramMap, String prefix,
			int perFileSize) {
			...
			for (int i = 1; i <= count; i++) {
				...
				if (!StringUtils.equalsIgnoreCase(column.getJavaType(), "Date")) {
					if (column.getPrecision() > 0) {
						e.addAttribute("Precision", String.valueOf(column.getPrecision()));
					}
					if (column.getScale() > 0) {
						e.addAttribute("Scale", String.valueOf(column.getScale()));
					}
				}
			}
			...
			while (rs.next()) {
				...
				while (iterator.hasNext()) {
					...
					if ("String".equals(javaType)) {
						...
						if (value != null) {
							row.addElement(columnLabel).addCDATA(String.valueOf(value));
						}
						...
						if (value != null) {
							row.addElement(columnLabel).addCDATA(DateUtils.getDateTime(value));
						}
					} else {
						Object value = rs.getObject(column.getOrdinal());
						if (value != null) {
							row.addElement(columnLabel).addCDATA(value.toString());
						}
					}
				}
				...
				if (index > 0 && index % perFileSize == 0) {
					...
					while (retry <= 2 && !success) {
						try {
							...
							String filename = rootDir + FileUtils.sp + prefix + "_" + pageNo + ".xml";
	...
	public void exportXml(String systemName, String rootDir, String tableName, String prefix, int perFileSize) {
		String sql = " select * from " + tableName;
		this.exportXml(systemName, rootDir, sql, new java.util.HashMap<String, Object>(), prefix, perFileSize);
	}
