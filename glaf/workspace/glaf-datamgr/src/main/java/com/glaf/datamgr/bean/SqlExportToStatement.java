...
public class SqlExportToStatement {
				...
				if (sqlDefIds != null && !sqlDefIds.isEmpty()) {
					...
					for (Long sqlDefId : sqlDefIds) {
						...
						while (retry < 3 && !success) {
							try {
								retry++;
								buffer.append(this.doExport(sqlExport, sqlDefId, databaseId, params));
								buffer.append(FileUtils.newline);
								buffer.append(FileUtils.newline);
								buffer.append(FileUtils.newline);
								buffer.append(FileUtils.newline);
	...
	public String doExport(SqlExport sqlExport, long sqlDefId, long databaseId, Map<String, Object> params) {
		StringBuilder buffer = new StringBuilder();
