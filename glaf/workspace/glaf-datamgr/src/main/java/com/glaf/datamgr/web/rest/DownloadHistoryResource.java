...
@Controller
@Path("/rs/download/history")
public class DownloadHistoryResource {
	protected static final Log logger = LogFactory.getLog(DownloadHistoryResource.class);
	...
	public byte[] view(@Context HttpServletRequest request) throws IOException {
		...
		if (StringUtils.isNotEmpty(request.getParameter("id"))) {
			downloadHistory = downloadHistoryService.getDownloadHistory(request.getParameter("id"));
		}
		JSONObject result = new JSONObject();
		if (downloadHistory != null) {
			result = downloadHistory.toJsonObject();
			result.put("id", downloadHistory.getId());
		}
		return result.toJSONString().getBytes("UTF-8");
	}
}
