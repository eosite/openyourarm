...
@Controller
@Path("/rs/datamgr/fileHistory")
public class FileHistoryResource {
	protected static final Log logger = LogFactory
			.getLog(FileHistoryResource.class);
	...
	public byte[] data(@Context HttpServletRequest request,
		...
		int total = fileHistoryService
				.getFileHistoryCountByQueryCriteria(query);
		if (total > 0) {
			...
			List<FileHistory> list = fileHistoryService
					.getFileHistoriesByQueryCriteria(start, limit, query);
	...
	public byte[] view(@Context HttpServletRequest request) throws IOException {
		...
		if (StringUtils.isNotEmpty(request.getParameter("fileId"))) {
			fileHistory = fileHistoryService.getFileHistory(request
					.getParameter("fileId"));
		}
		JSONObject result = new JSONObject();
		if (fileHistory != null) {
			result = fileHistory.toJsonObject();
			result.put("id", fileHistory.getFileId());
			result.put("fileHistoryId", fileHistory.getFileId());
		}
		return result.toJSONString().getBytes("UTF-8");
	}
}
