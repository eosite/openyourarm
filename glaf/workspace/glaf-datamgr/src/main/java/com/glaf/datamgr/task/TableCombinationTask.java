...
import com.glaf.datamgr.bean.TableCombinationBean;
...
public class TableCombinationTask extends RecursiveTask<Boolean> {
	public final static Log logger = LogFactory.getLog(TableCombinationTask.class);
	...
	public TableCombinationTask(long sourceDatabaseId, long targetDatabaseId, long tableCombinationId, String jobNo) {
	...
	public boolean execute() {
		...
		TableCombinationBean bean = new TableCombinationBean();
		boolean result = false;
		try {
			ExecutionHandlerFactory.doBefore("table_combination_" + tableCombinationId, context);
			result = bean.execute(sourceDatabaseId, targetDatabaseId, tableCombinationId);
		} catch (Exception ex) {
			result = false;
			...
			if (result) {
				...
				ExecutionHandlerFactory.doAfter("table_combination_" + tableCombinationId, context);
			}
		}
		JSONObject json = new JSONObject();
		json.put("sourceDatabaseId", sourceDatabaseId);
		json.put("targetDatabaseId", targetDatabaseId);
		json.put("tableCombinationId", tableCombinationId);
		...
		sysLog.setServiceKey("table_combination");
		sysLog.setModuleId("table_combination_" + tableCombinationId);
		sysLog.setBusinessKey("table_combination_" + tableCombinationId);
		SysLogFactory.getInstance().addLog(sysLog);

		Calendar calendar = Calendar.getInstance();
		calendar.setTime(new Date());

		log.setBusinessKey(String.valueOf(tableCombinationId));
