...
public class DataExportUpdateBean {
	protected final Logger logger = LoggerFactory.getLogger(getClass());

	protected static Configuration conf = BaseConfiguration.create();

	public void resetExportFlag(long databaseId, TableModel tableModel) {
		...
		try {
			...
			if (DBUtils.tableExists(systemName, tableModel.getTableName())) {
				...
				psmt = conn.prepareStatement(" update " + tableModel.getTableName() + " set _MD5_ = null ");
				psmt.executeUpdate();
				conn.commit();
				JdbcUtils.close(psmt);
			}
		} catch (Exception ex) {
			logger.error("reset export flag error", ex);
	...
	public void updateExportFlag(long databaseId, TableModel tableModel) {
		...
		try {
			...
			if (DBUtils.tableExists(systemName, tableModel.getTableName())) {
				...
				String sql = null;
				Map<Long, String> updateMap = new HashMap<Long, String>();
				...
				for (int i = 0; i < maxForEach; i++) {
					updateMap.clear();
					if (StringUtils.equals(idColumn.getJavaType(), "String")) {
						sql = " select " + colBuffer.toString() + " , _MD5_ from " + tableModel.getTableName();
					} else {
						sql = " select " + colBuffer.toString() + " , _MD5_ from " + tableModel.getTableName()
								+ " where " + idColumn.getColumnName() + " >= " + i * batchSize + " and "
								+ idColumn.getColumnName() + " <= " + ((i + 1) * batchSize);
					}
					logger.debug("sql:\n" + sql);
					psmt = conn.prepareStatement(sql);
					rs = psmt.executeQuery();
					...
					if (updateMap.size() > 0) {
						...
						Set<Entry<Long, String>> entrySet = updateMap.entrySet();
