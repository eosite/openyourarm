...
@Controller("/rs/system/sqlCondition")
@Path("/rs/system/sqlCondition")
public class SqlConditionResource {
	protected static final Log logger = LogFactory
			.getLog(SqlConditionResource.class);
	...
	public byte[] deleteAll(@Context HttpServletRequest request)
		...
		if (rowIds != null) {
			...
			if (ids != null && !ids.isEmpty()) {
				sqlConditionService.deleteByIds(ids);
			}
		}
	...
	public byte[] deleteById(@Context HttpServletRequest request)
			throws IOException {
		sqlConditionService.deleteById(RequestUtils.getLong(request, "id"));
		return ResponseUtils.responseJsonResult(true);
	}
	...
	public byte[] data(@Context HttpServletRequest request,
		...
		SqlConditionDomainFactory.processDataRequest(dataRequest);
		...
		int total = sqlConditionService
				.getSqlConditionCountByQueryCriteria(query);
		if (total > 0) {
			...
			List<SqlCondition> list = sqlConditionService
					.getSqlConditionsByQueryCriteria(start, limit, query);
	...
	@POST
	@Path("/saveSqlCondition")
	@ResponseBody
	@Produces({ MediaType.APPLICATION_JSON })
	public byte[] saveSqlCondition(@Context HttpServletRequest request) {
		Map<String, Object> params = RequestUtils.getParameterMap(request);
		...
		try {
			...
			sqlCondition.setDeleteFlag(RequestUtils.getInt(request,
					"deleteFlag"));
			sqlCondition.setCreateBy(request.getParameter("createBy"));
			sqlCondition.setCreateTime(RequestUtils.getDate(request,
					"createTime"));
			sqlCondition.setUpdateBy(request.getParameter("updateBy"));
			sqlCondition.setUpdateTime(RequestUtils.getDate(request,
					"updateTime"));
	...
	public byte[] view(@Context HttpServletRequest request) throws IOException {
		...
		if (StringUtils.isNotEmpty(request.getParameter("id"))) {
			sqlCondition = sqlConditionService.getSqlCondition(RequestUtils
					.getLong(request, "id"));
		}
		JSONObject result = new JSONObject();
		if (sqlCondition != null) {
			result = sqlCondition.toJsonObject();

			result.put("id", sqlCondition.getId());
			result.put("sqlConditionId", sqlCondition.getId());
		}
		return result.toJSONString().getBytes("UTF-8");
	}
}
