...
public class SqlExportDataBean {
	protected static final Log logger = LogFactory.getLog(SqlExportDataBean.class);
			...
			if (config.checkConfig(database)) {
				...
				if (total > 0) {
					...
					for (int i = 0; i < total / limit + 1; i++) {
						...
						SqlExportDataTask task = new SqlExportDataTask(database, sqlDef, start, limit, params);
						try {
							...
							for (Entry<String, Object> entry : entrySet) {
								...
								if (value != null) {
									if (value instanceof java.util.Date) {
										...
										buffer.append(datetime).append('\001');
									} else {
										String val = value.toString();
										buffer.append(val).append('\001');
									}
								} else {
									buffer.append("null").append('\001');
								}
							}
							buffer.append(database.getId()).append('\001');
							if (StringUtils.isNotEmpty(database.getMapping())) {
								buffer.append(database.getMapping()).append('\001');
							}
