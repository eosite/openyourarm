...
public class SqlExportToSqlite {
				...
				if (sqlDefIds != null && !sqlDefIds.isEmpty()) {
					...
					for (Long sqlDefId : sqlDefIds) {
						...
						while (retry < 3 && !success) {
							try {
								retry++;
								this.doExport(sqlDefId, databaseId, params, sqliteDB);
								successCount++;
								success = true;
								break;
							} catch (Exception ex) {
	...
	public void doExport(long sqlDefId, long databaseId, Map<String, Object> params, String sqliteDB) {
		...
		if (sqlDef != null) {
			...
			try {
				...
				if (!DBUtils.tableExists(conn2, tableName)) {
					...
					if (StringUtils.equalsIgnoreCase(sqlDef.getDeleteFetch(), "Y")) {
