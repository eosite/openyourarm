...
public class MxPanelServiceImpl implements PanelService {
	...
	public void saveUserPanel(UserPanel userPanel) {
		UserPanel model = this.getUserPanelByDashboardId(userPanel.getActorId(), userPanel.getDashboardId());
		if (model != null) {
		...
		userPortalMapper.deleteUserPortalByActorIdAndDashboardId(userPanel.getActorId(),userPanel.getDashboardId());
		int col = Integer.parseInt(userPanel.getLayoutName().substring(1, userPanel.getLayoutName().length()));
		for (PanelInstance panelInstance : panelInstances) {
			//等级P2,P3
			UserPortal userPortal = new UserPortal();
			userPortal.setId(idGenerator.getNextId());
			userPortal.setActorId(userPanel.getActorId());
			userPortal.setCreateDate(new Date());
			userPortal.setPanelId(panelInstance.getPanelId());
			int number = Integer.parseInt(panelInstance.getName()) ;
			int position = (int)((number+1)/col) ;
			if(number % col  == 0) {
				userPortal.setColumnIndex((col-1));//列
				userPortal.setPosition(position);//行
			}else {
				userPortal.setColumnIndex((number % col) - 1);//列
				userPortal.setPosition(position);//行
			}
			userPortal.setDashboardId(userPanel.getDashboardId());
	...
	@Override
	public UserPanel getUserPanelByDashboardId(String actorId, String dashboardId) {
		UserPanel userPanel = panelMapper.getUserPanelByDashboardId(actorId,dashboardId);
		if (userPanel != null) {
			if (userPanel.getLayoutId() != null) {
				Layout layout = layoutService
						.getLayout(userPanel.getLayoutId());
	...
	@Override
	public List<Panel> getPanelsByRole(List<String> roleIds) {
		return panelMapper.getPanelsByRole(roleIds);
	}
}