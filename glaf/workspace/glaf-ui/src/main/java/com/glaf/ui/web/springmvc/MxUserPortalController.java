...
public class MxUserPortalController {
	...
	public ModelAndView myPortal(ModelMap modelMap, HttpServletRequest request) {
		...
		// 获取分类
		String dashboardId = RequestUtils.getString(request, "dashboardId",
				"default");
		request.setAttribute("dashboardId", dashboardId);
		UserPanel userPanel = panelService.getUserPanelByDashboardId(
				loginContext.getActorId(), dashboardId);
		if (userPanel == null) {
		...
		List<UserPortal> userPortals = userPortalService
				.getUserPortalsByDashboardId(loginContext.getActorId(),
						dashboardId);
			...
			if (userPortals == null || userPortals.isEmpty()) {
				...
				if (panels != null && !panels.isEmpty()) {
					...
					userPortalService.save(loginContext.getActorId(),
							userPortals);
					userPortals = userPortalService
							.getUserPortalsByDashboardId(
									loginContext.getActorId(), dashboardId);
				}
			}
		}
		...
		if ("default".equals(dashboardId)) { // 只有首页才默认加载 系统公开portal
		...
		for (UserPortal p : userPortals) {
			...
			panel.setPanelButtons(panelButtonService
					.getPanelButtonByParentId(panel.getId()));
			p.setPanel(panel);
		}
		...
		String x_view = request.getParameter("xview");
		if (StringUtils.isNotEmpty(x_view)) {
			String view = "/modules/ui/portal/" + x_view;
	...
	public void savePortal(HttpServletRequest request) {
		...
		if (!rows.isEmpty()) {
			userPortalService.saveByDashboardId(actorId, rows, dashboardId);
		}
	}
