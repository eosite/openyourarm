...
public class StandardDialectResolver extends AbstractDialectResolver {
	...
	protected Dialect resolveDialectInternal(DatabaseMetaData metaData) throws SQLException {
		...
		if (databaseName.startsWith("Microsoft SQL Server")) {
			...
			default:
				log.warn("Unknown Microsoft SQL Server major version [" + databaseMajorVersion
						+ "] using SQL Server 2000 dialect");
			}
			return new SQLServerDialect();
		}
		...
		if ("DM DBMS".equalsIgnoreCase(databaseName)) {
			...
			try {
				dialectClass = org.hibernate.util.ReflectHelper.classForName("org.hibernate.dialect.DmDialect");
				org.hibernate.dialect.Dialect dialect = (org.hibernate.dialect.Dialect) dialectClass.newInstance();
				return dialect;
			} catch (Exception e) {
