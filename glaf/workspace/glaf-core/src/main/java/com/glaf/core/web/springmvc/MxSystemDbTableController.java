...
public class MxSystemDbTableController {
	protected static final Log logger = LogFactory.getLog(MxSystemDbTableController.class);
	...
	public void genMappings(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		if (StringUtils.isNotEmpty(dbType) && StringUtils.isNotEmpty(tables)) {
			...
			for (String table : list) {
				List<ColumnDefinition> columns = TableFactory.getColumnDefinitions(table);
				TableDefinition tableDefinition = new TableDefinition();
				tableDefinition.setPackageName("com.glaf.apps");
				tableDefinition.setModuleName("apps");
				tableDefinition.setTableName(table);
				tableDefinition.setClassName(StringTools.upper(StringTools.camelStyle(table)));
				tableDefinition.setTitle(tableDefinition.getClassName());
				tableDefinition.setEnglishTitle(tableDefinition.getClassName());
				tableDefinition.setEntityName(StringTools.upper(StringTools.camelStyle(table)));
				tableDefinition.setColumns(columns);
				for (ColumnDefinition column : columns) {
					column.setEditable(true);
					column.setUpdatable(true);
					column.setDisplayType(4);
					column.setName(StringTools.lower(StringTools.camelStyle(column.getColumnName())));
			...
			if (!bytesMap.isEmpty()) {
				...
				try {
					ResponseUtils.download(request, response, bytes, "mappings.zip");
				} catch (Exception ex) {
					ex.printStackTrace();
				}
			}
	...
	public byte[] json(HttpServletRequest request) throws IOException {
		...
		try {
			...
			while (rs.next()) {
				...
				if (tableName.toLowerCase().startsWith("userinfo")) {
					continue;
				}
