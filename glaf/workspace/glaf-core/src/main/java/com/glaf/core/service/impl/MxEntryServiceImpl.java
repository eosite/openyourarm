...
public class MxEntryServiceImpl implements IEntryService {
	protected final static Log logger = LogFactory.getLog(MxEntryServiceImpl.class);
	...
	public EntityEntry getEntityEntry(String moduleId, String entityId, String entryKey) {
		EntityEntry entityEntry = null;
	 ...
	 * @return
	 */
	public List<String> getEntityIds(LoginContext loginContext, String moduleId, String entryKey) {
		String cacheKey = "mx_" + loginContext.getActorId() + "_" + moduleId + "_" + entryKey;
		if (SystemConfig.getBoolean("use_query_cache") && CacheFactory.getString("entry", cacheKey) != null) {
			String text = CacheFactory.getString("entry", cacheKey);
		...
		if (SystemConfig.getBoolean("use_query_cache")) {
			...
			CacheFactory.put("entry", cacheKey, text);
		}
	...
	public List<TreeModel> getTreeModels(LoginContext loginContext) {
		...
		if (nodeIds.size() > 0) {
			...
			while (iter.hasNext()) {
				...
				if (entityEntry.isPropagationAllowed()) {
					List<TreeModel> children = treeModelService.getChildrenTreeModels(entityEntry.getNodeId());
		...
		if (treeModels != null && !treeModels.isEmpty()) {
			if (SystemConfig.getBoolean("use_query_cache")) {
				...
				CacheFactory.put("entry", cacheKey, array.toJSONString());
			}
		}
	 ...
	 * @return
	 */
	public boolean hasPermission(LoginContext loginContext, long nodeId, String permKey) {
		...
		if (treeNode != null) {
			if (StringUtils.equals(treeNode.getCreateBy(), loginContext.getActorId())) {
				return true;
			}
		}

		String cacheKey = "CX_" + permKey + "_" + nodeId + "_" + loginContext.getActorId();
		if (SystemConfig.getBoolean("use_query_cache") && CacheFactory.getString("entry", cacheKey) != null) {
		...
		if (!hasPermission) {
			 ...
			 */
			List<TreeModel> treeModels = treeModelService.getAncestorTreeModels(nodeId);
		...
		if (SystemConfig.getBoolean("use_query_cache")) {
			CacheFactory.put("entry", cacheKey, String.valueOf(hasPermission));
		}
	 ...
	 * @return
	 */
	public boolean hasPermission(LoginContext loginContext, String moduleId, String entityId, String permKey) {
		boolean hasPermission = false;
		...
		String cacheKey = "PX_" + moduleId + "_" + permKey + "_" + entityId + "_" + loginContext.getActorId();
