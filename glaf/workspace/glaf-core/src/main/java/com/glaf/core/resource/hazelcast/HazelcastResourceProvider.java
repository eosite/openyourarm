...
public class HazelcastResourceProvider implements ResourceProvider {
	...
	protected final ConcurrentMap<String, Resource> cacheMap = new ConcurrentHashMap<String, Resource>();
	...
	public Resource buildResource(String regionName, boolean autoCreate) {
		Resource resource = this.getResource(regionName);
		return resource;
	}

	public Resource getResource(String regionName) {
		Resource resource = cacheMap.get(regionName);
		if (resource == null) {
			final IMap<Object, Object> map = hazelcastInstance.getMap(regionName);
			resource = new HazelcastResource(map);
			final Resource currentResource = cacheMap.putIfAbsent(regionName, resource);
	...
	public void start(Properties props) {
		...
		for (DistributedObject distributedObject : distributedObjects) {
			if (distributedObject instanceof IMap) {
				...
				cacheMap.put(map.getName(), new HazelcastResource(map));
			}
		}
	}
