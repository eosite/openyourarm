...
import com.glaf.core.access.domain.AccessUri;
import com.glaf.core.access.query.AccessUriQuery;
...
@Controller
@Path("/rs/sys/accessUri")
public class AccessUriResource {
	protected static final Log logger = LogFactory.getLog(AccessUriResource.class);
	...
	public byte[] data(@Context HttpServletRequest request, @RequestBody DataRequest dataRequest) throws IOException {
		...
		AccessUriDomainFactory.processDataRequest(dataRequest);
	...
	@ResponseBody
	@Produces({ MediaType.APPLICATION_JSON })
	public byte[] saveAccessUri(@Context HttpServletRequest request) {
		Map<String, Object> params = RequestUtils.getParameterMap(request);
		AccessUri accessUri = new AccessUri();
		try {
			Tools.populate(accessUri, params);

			accessUri.setUri(request.getParameter("uri"));
			accessUri.setLimit(RequestUtils.getInt(request, "limit"));
			accessUri.setTotal(RequestUtils.getInt(request, "total"));
	...
	public byte[] view(@Context HttpServletRequest request) throws IOException {
		...
		if (StringUtils.isNotEmpty(request.getParameter("id"))) {
			accessUri = accessUriService.getAccessUri(RequestUtils.getLong(request, "id"));
		}
		JSONObject result = new JSONObject();
		if (accessUri != null) {
			result = accessUri.toJsonObject();
			result.put("id", accessUri.getId());
			result.put("accessUriId", accessUri.getId());
		}
		return result.toJSONString().getBytes("UTF-8");
	}
}
