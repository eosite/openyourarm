...
public class XmlMappingReader {
	...
	public TableModel read(java.io.InputStream inputStream) {
		...
		try {
			...
			if (element != null) {
				...
				tableModel.setExportType(element.attributeValue("exportType"));
				...
				if (StringUtils.equals(element.attributeValue("updateAllowed"), "false")) {
					tableModel.setUpdateAllowed(false);
				} else {
					tableModel.setUpdateAllowed(true);
				}
				...
				while (iter.hasNext()) {
					...
					tableModel.addAttribute(attr.getName(), attr.getStringValue());
				}
				...
				List<?> statements = element.elements("statements");
				if (statements != null && statements.size() > 0) {
					Iterator<?> iterator = statements.iterator();
					while (iterator.hasNext()) {
						Element elem = (Element) iterator.next();
						tableModel.addStatement(elem.getStringValue());
					}
				}
	...
	protected void readField(Element elem, ColumnModel field) {
		...
		field.setJavaType(field.getType());
		...
		field.setExportFlag(elem.attributeValue("exportFlag"));

		List<?> attributes = elem.attributes();
		Iterator<?> iter = attributes.iterator();
		while (iter.hasNext()) {
			Attribute attr = (Attribute) iter.next();
			field.addAttribute(attr.getName(), attr.getStringValue());
		}
