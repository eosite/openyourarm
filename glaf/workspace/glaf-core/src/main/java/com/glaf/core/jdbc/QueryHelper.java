...
public class QueryHelper {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public ResultModel getResultList(Connection conn, String sql, Map<String, Object> paramMap, int start, int limit) {
		if (!DBUtils.isLegalQuerySql(sql)) {
			throw new RuntimeException(" SQL statement illegal ");
		}
		if (!DBUtils.isAllowedSql(sql)) {
				...
				while (iterator.hasNext()) {
					...
					if ("String".equals(javaType)) {
						...
						try {
							...
							c.setValue(rs.getDate(column.getIndex()));
						}
					} else {
	 ...
	 * @return
	 */
	public ResultModel getResultModel(Connection conn, String tableName, Map<String, Object> paramMap, int start,
			int pageSize) {
		...
		if (total > 0) {
			...
			resultModel = this.getResultList(conn, sql, paramMap, start, pageSize);
