...
public class MxBlobServiceImpl implements IBlobService {
	...
	protected final static Log logger = LogFactory.getLog(MxBlobServiceImpl.class);
	...
	public BlobItem getBlobByFileId(String fileId) {
		...
		if (SystemConfig.getBoolean("use_query_cache")) {
			...
			if (StringUtils.isNotEmpty(text)) {
				try {
					...
					BlobItemJsonFactory.jsonToObject(jsonObject);
				} catch (Exception ex) {
				}
			}
		}
		...
		if (list != null && !list.isEmpty()) {
			...
			if (blobItem != null) {
				if (SystemConfig.getBoolean("use_query_cache")) {
					CacheFactory.put("blob", cacheKey, blobItem.toJsonObject().toJSONString());
				}
			}
		}
		return blobItem;
	...
	public byte[] getBytesByFileId(String fileId) {
		...
		if (StringUtils.equals(DBUtils.POSTGRESQL, DBConnectionFactory.getDatabaseType())) {
			blobItem = (BlobItem) entityDAO.getSingleObject("getBlobItemFileInfoByFileId_postgres", fileId);
		} else {
			blobItem = (BlobItem) entityDAO.getSingleObject("getBlobItemFileInfoByFileId", fileId);
		}
		if (blobItem != null && blobItem.getData() != null) {
			if (SystemConfig.getBoolean("use_file_cache")
					&& blobItem.getData().length < conf.getInt("cache_file_size", 500) * FileUtils.KB_SIZE) {
				ResourceFactory.put("mx_blob", cacheKey, blobItem.getData());
	...
	public byte[] getBytesById(String id) {
		...
		if (StringUtils.equals(DBUtils.POSTGRESQL, DBConnectionFactory.getDatabaseType())) {
			blobItem = (BlobItem) entityDAO.getSingleObject("getBlobItemFileInfoById_postgres", id);
		} else {
			blobItem = (BlobItem) entityDAO.getSingleObject("getBlobItemFileInfoById", id);
		}
		if (blobItem != null && blobItem.getData() != null) {
			if (SystemConfig.getBoolean("use_file_cache")
					&& blobItem.getData().length < conf.getInt("cache_file_size", 500) * FileUtils.KB_SIZE) {
	...
	private void insertBlobItem(BlobItem blobItem) {
		...
		String deviceId = SystemConfig.getString("fs_storage_strategy", "DATABASE");
