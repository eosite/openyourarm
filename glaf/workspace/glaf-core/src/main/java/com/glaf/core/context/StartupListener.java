...
public class StartupListener extends ContextLoaderListener implements ServletContextListener {
	private static class SchedulerRunner extends Thread {
		public void run() {
			...
			/**
			 * 在集群环境里可以设置-Dstart_biz_scheduler=true启动业务调度
			 */
			if ("true".equals(System.getProperty("start_biz_scheduler"))) {
				try {
					Object object = ReflectUtils.instantiate("com.glaf.scheduler.startup.SchedulerStartup");
					if (object != null) {
						ReflectUtils.invoke(object, "startup");
					}
				} catch (Exception ex) {
					logger.error(ex);
				}
	...
	public void contextInitialized(ServletContextEvent event) {
		...
		if (DBConnectionFactory.checkConnection()) {
			...
			if (SystemProperties.getDeploymentSystemName() != null) {
				...
				String path = SystemProperties.getConfigRootPath() + Constants.DEPLOYMENT_JDBC_PATH
