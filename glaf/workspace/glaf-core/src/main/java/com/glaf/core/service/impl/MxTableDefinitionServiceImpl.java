...
public class MxTableDefinitionServiceImpl implements ITableDefinitionService {
	protected final static Log logger = LogFactory.getLog(MxTableDefinitionServiceImpl.class);
	...
	public List<ColumnDefinition> getColumnDefinitionsByTableName(String tableName) {
	 ...
	 */
	@Transactional
	public String getNextTableName(String type) {
	...
	public List<TableDefinition> getTableColumnsCount(TableDefinitionQuery query) {
		List<TableDefinition> list = tableDefinitionMapper.getTableColumnsCount(query);
		return list;
	}
	...
	public TableDefinition getTableDefinition(String tableName) {
		...
		TableDefinition tableDefinition = tableDefinitionMapper.getTableDefinitionById(tableName);
		if (tableDefinition != null) {
			if (StringUtils.isNotEmpty(tableDefinition.getQueryIds())) {
				QueryDefinitionQuery query = new QueryDefinitionQuery();
				List<String> queryIds = StringTools.split(tableDefinition.getQueryIds());
				query.queryIds(queryIds);
				List<QueryDefinition> list = queryDefinitionMapper.getQueryDefinitions(query);
			...
			if (StringUtils.isNotEmpty(tableDefinition.getAggregationQueryIds())) {
				QueryDefinitionQuery query = new QueryDefinitionQuery();
				List<String> queryIds = StringTools.split(tableDefinition.getAggregationQueryIds());
				query.queryIds(queryIds);
				List<QueryDefinition> list = queryDefinitionMapper.getQueryDefinitions(query);
				if (list != null && !list.isEmpty()) {
					for (QueryDefinition q : list) {
			...
			List<ColumnDefinition> columns = columnDefinitionMapper.getColumnDefinitionsByTableName(tableName);
	...
	@Transactional
	public void insertColumns(String tableName, List<ColumnDefinition> columns) {
		tableName = tableName.toUpperCase();
		Set<String> cols = new HashSet<String>();
		for (ColumnDefinition column : columns) {
			String id = (tableName + "_" + column.getColumnName()).toUpperCase();
			if (!cols.contains(id)) {
				column.setId(id);
	...
	public List<TableDefinition> list(TableDefinitionQuery query) {
		...
		List<TableDefinition> list = tableDefinitionMapper.getTableDefinitions(query);
		return list;
	}
	...
	public void save(TableDefinition tableDefinition) {
		...
		TableDefinition table = this.getTableDefinition(tableDefinition.getTableName());
		if (table == null) {
			...
			if (tableDefinition.getColumns() != null && !tableDefinition.getColumns().isEmpty()) {
				tableDefinition.setColumnQty(tableDefinition.getColumns().size());
			}
	...
	public void saveColumn(String tableName, ColumnDefinition columnDefinition) {
		...
		if (tableDefinition != null) {
			...
			if (!exists) {
				String id = (tableName + "_" + columnDefinition.getColumnName()).toUpperCase();
				if (!cols.contains(id)) {
	...
	@Transactional
	public void saveColumns(String targetId, List<ColumnDefinition> columnDefinitions) {
	...
	@javax.annotation.Resource
	public void setQueryDefinitionMapper(QueryDefinitionMapper queryDefinitionMapper) {
	...
	@javax.annotation.Resource
	public void setTableDefinitionMapper(TableDefinitionMapper tableDefinitionMapper) {
