...
public class MxUploadController {
	protected final static Log logger = LogFactory.getLog(MxUploadController.class);
	...
	public ModelAndView remark(HttpServletRequest request, HttpServletResponse response, ModelMap modelMap)
			throws IOException {
		...
		try {
			...
			if (dataFile != null) {
				if (StringUtils.equals(loginContext.getActorId(), dataFile.getCreateBy())
	...
	public ModelAndView remove(HttpServletRequest request, HttpServletResponse response, ModelMap modelMap)
			throws IOException {
		LoginContext loginContext = RequestUtils.getLoginContext(request);
		Map<String, Object> paramMap = RequestUtils.getParameterMap(request);
		logger.debug("paramMap:" + paramMap);
