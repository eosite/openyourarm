...
public class ViewProperties {
	...
	public static void reload() {
		if (!loading.get()) {
			...
			try {
				loading.set(true);
				String config = SystemProperties.getConfigRootPath() + "/conf/props/views";
				...
				ModulePropertyService modulePropertyService = ContextFactory.getBean("modulePropertyService");
				List<ModuleProperty> listx = modulePropertyService.getModuleProperties("message");
				if (listx != null && !listx.isEmpty()) {
					for (ModuleProperty p : listx) {
						if (StringUtils.isNotEmpty(p.getName()) && StringUtils.isNotBlank(p.getValue())) {
							properties.setProperty(p.getName(), p.getValue());
							properties.setProperty(p.getName().toLowerCase(), p.getValue());
							properties.setProperty(p.getName().toUpperCase(), p.getValue());
						}
					}
				}
