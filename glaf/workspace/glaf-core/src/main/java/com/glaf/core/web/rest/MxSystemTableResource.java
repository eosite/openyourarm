...
public class MxSystemTableResource {
	...
	protected static final Log logger = LogFactory.getLog(MxSystemTableResource.class);
	...
	public byte[] resultList(@Context HttpServletRequest request, @Context UriInfo uriInfo) {
	...
	public byte[] updateMetaInfo(@Context HttpServletRequest request, @Context UriInfo uriInfo) throws IOException {
		String tables = request.getParameter("tables");

		if (StringUtils.isNotEmpty(tables)) {
			TableDefinitionQuery query = new TableDefinitionQuery();

			List<TableDefinition> list = tableDefinitionService.getTableColumnsCount(query);
			...
			for (String tableName : list2) {
				...
				if (tableMap.get(tableName) == null) {
					try {
						List<ColumnDefinition> columns = TableFactory.getColumnDefinitions(tbl);
						tableDefinitionService.saveSystemTable(tableName, columns);
