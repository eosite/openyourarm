...
public class RequestUtils {
	...
	private static String encodeValues(String ip, String systemName, String actorId) {
		...
		//暂时ip地址屏蔽 解决因ip变动导致session丢失问题
		c_x = DigestUtils.md5Hex(/*ip*/ "" + SystemConfig.getIntToken()) + c_x;
	...
	public static String getActorId(HttpServletRequest request) {
		...
		if (session != null) {
			String value = (String) session
					.getAttribute(Constants.LOGIN_INFO + "_" + StringTools.replace(request.getContextPath(), "/", ""));
			...
			if (cookies != null && cookies.length > 0) {
				for (Cookie cookie : cookies) {
					if (StringUtils.equals(cookie.getName(),
							Constants.COOKIE_NAME + "_" + StringTools.replace(request.getContextPath(), "/", ""))) {
	...
	public static Map<String, Object> getParameterMap(HttpServletRequest request) {
		...
		while (enumeration.hasMoreElements()) {
			...
			if (StringUtils.isNotEmpty(paramName) && StringUtils.isNotEmpty(paramValue)) {
				...
				if (/*tmp.indexOf("date") > 0 && */StringUtils.endsWith(tmp, "date")//eg : updateBy 不需要转
						&& !StringUtils.equals(paramValue, "asc")
	...
	public static void setLoginUser(HttpServletRequest request, HttpServletResponse response, String systemName,
		...
		HttpSession session = request.getSession(false);
		if (session != null) {
			session.setAttribute(Constants.LOGIN_INFO + "_" + StringTools.replace(request.getContextPath(), "/", ""),
					value);
		}
		Cookie cookie = new Cookie(Constants.COOKIE_NAME + "_" + StringTools.replace(request.getContextPath(), "/", ""),
				value);
		cookie.setPath("/");
		cookie.setMaxAge(-1);
		response.addCookie(cookie);
