...
public class MxAccessServiceImpl implements IAccessService {
	protected final static Log logger = LogFactory.getLog(MxAccessServiceImpl.class);
	...
	public List<Accessable> getAccessableList(String serviceKey, String dataInstanceId) {
		if (StringUtils.isEmpty(serviceKey)) {
			throw new RuntimeException("serviceKey is null");
		}

		List<Accessable> rows = new java.util.ArrayList<Accessable>();
		List<DataAccess> list01 = this.getDataAccesses(serviceKey, dataInstanceId);
	...
	public List<DataAccess> getDataAccesses(String serviceKey, String businessKey) {
		if (StringUtils.isEmpty(serviceKey)) {
			throw new RuntimeException("serviceKey is null");
		}
		...
		List<Object> rows = this.entityDAO.getList("getDataAccessByBusinessKey", parameter);
	...
	public List<ModuleAccess> getModuleAccesses(String serviceKey) {
		...
		Map<String, Object> parameter = new java.util.HashMap<String, Object>();
		parameter.put("serviceKey", serviceKey);
		List<Object> rows = this.entityDAO.getList("getModuleAccessByServiceKey", parameter);
	...
	public void saveAccessEntry(AccessEntry accessEntry) {
		...
		if (SystemConfig.getBoolean("use_query_cache")) {
			String cacheKey = "x_acs_proc_" + accessEntry.getProcessName() + "_" + accessEntry.getTaskName();
			CacheFactory.remove("accessEntry", cacheKey);
