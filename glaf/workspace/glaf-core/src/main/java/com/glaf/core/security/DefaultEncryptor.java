...
public class DefaultEncryptor implements Encryptor {
	...
	public String decrypt(String str) throws NoSuchPaddingException, NoSuchAlgorithmException,
		...
		byte[] decrypted = null;
		try {
			decrypted = Base64.decodeBase64(new String(bytes, SystemConfig.getDefaultEncoding()));
			byte[] decs = crypt(Cipher.DECRYPT_MODE, decrypted);
			String decryptedText = new String(decs, SystemConfig.getDefaultEncoding());
	...
	public String encrypt(String str) throws NoSuchPaddingException, NoSuchAlgorithmException,
		...
		byte[] encs = null;
		try {
			encs = crypt(Cipher.ENCRYPT_MODE, str.getBytes(SystemConfig.getDefaultEncoding()));
