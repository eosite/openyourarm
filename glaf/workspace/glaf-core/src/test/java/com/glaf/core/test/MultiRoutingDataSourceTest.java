...
public class MultiRoutingDataSourceTest extends AbstractTest {
	...
	public void testMultiDS() throws IOException {
		...
		String currentName = com.glaf.core.config.Environment.getCurrentSystemName();// 记下之前的数据源环境，默认是default
		try {
			java.util.Map<String, Properties> dataSourceProperties = com.glaf.core.config.DBConfiguration
					.getDataSourceProperties();
			java.util.Iterator<String> iter = dataSourceProperties.keySet().iterator();
			while (iter.hasNext()) {
				...
				for (int i = 0; i < 100; i++) {
					SysDataLog bean = new SysDataLog();
					bean.setActorId("test");
					bean.setCreateTime(new Date());
					bean.setFlag(1);
					bean.setIp("127.0.0.1");
					bean.setOperate("insert");
					SysLogFactory.getInstance().addLog(bean);
				}
			}
		} finally {
