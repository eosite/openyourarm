...
public class H2Test1 {
	...
	public static void main(String[] args) throws Exception {
		...
		try {
			...
			conn2 = DriverManager.getConnection("jdbc:h2:./testdb", "sa", "");
			conn2.setAutoCommit(false);
			psmt1 = conn1.prepareStatement(" select index_id, parent_id, id, index_name from treepinfo ");
			psmt2 = conn2.prepareStatement(
					" create table test_treepinfo(index_id int not null, parent_id int, id varchar(200), index_name varchar(200), primary key (index_id)) ");
			psmt2.executeUpdate();
			conn2.commit();
			JdbcUtils.close(psmt2);
			psmt2 = conn2.prepareStatement(
					" insert into test_treepinfo(index_id, parent_id, id, index_name) values(?, ?, ?, ?) ");
			...
			while (rs1.next()) {
				...
				psmt2.setString(4, rs1.getString(4));
				psmt2.addBatch();
