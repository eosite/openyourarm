...
@Controller("/enterprise/schedulerLog")
@RequestMapping("/enterprise/schedulerLog")
public class SchedulerLogController {
	protected static final Log logger = LogFactory.getLog(SchedulerLogController.class);

	protected ISchedulerService schedulerService;

	protected ISchedulerLogService schedulerLogService;

	public SchedulerLogController() {
		...
		if (StringUtils.isNotEmpty(ids)) {
			...
			while (token.hasMoreTokens()) {
				...
				if (StringUtils.isNotEmpty(x)) {
					ExSchedulerLog schedulerLog = schedulerLogService.getSchedulerLog(String.valueOf(x));
			...
			ExSchedulerLog schedulerLog = schedulerLogService.getSchedulerLog(String.valueOf(id));
	...
	public byte[] deleteAll(HttpServletRequest request, ModelMap modelMap) {
		...
		if (StringUtils.isNotEmpty(taskId)) {
			Scheduler scheduler = schedulerService.getSchedulerByTaskId(taskId);
	...
	public byte[] json(HttpServletRequest request, ModelMap modelMap) throws IOException {
		...
		ExSchedulerLogQuery query = new ExSchedulerLogQuery();
		Tools.populate(query, params);
		if (ParamUtils.getDate(params, "startDate") != null) {
			query.setStartDateGreaterThanOrEqual(ParamUtils.getDate(params, "startDate"));
		}
		if (ParamUtils.getDate(params, "endDate") != null) {
			query.setEndDateLessThanOrEqual(ParamUtils.getDate(params, "endDate"));
		}
		query.deleteFlag(0);
		...
		int total = schedulerLogService.getSchedulerLogCountByQueryCriteria(query);
		if (total > 0) {
			...
			List<ExSchedulerLog> list = schedulerLogService.getSchedulerLogsByQueryCriteria(start, limit, query);
			...
			if (list != null && !list.isEmpty()) {
				...
				for (ExSchedulerLog schedulerLog : list) {
					...
					if (userMap.get(schedulerLog.getCreateBy()) != null) {
						rowJSON.put("createUserName", userMap.get(schedulerLog.getCreateBy()).getName());
					} else {
						rowJSON.put("createUserName", schedulerLog.getCreateBy());
					}
	...
	public ModelAndView list(HttpServletRequest request, ModelMap modelMap) {
		...
		Scheduler scheduler = schedulerService.getSchedulerByTaskId(request.getParameter("taskId"));
		if (scheduler != null) {
		...
		return new ModelAndView("/enterprise/scheduler/loglist", modelMap);
	}
	...
	public ModelAndView view(HttpServletRequest request, ModelMap modelMap) {
		...
		ExSchedulerLog schedulerLog = schedulerLogService.getSchedulerLog(request.getParameter("id"));
		...
		return new ModelAndView("/enterprise/scheduler/viewLog");
	}
