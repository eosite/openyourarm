...
public class ExpressionEvaluator {
	...
	public static String compile(String expression, Collection<Variable> variables){
		if (expression == null) {
			throw new ExpressionRuntimeException("表达式为空","",0);
		}
		...
		try{
			...
			e.printStackTrace();
			throw new ExpressionRuntimeException(e.getMessage()+"表达式：\"" + expression + "\" 编译期检查异常！",e.getErrorTokenText(),e.getErrorPosition());
		}finally{
			//释放脚本变量容器
	...
	public static PreparedExpression preparedCompile(String expression ,  Collection<Variable> variables){
		...
		try{
			...
			e.printStackTrace();
			throw new ExpressionRuntimeException(e.getMessage()+"表达式：\"" + expression + "\" 预编译异常！",e.getErrorTokenText(),e.getErrorPosition());
		}finally{
			//释放脚本变量容器
			VariableContainer.removeVariableMap();
		}
	...
	public static Object evaluate(String expression, Collection<Variable> variables) {
		...
		try{
			...
			e.printStackTrace();
			throw new ExpressionRuntimeException(e.getMessage()+"，表达式：\"" + expression + "\" 执行异常！",e.getErrorTokenText(),e.getErrorPosition());
		} catch (ParseException e) {
			e.printStackTrace();
			throw new ExpressionRuntimeException("表达式：\"" + expression + "\" 执行异常","",0);
		}finally{
			//释放脚本变量容器
			VariableContainer.removeVariableMap();
		}
