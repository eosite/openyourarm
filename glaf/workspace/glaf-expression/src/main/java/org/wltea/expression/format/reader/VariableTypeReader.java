...
public class VariableTypeReader implements ElementReader {
	...
	public static final char FIELD_WORD='~';//字段变量
	
	public static final String EXCUL_CHAR="~{}."; 
	...
	private String readWord(ExpressionReader sr) throws FormatException, IOException {
		...
		while ((b = sr.read()) != -1) {
			...
			if (!Character.isJavaIdentifierPart(c)&&EXCUL_CHAR.indexOf(c)<0) {
				throw new FormatException("名称不能为非法字符：" + c);
			}
			if (readStart) {
				if (!Character.isJavaIdentifierStart(c)&&EXCUL_CHAR.indexOf(c)<0) {
