				...
				if (processedActivityInfos != null
					...
					for (ActivityInfo info : processedActivityInfos) {
						...
						if(activityType.equals("callActivity")) {
							//获取子流程实例
							String subProcessInstId=activityInstance.getCalledProcessInstanceId();
							buffer.append("\n        <script>$('").append(elId)
							.append("').on(\"click\", {proInstId:\""+processInstanceId+"\",subProInstId: \""+subProcessInstId+"\"}, openSubView);");
				...
				if (activeActivityInfos != null
					...
					for (ActivityInfo activityInfo : activeActivityInfos) {
						...
						if(activityType.equals("callActivity")) {
							...
							String subProcessInstId=activityInstance.getCalledProcessInstanceId();
							buffer.append("\n        <script>$('").append(elId)
							.append("').addEvent(\"click\", function(){openSubView({proInstId:\""+processInstanceId+"\",subProInstId: \""+subProcessInstId+"\"});});");
							buffer.append("</script>");
						}
