...
public class ActivitiProcessQueryServiceImpl implements ActivitiProcessQueryService {
	protected static final Log logger = LogFactory.getLog(ActivitiProcessQueryService.class);
	...
	public HistoricActivityInstance getHistoricActivityInstance(String activityInstanceId) {
		HistoricActivityInstanceQuery query = historyService.createHistoricActivityInstanceQuery();
		query.activityInstanceId(activityInstanceId);
		return query.singleResult();
	}

	public long getHistoricActivityInstanceCount(HistoricActivityInstanceQuery query) {
		return query.count();
	}
	...
	public long getHistoricActivityInstanceCount(Map<String, Object> paramMap) {
		...
		if (paramMap != null && paramMap.size() > 0) {
			...
			if (paramMap.get("processDefinitionId") != null) {
				Object processDefinitionId = paramMap.get("processDefinitionId");
	...
	public List<HistoricActivityInstance> getHistoricActivityInstances(int firstResult, int maxResults,
	...
	public List<HistoricDetail> getHistoricDetails(int firstResult, int maxResults, HistoricDetailQuery query) {
		if (firstResult >= 0 && maxResults > 0) {
			return query.listPage(firstResult, maxResults);
		}
	...
	public List<HistoricDetail> getHistoricDetails(int firstResult, int maxResults, Map<String, Object> paramMap) {
		HistoricDetailQuery query = historyService.createHistoricDetailQuery();
		if (paramMap != null && paramMap.size() > 0) {
			if (paramMap.get("processInstanceId") != null) {
				Object processInstanceId = paramMap.get("processInstanceId");
	...
	public long getHistoricProcessInstanceCount(HistoricProcessInstanceQuery query) {
		return query.count();
	}
	...
	public List<HistoricProcessInstance> getHistoricProcessInstances(int firstResult, int maxResults,
	...
	public ProcessDefinitionInfo getLatestProcessDefinitionByKey(String processDefinitionKey) {
		ProcessDefinitionInfo processDefinitionInfo = new ProcessDefinitionInfo();
		ProcessDefinitionQuery query = repositoryService.createProcessDefinitionQuery();
		List<ProcessDefinition> list = query.processDefinitionKey(processDefinitionKey).latestVersion()
		...
		if (processDefinitionImpl != null) {
			...
			if (inputStream != null) {
				...
				List<ActivityInfo> activities = reader.read(root, processDefinitionKey);
				processDefinitionInfo.setActivities(activities);
				List<UserTask> userTasks = reader.readUserTasks(root, processDefinitionKey);
	...
	public List<ProcessDefinition> getLatestProcessDefinitions(int firstResult, int maxResults,
			Map<String, Object> paramMap) {
		ProcessDefinitionQuery query = repositoryService.createProcessDefinitionQuery();
		if (paramMap != null && paramMap.size() > 0) {
			...
			if (paramMap.get("version") != null) {
				...
				if (version instanceof Integer) {
					...
					query.processDefinitionVersion(Integer.parseInt(version.toString()));
				}
			}
	...
	public ProcessDefinition getProcessDefinitionByDeploymentId(String deploymentId) {
		ProcessDefinition processDefinition = null;
		ProcessDefinitionQuery query = repositoryService.createProcessDefinitionQuery();
		query.deploymentId(deploymentId).orderByProcessDefinitionVersion().desc();
	...
	public long getProcessDefinitionCount(Map<String, Object> paramMap) {
		...
		if (paramMap != null && paramMap.size() > 0) {
			...
			if (paramMap.get("lastVersion") != null) {
				query.latestVersion();
			}
	...
	public List<ProcessDefinition> getProcessDefinitions(int firstResult, int maxResults,
			Map<String, Object> paramMap) {
		ProcessDefinitionQuery query = repositoryService.createProcessDefinitionQuery();
		if (paramMap != null && paramMap.size() > 0) {
			if (paramMap.get("deploymentId") != null) {
	...
	public long getProcessInstanceCount(Map<String, Object> paramMap) {
		...
		if (paramMap != null && paramMap.size() > 0) {
			...
			if (paramMap.get("processDefinitionKey") != null) {
				Object processDefinitionKey = paramMap.get("processDefinitionKey");
			...
			if (paramMap.get("superProcessInstanceId") != null) {
				Object superProcessInstanceId = paramMap.get("superProcessInstanceId");
				query.superProcessInstanceId(superProcessInstanceId.toString());
			}

			if (paramMap.get("subProcessInstanceId") != null) {
				Object subProcessInstanceId = paramMap.get("subProcessInstanceId");
	...
	public ProcessInstanceInfo getProcessInstanceInfo(String processInstanceId) {
		...
		if (processInstance != null) {
			...
			ProcessDefinition processDefinition = this.getProcessDefinitionByProcessInstanceId(processInstanceId);
			info.setProcessDefinition(processDefinition);
			HistoricProcessInstance hpi = this.getHistoricProcessInstance(processInstanceId);
			info.setHistoricProcessInstance(hpi);
			ProcessDefinitionInfo pdi = this.getLatestProcessDefinitionByKey(processDefinition.getKey());
			...
			if (activities != null && !activities.isEmpty()) {
				...
				for (HistoricActivityInstance historicActivityInstance : historicActivityInstances) {
					...
					ActivityInfo act = actMap.get(historicActivityInstance.getActivityId());
					if (act != null) {
	...
	public List<ProcessInstance> getProcessInstances(int firstResult, int maxResults, Map<String, Object> paramMap) {
		ProcessInstanceQuery query = runtimeService.createProcessInstanceQuery();
		if (paramMap != null && paramMap.size() > 0) {
			if (paramMap.get("processInstanceId") != null) {
				Object processInstanceId = paramMap.get("processInstanceId");
	...
	public List<ProcessInstance> getProcessInstances(int firstResult, int maxResults, ProcessInstanceQuery query) {
		if (firstResult >= 0 && maxResults > 0) {
			return query.listPage(firstResult, maxResults);
		}
