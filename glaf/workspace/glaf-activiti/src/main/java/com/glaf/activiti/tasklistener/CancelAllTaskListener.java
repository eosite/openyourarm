...
public class CancelAllTaskListener implements TaskListener {
	...
	protected final static Log logger = LogFactory.getLog(CancelAllTaskListener.class);

	private static Cache<String, Object> cache = CacheBuilder.newBuilder().maximumSize(1000)
			...
			while (iterator.hasNext()) {
				...
				if (task instanceof TaskEntity) {
					...
					if (cache.getIfPresent(cacheKey) == null) {
						cache.put(cacheKey, "1");
						logger.debug(taskEntity.getTaskDefinitionKey() + " [" + taskEntity.getName() + "] canceled");
						// taskEntity.complete();
					}
				}
			}
