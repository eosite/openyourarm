...
public class ActivitiProcessServiceImpl implements ActivitiProcessService {
		...
		try {
			...
			for (PvmTransition nextTransition : nextTransitionList) {
				...
				List<HistoricTaskInstance> completeTasks = historyService.createHistoricTaskInstanceQuery()
						.processInstanceId(processInstance.getId()).taskDefinitionKey(nextActivity.getId()).finished()
						.list();
				int finished = completeTasks.size();
				if (finished > 0) {
					return ResponseUtils.responseJsonResult(false, "存在已经完成的活动，流程不能取回");
				}
				List<Task> nextTasks = taskService.createTaskQuery().processInstanceId(processInstance.getId())
	...
	public void completeTask(ProcessContext ctx) {
		...
		if (taskId != null) {
			...
			if (tasks != null && !tasks.isEmpty()) {
				...
				List<TaskItem> items = this.activitiTaskQueryService.getTaskItems(ctx.getProcessInstanceId());
	 ...
	 *            驳回原因
	 */
	public void rejectTask(String processInstanceId, String destTaskDefKey, String rejectMessage) {
		// 获得当前任务的对应实列
	...
	public byte[] rollback(String taskId) {
		...
		try {
			...
			ActivityImpl activity = ((ProcessDefinitionImpl) definition)
					.findActivity(historicTaskInstance.getTaskDefinitionKey());
			List<PvmTransition> nextTransitionList = activity.getIncomingTransitions();
			// 清除当前活动的出口
			List<PvmTransition> oriPvmTransitionList = new ArrayList<PvmTransition>();
			...
			List<Task> tasks = taskService.createTaskQuery().processInstanceId(processInstance.getId())
					.taskDefinitionKey(historicTaskInstance.getTaskDefinitionKey()).list();
			for (Task task : tasks) {
