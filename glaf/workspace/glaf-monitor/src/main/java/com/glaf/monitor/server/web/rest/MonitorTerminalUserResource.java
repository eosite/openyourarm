...
import com.glaf.monitor.server.domain.MonitorTerminalUser;
import com.glaf.monitor.server.query.MonitorTerminalUserQuery;
import com.glaf.monitor.server.service.MonitorTerminalUserService;
...
@Controller
@Path("/rs/monitor/monitorTerminalUser")
public class MonitorTerminalUserResource {
	protected static final Log logger = LogFactory.getLog(MonitorTerminalUserResource.class);
	...
	public byte[] deleteAll(@Context HttpServletRequest request)
		...
		if (rowIds != null) {
			...
			if (ids != null && !ids.isEmpty()) {
				monitorTerminalUserService.deleteByIds(ids);
			}
		}
	...
	public byte[] deleteById(@Context HttpServletRequest request) throws IOException {
                monitorTerminalUserService.deleteById(request.getParameter("id"));
		return ResponseUtils.responseJsonResult(true);
	}
	...
	@POST
	@Path("/saveMonitorTerminalUser")
	@ResponseBody
	@Produces({ MediaType.APPLICATION_JSON })
	public byte[] saveMonitorTerminalUser(@Context HttpServletRequest request) {
	        Map<String, Object> params = RequestUtils.getParameterMap(request);
	...
	public byte[] view(@Context HttpServletRequest request) throws IOException {
		MonitorTerminalUser monitorTerminalUser = null;
		if (StringUtils.isNotEmpty(request.getParameter("id"))) {
                  monitorTerminalUser = monitorTerminalUserService.getMonitorTerminalUser(request.getParameter("id"));
		}
		JSONObject result = new JSONObject();
		if (monitorTerminalUser != null) {
		    result =  monitorTerminalUser.toJsonObject();
		    Map<String, User> userMap = IdentityFactory.getUserMap();
		    result.put("id", monitorTerminalUser.getId());
		    result.put("monitorTerminalUserId", monitorTerminalUser.getId());
		}
		return result.toJSONString().getBytes("UTF-8");
	}
}
