...
import com.glaf.monitor.server.domain.MonitorEvent;
import com.glaf.monitor.server.query.MonitorEventQuery;
import com.glaf.monitor.server.service.MonitorEventService;
...
@Controller
@Path("/rs/monitor/monitorEvent")
public class MonitorEventResource {
	protected static final Log logger = LogFactory.getLog(MonitorEventResource.class);
	...
	public byte[] deleteAll(@Context HttpServletRequest request)
			throws IOException {
		String rowIds = request.getParameter("eventIds");
		if (rowIds != null) {
 			List<String> ids = StringTools.split(rowIds);
			if (ids != null && !ids.isEmpty()) {
				monitorEventService.deleteByIds(ids);
			}
		}
	...
	public byte[] deleteById(@Context HttpServletRequest request) throws IOException {
                monitorEventService.deleteById(request.getParameter("eventId"));
		return ResponseUtils.responseJsonResult(true);
	}
	...
	@POST
	@Path("/saveMonitorEvent")
	@ResponseBody
	@Produces({ MediaType.APPLICATION_JSON })
	public byte[] saveMonitorEvent(@Context HttpServletRequest request) {
	        Map<String, Object> params = RequestUtils.getParameterMap(request);
	...
	public byte[] view(@Context HttpServletRequest request) throws IOException {
		...
		if (StringUtils.isNotEmpty(request.getParameter("eventId"))) {
                  monitorEvent = monitorEventService.getMonitorEvent(request.getParameter("eventId"));
		}
		JSONObject result = new JSONObject();
		if (monitorEvent != null) {
		    result =  monitorEvent.toJsonObject();
		    Map<String, User> userMap = IdentityFactory.getUserMap();
		}
		return result.toJSONString().getBytes("UTF-8");
	}
