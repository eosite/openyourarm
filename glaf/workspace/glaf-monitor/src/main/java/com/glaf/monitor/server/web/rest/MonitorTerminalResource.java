...
@Controller
@Path("/rs/monitor/monitorTerminal")
public class MonitorTerminalResource {
	protected static final Log logger = LogFactory.getLog(MonitorTerminalResource.class);
	...
	public byte[] deleteAll(@Context HttpServletRequest request)
		...
		if (rowIds != null) {
			...
			if (ids != null && !ids.isEmpty()) {
				monitorTerminalService.deleteByIds(ids);
			}
		}
	...
	public byte[] deleteById(@Context HttpServletRequest request) throws IOException {
                monitorTerminalService.deleteById(request.getParameter("id"));
		return ResponseUtils.responseJsonResult(true);
	}
	...
	public byte[] data(@Context HttpServletRequest request, @RequestBody DataRequest dataRequest) throws IOException {
		...
		MonitorTerminalDomainFactory.processDataRequest(dataRequest);
		...
		if (total > 0) {
			...
			List<MonitorTerminal> list = monitorTerminalService.getMonitorTerminalsByQueryCriteria(start, limit,
					query);
	...
	@POST
	@Path("/saveMonitorTerminal")
	@ResponseBody
	@Produces({ MediaType.APPLICATION_JSON })
	public byte[] saveMonitorTerminal(@Context HttpServletRequest request) {
	        Map<String, Object> params = RequestUtils.getParameterMap(request);
	...
	public byte[] view(@Context HttpServletRequest request) throws IOException {
		MonitorTerminal monitorTerminal = null;
		if (StringUtils.isNotEmpty(request.getParameter("id"))) {
                  monitorTerminal = monitorTerminalService.getMonitorTerminal(request.getParameter("id"));
		}
		JSONObject result = new JSONObject();
		if (monitorTerminal != null) {
		    result =  monitorTerminal.toJsonObject();
		    Map<String, User> userMap = IdentityFactory.getUserMap();
		    result.put("id", monitorTerminal.getId());
		    result.put("monitorTerminalId", monitorTerminal.getId());
		}
		return result.toJSONString().getBytes("UTF-8");
	}
}
