...
@Controller
@Path("/rs/monitor/monitorProcItem")
public class MonitorProcItemResource {
	protected static final Log logger = LogFactory.getLog(MonitorProcItemResource.class);
	...
	public byte[] deleteAll(@Context HttpServletRequest request)
		...
		if (rowIds != null) {
			...
			if (ids != null && !ids.isEmpty()) {
				monitorProcItemService.deleteByIds(ids);
			}
		}
	...
	public byte[] deleteById(@Context HttpServletRequest request) throws IOException {
                monitorProcItemService.deleteById(request.getParameter("id"));
		return ResponseUtils.responseJsonResult(true);
	}
	...
	public byte[] data(@Context HttpServletRequest request, @RequestBody DataRequest dataRequest) throws IOException {
		...
		MonitorProcItemDomainFactory.processDataRequest(dataRequest);
	...
	@POST
	@Path("/saveMonitorProcItem")
	@ResponseBody
	@Produces({ MediaType.APPLICATION_JSON })
	public byte[] saveMonitorProcItem(@Context HttpServletRequest request) {
	        Map<String, Object> params = RequestUtils.getParameterMap(request);
	...
	public byte[] view(@Context HttpServletRequest request) throws IOException {
		MonitorProcItem monitorProcItem = null;
		if (StringUtils.isNotEmpty(request.getParameter("id"))) {
                  monitorProcItem = monitorProcItemService.getMonitorProcItem(request.getParameter("id"));
		}
		JSONObject result = new JSONObject();
		if (monitorProcItem != null) {
		    result =  monitorProcItem.toJsonObject();
		    Map<String, User> userMap = IdentityFactory.getUserMap();
		    result.put("id", monitorProcItem.getId());
		    result.put("monitorProcItemId", monitorProcItem.getId());
		}
		return result.toJSONString().getBytes("UTF-8");
	}
}
