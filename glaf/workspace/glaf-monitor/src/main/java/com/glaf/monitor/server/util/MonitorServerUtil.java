...
public class MonitorServerUtil {
	protected static final Log logger = LogFactory.getLog(MonitorServerUtil.class);
...
//	protected MonitorLogService monitorLogService;
	...
	public void parseClientData(JSONObject data){
		parseMonitorTerminalData(data);
		parseProcData(data);
	}
	
	public void parseProcData(JSONObject data){
		//客户端的id
		String terminalId = data.getString("terminalId");
		//服务/进程返回信息（全部）
		JSONObject procDatas = data.getJSONObject("procDatas");
		...
		monitorProcQuery.setTerminalId(terminalId);
		...
		for (MonitorProc monitorProc : procList) {
			//获取对应服务/进程反馈信息
			itemParent = procDatas.getJSONObject(monitorProc.getId());
			
			if(itemParent == null){
				continue;
			}
			//将数据放入缓存中
			MonitorCacheUtil.setMonitorProcData(monitorProc.getId(), itemParent);
			//解析数据
...
//						warningPeople(monitorProc, errorMessage);
	...
	public void parseTerminalData(JSONObject data){
		BufferedWriter bufferWrite  = null;
		
		//客户端的id
		String terminalId = data.getString("terminalId");
		String strValue = null,errorMessage = null;
		Double  intValue;
		
		MonitorTerminal terminal = monitorTerminalService.getMonitorTerminal(terminalId);
		if(terminal != null){
			//将数据放入缓存中
			MonitorCacheUtil.setMonitorTerminalData(terminalId, data.getJSONObject("infoData"));
			
			try{
				bufferWrite = monitorLogUtils.getLogWriter(terminal);
				monitorLogUtils.writeLog(bufferWrite, "解析终端反馈结果信息:");
				monitorLogUtils.writeLog(bufferWrite, data.toJSONString());
			}catch(Exception e){
				e.printStackTrace();
				logger.error(e.getMessage());
			...
			//获取操作系统信息
			JSONObject itemParent = data.getJSONObject("infoData");
			...
			List<MonitorTerminalItem> terminalItemList = monitorTerminalItemService.list(terminalItemQuery);
			JSONObject item = null;
			JSONObject item2 = null;
	 ...
	 * 校验CPU是否需要预警
	 * @param data
	 * @param monitorTerminalItem
	 * @return
	 */
	public boolean checkCpuData(JSONObject data,MonitorTerminalItem terminalItem){
		String strValue = terminalItem.getRefType();	//获取参照类型
		boolean isWarning = false; //是否预警
		if (StringUtils.isNotEmpty(strValue)) {
			if (StringUtils.isEquals(strValue, "1")) {
				// 判断可用不低于
				//CPU只能判断百分比。
				double cpuIdle = data.getDoubleValue("cpuIdle");// CPU总的空闲率
				if(terminalItem.getAlarmVal() > cpuIdle){
					//当空闲率低于预警值时，进行预警
					isWarning = true;
				}
			} else {
				// 判断使用不高于
				//CPU只能判断百分比。
				double cpuCombined = data.getDoubleValue("cpuCombined");// CPU总的使用率
				if(terminalItem.getAlarmVal() < cpuCombined){
					//当使用率高于预警值时，进行预警
					isWarning = true;
				}
			}
	 ...
	 * @return
	 */
	public boolean checkMemData(JSONObject data,MonitorTerminalItem terminalItem){
		String strValue = terminalItem.getRefType();	//获取参照类型
		boolean isWarning = false; //是否预警
		String prevType = terminalItem.getUnit();
		if (StringUtils.isNotEmpty(strValue)) {
			if (StringUtils.isEquals(strValue, "1")) {
				// 判断可用不低于
				double value;
				if (StringUtils.isEquals(prevType, "percent")) {
					//判断百分比
					value = data.getDoubleValue("usedPercent");//使用率
				} else {
					//判断数值
					value = data.getDoubleValue("used");//使用空间
				}
				if(terminalItem.getAlarmVal() > value){
					//当空闲率低于预警值时，进行预警
					isWarning = true;
				}
			} else {
	 ...
	 * @return
	 */
	public boolean checkDiskData(JSONObject data,MonitorTerminalItem terminalItem){
		String strValue = terminalItem.getRefType();	//获取参照类型
		boolean isWarning = false; //是否预警
		String prevType = terminalItem.getUnit();
		if (StringUtils.isNotEmpty(strValue)) {
			if (StringUtils.isEquals(strValue, "1")) {
				...
				if (StringUtils.isEquals(prevType, "percent")) {
					//判断百分比
					value = data.getDoubleValue("used") / data.getDoubleValue("total") * 100;//使用率
				} else {
					//判断数值
					value = convertTob(data.getDoubleValue("used"),data.getString("unit"));//使用空间
				}
				if(terminalItem.getAlarmVal() > value){
					//当空闲率低于预警值时，进行预警
					isWarning = true;
	...
	public void parseMonitorTerminalData(JSONObject data){
		BufferedWriter bufferWrite  = null;
		...
		if(terminal != null){
			...
			JSONObject itemParent = data.getJSONObject("infoData");
			//将数据放入缓存中
			MonitorCacheUtil.setMonitorTerminalData(terminalId, itemParent);
			// 遍历预警项值
			MonitorTerminalItemQuery terminalItemQuery = new MonitorTerminalItemQuery();
			terminalItemQuery.setTenantId(terminal.getId());
			terminalItemQuery.setDeleteFlag(0);
			...
			for (MonitorTerminalItem terminalItem : terminalItemList) {
				...
				if(StringUtils.isEmpty(terminalItem.getCode())){
					continue;
				}
				boolean isWarning = false;	//是否预警
				switch(terminalItem.getCode()){
					case "cpu":isWarning = checkCpuData(item.getJSONObject(terminalItem.getCode()),terminalItem);break;
					case "mem":isWarning = checkMemData(item.getJSONObject(terminalItem.getCode()),terminalItem);break;
					case "disk":isWarning = checkDiskData(item.getJSONObject(terminalItem.getCode()),terminalItem);break;
					case "temp":break;
					case "fans":break;
					default: break;
				}
