...
import com.glaf.matrix.data.sync.mapper.SyncAppMapper;
import com.glaf.matrix.data.sync.mapper.SyncItemMapper;
import com.glaf.matrix.data.sync.query.SyncAppQuery;
import com.glaf.matrix.data.sync.query.SyncItemQuery;

@Service("com.glaf.matrix.data.sync.service.syncAppService")
...
public class SyncAppServiceImpl implements SyncAppService {
	...
	public long saveAs(long syncId, String createBy) {
		SyncApp model = this.getSyncApp(syncId);
		if (model != null) {
			model.setId(idGenerator.nextId("SYS_SYNC_APP"));
			model.setCreateTime(new Date());
			model.setCreateBy(createBy);
			syncAppMapper.insertSyncApp(model);

			if (model.getItems() != null && !model.getItems().isEmpty()) {
				for (SyncItem item : model.getItems()) {
					item.setId(idGenerator.nextId("SYS_SYNC_ITEM"));
					item.setCreateTime(new Date());
					item.setCreateBy(createBy);
					item.setSyncId(model.getId());
					syncItemMapper.insertSyncItem(item);
				}
			}
	...
	@javax.annotation.Resource(name = "com.glaf.matrix.data.sync.mapper.SyncAppMapper")
	public void setSyncAppMapper(SyncAppMapper syncAppMapper) {
		this.syncAppMapper = syncAppMapper;
	}

	@javax.annotation.Resource(name = "com.glaf.matrix.data.sync.mapper.SyncItemMapper")
