...
public class DataFileController {
	...
	public byte[] deleteById(HttpServletRequest request) {
		...
		if (StringUtils.isNotEmpty(fileId)) {
			DataFile dataFile = DataFileFactory.getInstance().getDataFileByFileId(fileId);
			if (dataFile != null && (loginContext.isSystemAdministrator()
					|| StringUtils.equals(dataFile.getCreateBy(), loginContext.getActorId()))) {
				DataFileFactory.getInstance().deleteDataFileByFileId(fileId);
	...
	public byte[] deleteByName(HttpServletRequest request) {
		...
		if (StringUtils.isNotEmpty(serviceKey) && StringUtils.isNotEmpty(filename)) {
			DataFileFactory.getInstance().deleteByServiceKey(serviceKey, loginContext.getActorId(), filename, status);
			return ResponseUtils.responseJsonResult(true);
		}
		return ResponseUtils.responseJsonResult(false);
	}
					...
					if (StringUtils.equals(tableDefinition.getAttachmentFlag(), "1")
						...
						for (Entry<String, MultipartFile> entry : entrySet) {
							...
							if (mFile.getOriginalFilename() != null && mFile.getSize() > 0
								...
								DataFileFactory.getInstance().insertDataFile(dataFile, mFile.getBytes());
	...
	public void download(HttpServletRequest request, HttpServletResponse response) {
		...
		if (StringUtils.isNotEmpty(fileId)) {
			...
			DataFile blob = DataFileFactory.getInstance().getDataFileByFileId(fileId);
			if (blob != null) {
				logger.debug("id:" + blob.getId());
				InputStream inputStream = null;
				try {
					inputStream = DataFileFactory.getInstance().getInputStreamById(blob.getId());
