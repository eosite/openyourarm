		...
		try {
			...
			if (total > 0) {
				...
				while (sourceRS.next()) {
					...
					/**
					 * Map<String, Object> resultMap = new LowerLinkedMap(); for
					 * (int i = 0, len = columns.size(); i < len; i++) { String
					 * name = columns.get(i); TypeHandler<?> handler =
					 * typeHandlers.get(i); Object value =
					 * handler.getResult(sourceRS, name); resultMap.put(name,
					 * value); if (value != null && value instanceof
					 * java.util.Date) { java.util.Date date = (java.util.Date)
					 * value; resultMap.put(name, DateUtils.getDate(date)); } if
					 * (value != null && value instanceof java.sql.Clob) {
					 * java.sql.Clob clob = (java.sql.Clob) value; String str =
					 * this.clobToString(clob); resultMap.put(name, str); } }
					 **/
