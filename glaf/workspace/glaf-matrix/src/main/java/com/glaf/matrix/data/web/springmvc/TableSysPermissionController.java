...
public class TableSysPermissionController {
	...
	public ModelAndView list(HttpServletRequest request, ModelMap modelMap) {
		...
		return new ModelAndView("/matrix/sys/tableSysPermission/list", modelMap);
	}
	...
	public ModelAndView privilege(HttpServletRequest request, ModelMap modelMap) {
		...
		if (StringUtils.isNotEmpty(tableId) && StringUtils.isNotEmpty(granteeType)
			...
			if (StringUtils.equals(granteeType, "role")) {
					...
					for (Role role : roles) {
						...
						if (list != null && !list.isEmpty()) {
							for (TableSysPermission p : list) {
								if (StringUtils.equals(String.valueOf(role.getId()), p.getGrantee())) {
			...
			} else if (StringUtils.equals(granteeType, "user")) {
				...
				if (users != null && !users.isEmpty()) {
					...
					for (User user : users) {
						...
						if (list != null && !list.isEmpty()) {
							for (TableSysPermission p : list) {
								if (StringUtils.equals(user.getActorId(), p.getGrantee())) {
									BaseItem item = new BaseItem();
									item.setName(user.getName());
									item.setValue(user.getActorId());
									selected.add(item);
									include = true;
								}
							}
				...
				List<Group> groups = IdentityFactory.getGroups();
		...
		return new ModelAndView("/matrix/sys/tableSysPermission/privilege", modelMap);
	}
