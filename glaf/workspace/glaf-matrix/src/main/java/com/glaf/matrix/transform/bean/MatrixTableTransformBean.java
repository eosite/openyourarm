...
public class MatrixTableTransformBean implements java.lang.Runnable {
	...
	public MatrixTableTransformBean() {

	}

	public MatrixTableTransformBean(long databaseId, String transformId, Map<String, Object> parameter) {
	...
	public boolean execute(long databaseId, String transformId, Map<String, Object> params) {
		...
		try {
			MatrixTableTransform matrixTableTransform = matrixTableTransformService
					.getMatrixTableTransform(transformId);
			logger.debug("transformId:" + transformId);
			// logger.debug("matrixTableTransform:" + matrixTableTransform);
			...
			List<MatrixColumnTransform> cols = matrixTableTransform.getColumns();
			if (cols != null && !cols.isEmpty()) {
				...
				for (ColumnDefinition column : columns) {
					...
					if (StringUtils.equalsIgnoreCase(column.getColumnName(), matrixTableTransform.getPrimaryKey())) {
						idColumn = column;
					}
				...
				if (idColumn == null) {
					ExceptionUtils.addMsg("matrix_table_transform_" + tableName, "转换表未设置主键字段");
					return false;
				}

				int index = 1;
				List<MatrixColumnTransform> updateColumns = new ArrayList<MatrixColumnTransform>();
				for (MatrixColumnTransform c : cols) {
				...
				for (MatrixColumnTransform cm : cols) {
				...
				if (StringUtils.equals(idColumn.getJavaType(), "String")) {
					sql = " select count(" + idColumn.getColumnName() + ") from " + matrixTableTransform.getTableName();
					if (StringUtils.isNotEmpty(matrixTableTransform.getSqlCriteria())) {
						sql = sql + " where 1=1 and " + matrixTableTransform.getSqlCriteria();
					}
					...
					sql = " select max(" + idColumn.getColumnName() + "), min(" + idColumn.getColumnName() + ") from "
							+ matrixTableTransform.getTableName();
					if (StringUtils.isNotEmpty(matrixTableTransform.getSqlCriteria())) {
						sql = sql + " where 1=1 and " + matrixTableTransform.getSqlCriteria();
					}
					if (!DBUtils.isAllowedSql(sql)) {
				...
				if (StringUtils.isNotEmpty(matrixTableTransform.getTransformColumns())) {
					selectBuffer.append(idColumnName).append(", ");
					selectBuffer.append(matrixTableTransform.getTransformColumns());
				} else {
					selectBuffer.append("*");
				}
				...
				for (int i = 0; i < maxForEach; i++) {
					if (StringUtils.equals(idColumn.getJavaType(), "String")) {
						sql = " select " + selectBuffer.toString() + " from " + matrixTableTransform.getTableName();
						if (StringUtils.isNotEmpty(matrixTableTransform.getSqlCriteria())) {
							sql = sql + " where 1=1 and " + matrixTableTransform.getSqlCriteria();
						}
					} else {
						...
						if (StringUtils.isNotEmpty(matrixTableTransform.getSqlCriteria())) {
							sql = sql + " and " + matrixTableTransform.getSqlCriteria();
						}
					}
					...
					if (dataList != null && !dataList.isEmpty()) {
						...
						for (Map<String, Object> dataMap : dataList2) {
							...
							for (MatrixColumnTransform cm : updateColumns) {
			...
			ExceptionUtils.addMsg("matrix_table_transform_" + transformId, ex.getMessage());
