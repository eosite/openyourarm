					...
					if (columns != null && !columns.isEmpty()) {
						...
						for (int i = 0, len = array.size(); i < len; i++) {
							...
							for (TableColumn column : exportColumns) {
								...
								switch (column.getType()) {
								case "Date":
									str = json.getString(column.getLowerColumnName());
									cell.setCellValue(str);
									txtStyle.setAlignment(HorizontalAlignment.CENTER);
									cell.setCellStyle(txtStyle);
									break;
								case "Integer":
									Integer ix = json.getInteger(column.getLowerColumnName());
									cell.setCellValue(ix);
									numStyle.setAlignment(HorizontalAlignment.RIGHT);
									cell.setCellStyle(numStyle);
									break;
								case "Long":
									Long lx = json.getLong(column.getLowerColumnName());
									cell.setCellValue(lx);
									numStyle.setAlignment(HorizontalAlignment.RIGHT);
									cell.setCellStyle(numStyle);
									break;
								case "Double":
									double d = json.getDouble(column.getLowerColumnName());
									cell.setCellValue(d);
									numStyle.setAlignment(HorizontalAlignment.RIGHT);
									cell.setCellStyle(numStyle);
									break;
								case "String":
									str = json.getString(column.getLowerColumnName());
									//logger.debug(column.getLowerColumnName()+":" + str);
									cell.setCellValue(str);
									txtStyle.setAlignment(HorizontalAlignment.LEFT);
									cell.setCellStyle(txtStyle);
									break;
								default:
									object = json.get(column.getLowerColumnName());
