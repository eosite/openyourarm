...
public class AutoStorageJob extends BaseJob {
	...
	public void runJob(JobExecutionContext context) throws JobExecutionException {
		...
		if (list != null && !list.isEmpty()) {
			...
			DBToSQLite bean = new DBToSQLite();
			...
			for (StorageApp storageApp : list) {
				if (StringUtils.containsIgnoreCase(storageApp.getTableNames(), "FORM_PAGE")
						|| StringUtils.containsIgnoreCase(storageApp.getTableNames(), "FORM_RULE")
						|| StringUtils.containsIgnoreCase(storageApp.getTableNames(), "FORM_RULE_PROPERTY")) {
					if (storageApp.getInterval() < 60) {
						storageApp.setInterval(60);
					}
				}
				try {
					...
					String sqliteDB = "storage_" + DateUtils.getNowYearMonthDay() + "_" + storageApp.getId() + "_"
							+ version + ".db";
