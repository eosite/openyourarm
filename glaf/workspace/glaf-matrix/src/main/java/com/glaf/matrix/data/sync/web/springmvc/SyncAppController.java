...
import com.glaf.matrix.data.sync.bean.SqlToTableSyncBean;
import com.glaf.matrix.data.sync.domain.*;
import com.glaf.matrix.data.sync.query.*;
import com.glaf.matrix.data.sync.service.*;
...
public class SyncAppController {
			...
			if (syncApp != null && StringUtils.equals(syncApp.getActive(), "Y")) {
				...
				for (Long targetDBId : targetDBIds) {
					...
					if (database != null) {
						...
						bean.execute(syncApp.getSrcDatabaseId(), targetDBId, syncId, params);
	...
	public byte[] save(HttpServletRequest request) {
		...
		try {
			...
			syncApp.setSortNo(RequestUtils.getInt(request, "sortNo"));
	...
	public byte[] saveAs(HttpServletRequest request) {
		...
		try {
			long syncId = RequestUtils.getLong(request, "syncId");
			if (syncId > 0) {
				long nid = syncAppService.saveAs(syncId, actorId);
				if (nid > 0) {
					return ResponseUtils.responseJsonResult(true);
				}
			}
	...
	@javax.annotation.Resource(name = "com.glaf.matrix.data.sync.service.syncAppService")
	public void setSyncAppService(SyncAppService syncAppService) {
		this.syncAppService = syncAppService;
	}

	@javax.annotation.Resource(name = "com.glaf.matrix.data.sync.service.syncHistoryService")
