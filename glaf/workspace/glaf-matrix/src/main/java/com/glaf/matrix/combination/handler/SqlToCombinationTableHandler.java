			...
			for (long targetDatabaseId : databaseIds) {
				...
				try {
					...
					if (database != null) {
						...
						if (combinationApp.getSrcDatabaseId() > 0) {
							task = new CombinationAppTask(combinationApp.getSrcDatabaseId(), database.getId(), syncId,
									jobNo, context);
						} else {
							task = new CombinationAppTask(database.getId(), database.getId(), syncId, jobNo, context);
						}
