			...
			for (CombinationApp app : rows) {
				if (StringUtils.isNotEmpty(app.getTargetDatabaseIds())) {
					...
					for (long targetDatabaseId : targetDatabaseIds) {
						...
						try {
							CombinationHistory last = combinationHistoryService.getLatestCombinationHistory(app.getId(),
									targetDatabaseId);
							...
							if (database != null) {
								jobNo = "combination_app_" + app.getId() + "_" + targetDatabaseId + "_"
										+ DateUtils.getNowYearMonthDayHHmmss();
								ExecutionUtils.put(jobNo, new Date());
								if (app.getSrcDatabaseId() > 0) {
									task = new CombinationAppTask(app.getSrcDatabaseId(), targetDatabaseId, app.getId(),
											jobNo, parameter);
								} else {
									task = new CombinationAppTask(targetDatabaseId, targetDatabaseId, app.getId(),
											jobNo, parameter);
								}
