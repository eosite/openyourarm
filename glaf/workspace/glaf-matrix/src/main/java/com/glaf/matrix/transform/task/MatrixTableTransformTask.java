...
import com.glaf.matrix.transform.bean.MatrixTableTransformBean;
...
public class MatrixTableTransformTask extends RecursiveTask<Boolean> {
	public final static Log logger = LogFactory.getLog(MatrixTableTransformTask.class);
	...
	public MatrixTableTransformTask(long databaseId, String transformId, String jobNo, Map<String, Object> parameter) {
	...
	public boolean execute() {
		...
		MatrixTableTransformBean bean = new MatrixTableTransformBean();
		boolean result = false;
		try {
			ExecutionHandlerFactory.doBefore("matrix_table_transform_" + transformId, context);
			result = bean.execute(databaseId, transformId, parameter);
		} catch (Exception ex) {
			result = false;
			logger.error(ex);
			...
			if (result) {
				...
				ExecutionHandlerFactory.doAfter("matrix_table_transform_" + transformId, context);
			}
		}
		...
		sysLog.setServiceKey("matrix_table_transform");
		sysLog.setModuleId("matrix_table_transform_" + transformId);
		sysLog.setBusinessKey("matrix_table_transform_" + transformId);
		SysLogFactory.getInstance().addLog(sysLog);

		Calendar calendar = Calendar.getInstance();
		calendar.setTime(new Date());
		log.setBusinessKey(transformId);
		...
		log.setType("matrix_table_transform");
		try {
