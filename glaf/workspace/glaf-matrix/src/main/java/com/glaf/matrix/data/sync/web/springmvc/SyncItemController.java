...
public class SyncItemController {
	...
	public byte[] save(HttpServletRequest request) {
		...
		try {
			...
			syncItem.setPrimaryKey(request.getParameter("primaryKey"));
			syncItem.setTargetTableName(request.getParameter("targetTableName"));
			syncItem.setExpression(request.getParameter("expression"));
			syncItem.setCreateTableFlag(request.getParameter("createTableFlag"));
			syncItem.setDeleteFetch(request.getParameter("deleteFetch"));
	...
	@javax.annotation.Resource(name = "com.glaf.matrix.data.sync.service.syncItemService")
