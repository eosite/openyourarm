...
@Service("com.glaf.base.usertask.service.userTaskService")
@Transactional(readOnly = true)
public class UserTaskServiceImpl implements IUserTaskService {
	protected static final Log logger = LogFactory.getLog(UserTaskServiceImpl.class);
	...
	public List<Long> getUserNetRoleIds(String actorId) {
		List<Long> roleIds = new ArrayList<Long>();
		List<SysRole> roles = sysRoleMapper.getSysRolesOfUser(actorId);
	 ...
	 * @return
	 */
	public List<UserTask> getUserTasksByQueryCriteria(int start, int pageSize, UserTaskQuery query) {
		RowBounds rowBounds = new RowBounds(start, pageSize);
		List<UserTask> rows = sqlSession.selectList("getUserTasksByQueryCriteria", query, rowBounds);
		return rows;
	}

	public int getWorkflowUserTaskCount(Map<String, Object> parameter) {
		return userTaskMapper.getWorkflowUserTaskCount(parameter);
	}

	public List<UserTask> list(UserTaskQuery query) {
		List<UserTask> list = userTaskMapper.getUserTasksByQueryCriteria(query);
		return list;
	}
	...
	public void reloadUserTasksGroupByTime(String actorId, String taskType) {
		...
		List<ObjectEntity> taskIds = userTaskMapper.getMyCellTaskIds(params);
		if (taskIds != null && !taskIds.isEmpty()) {
			...
			for (ObjectEntity o : taskIds) {
				...
				if (tmpList.size() % 500 == 0) {
					...
					List<ObjectEntity> taskDates = userTaskMapper.getMyCellTaskDates(params2);
				...
				for (String date : allDates) {
					...
					for (ObjectEntity o : taskIds) {
						...
						if (tmpList.size() % 500 == 0) {
							...
							List<TaskItem> taskItems = userTaskMapper.getAllMyTaskItemMap(params);
							if (taskItems != null && !taskItems.isEmpty()) {
								for (TaskItem ti : taskItems) {
									...
									userTask.setLastModified(System.currentTimeMillis());
									userTaskMapper.insertUserTask(userTask);
								}
							}
							// clear
		...
		if (taskIds != null && !taskIds.isEmpty()) {
			...
			for (ObjectEntity o : taskIds) {
				...
				if (tmpList.size() % 500 == 0) {
					Map<String, Object> params2 = new HashMap<String, Object>();
					params2.put("taskIds", tmpList);
					List<ObjectEntity> indexIds = userTaskMapper.getMyCellTaskIndexIds(params2);
					tmpList.clear();
					if (indexIds.size() > 0) {
			...
			if (allIndexIds.size() > 0) {
				...
				for (Integer ix : allIndexIds) {
					...
					if (trees != null && !trees.isEmpty()) {
						if (trees.size() >= 2) {
							if (!activIndexIds.contains(trees.get(1).getIndexId())) {
								UserTask userTask = new UserTask();
								userTask.setId(UUID32.getUUID());
								userTask.setIndexId(idGenerator.nextId().intValue());
								userTask.setActivityId(String.valueOf(trees.get(1).getIndexId()));
								userTask.setActorId(actorId);
								userTask.setIndexName(trees.get(0).getIndexName() + "\\" + trees.get(1).getIndexName());
								userTask.setName(userTask.getIndexName());
								userTask.setParentId(-1);
								userTask.setLevel(1);
								userTask.setState(-1);
								userTaskMapper.insertUserTask(userTask);
								intMap.put(trees.get(1).getIndexId(), userTask.getIndexId());
				...
				for (Integer ix : activIndexIds) {
					...
					for (ObjectEntity o : taskIds) {
						...
						if (tmpList.size() % 500 == 0) {
							...
							List<TaskItem> taskItems = userTaskMapper.getMyTaskItemMap(params);
							if (taskItems != null && !taskItems.isEmpty()) {
								for (TaskItem ti : taskItems) {
									UserTask userTask = new UserTask();
									userTask.setId(UUID32.getUUID());
			...
			if (allIndexIds.size() > 0) {
				...
				if (trees != null && !trees.isEmpty()) {
					...
					if (trees.size() >= 2) {
						...
						List<BaseTree> trees5 = userTaskMapper.getMyTaskTreepinfos(params5);
						...
						for (ITree tree : trees) {
							...
							if (parent != null) {
								...
								userTask.setId(UUID32.getUUID());
								userTask.setIndexId(idGenerator.nextId().intValue());
								userTask.setActivityId(String.valueOf(tree.getIndexId()));
								userTask.setActorId(actorId);
								userTask.setIndexName(parent.getIndexName() + "\\" + tree.getIndexName());
								userTask.setName(userTask.getIndexName());
								userTask.setParentId(-1);
								userTask.setLevel(1);
								userTask.setState(-1);
								userTaskMapper.insertUserTask(userTask);
								intMap.put(tree.getIndexId(), userTask.getIndexId());
	...
	@javax.annotation.Resource(name = "com.glaf.base.usertask.mapper.UserTaskMapper")
