...
@Controller("/identityTokenLogin")
@RequestMapping("/identityTokenLogin")
public class IdentityTokenLoginController {
	private static final Log logger = LogFactory.getLog(IdentityTokenLoginController.class);
	...
	public ModelAndView doLogin(HttpServletRequest request, HttpServletResponse response, ModelMap modelMap) {
		...
		if (StringUtils.isNotEmpty(account) && StringUtils.isNotEmpty(password)) {
			try {
				...
				request.setAttribute("identityToken", token);
		...
		if (bean == null) {
			...
			ContextUtil.put(bean.getActorId(), bean);// 传入全局变量
			...
			RequestUtils.setLoginUser(request, response, systemName, bean.getActorId());
					...
					try {
						...
						if (obj instanceof LoginCallback) {
							...
							if (StringUtils.equals(className, "com.glaf.shiro.ShiroLoginCallback")) {
								callback.afterLogin(bean.getActorId(), request, response);
							} else {
								LoginCallbackThread command = new LoginCallbackThread(callback, bean.getActorId(),
										request, response);
