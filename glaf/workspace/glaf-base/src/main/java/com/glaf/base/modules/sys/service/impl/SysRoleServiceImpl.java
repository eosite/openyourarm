...
public class SysRoleServiceImpl implements SysRoleService {
	...
	public boolean create(SysRole bean) {
		...
		if (SystemConfig.getBoolean("IDAutoIncrement")
				|| DBUtils.SQLSERVER.equals(DBConnectionFactory.getDatabaseType())) {
			sysRoleMapper.insertSysRole(bean);
		} else {
			sysRoleMapper.insertSysRoleWithId(bean);
		}
		return true;
	}
	...
	public PageResult getSysRoleList(int pageNo, int pageSize, SysRoleQuery query) {
		...
		query.setOrderBy(" E.TASKSORT asc ");
	...
	public List<SysRole> getSysRolesByAppId(long appId) {
		if (StringUtils.equals(DBUtils.ORACLE, DBConnectionFactory.getDatabaseType())) {
			return sysRoleMapper.getSysRolesByAppId_oracle(appId);
		} else if (StringUtils.equals(DBUtils.POSTGRESQL, DBConnectionFactory.getDatabaseType())) {
			return sysRoleMapper.getSysRolesByAppId_postgresql(appId);
		}
	...
	public void save(SysRole sysRole) {
		if (sysRole.getId() == 0) {
			...
			if (SystemConfig.getBoolean("IDAutoIncrement")
				...
				sysRoleMapper.insertSysRoleWithId(sysRole);
			}
		} else {
