 ...
 *
 */
public class ProjectJsonFactory {

	public static java.util.List<Project> arrayToList(JSONArray array) {
		java.util.List<Project> list = new java.util.ArrayList<Project>();
		for (int i = 0, len = array.size(); i < len; i++) {
			JSONObject jsonObject = array.getJSONObject(i);
			Project model = jsonToObject(jsonObject);
			list.add(model);
		}
		return list;
	}

	public static Project jsonToObject(JSONObject jsonObject) {
		...
		if (jsonObject.containsKey("accessors")) {
			...
			while (token.hasMoreTokens()) {
				...
				if (StringUtils.isNotEmpty(str)) {
					...
					access.setProjectId(model.getId());
		...
		if (jsonObject.containsKey("owners")) {
			...
			while (token.hasMoreTokens()) {
				...
				if (StringUtils.isNotEmpty(str)) {
					...
					owner.setProjectId(model.getId());
		...
		if (jsonObject.containsKey("subordinateIds")) {
			...
			while (token.hasMoreTokens()) {
				String str = token.nextToken();
				if (StringUtils.isNotEmpty(str)) {
					ProjectSubordinate sub = new ProjectSubordinate();
					sub.setProjectId(model.getId());
	...
	public static JSONArray listToArray(java.util.List<Project> list) {
	...
	public static JSONObject toJsonObject(Project model) {
		...
		if (model.getOwners() != null && !model.getOwners().isEmpty()) {
			StringBuilder buff = new StringBuilder();
			for (ProjectOwner owner : model.getOwners()) {
	...
	public static ObjectNode toObjectNode(Project model) {
