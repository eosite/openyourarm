...
@Controller("/mqlogin")
@RequestMapping("/mqlogin")
public class MQLoginController {
	protected static final Log logger = LogFactory.getLog(MQLoginController.class);
	...
	public ModelAndView login(HttpServletRequest request, HttpServletResponse response, ModelMap modelMap) {
		...
		if (StringUtils.isNotEmpty(userId) && StringUtils.isNotEmpty(token)) {
			LoginMessage loginMessage = loginMessageService.getLoginMessageByToken(token);
			if (loginMessage != null && loginMessage.getUserId() != null) {
				if (StringUtils.equals(userId, loginMessage.getUserId())) {
					long ts = System.currentTimeMillis() - loginMessage.getLoginTime();
					if (ts < 60000) {// 60秒内登录有效
						...
						// loginMessageService.deleteById(token);
					}
				}
			}
		}
