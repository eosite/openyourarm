...
@Controller("/rsa_validCode_login")
@RequestMapping("/rsa_validCode_login")
public class RSAValidateCodeLoginController {
	private static final Log logger = LogFactory.getLog(RSAValidateCodeLoginController.class);
	...
	public ModelAndView login(HttpServletRequest request, HttpServletResponse response, ModelMap modelMap) {
		...
		String validCode = request.getParameter("validCode");// 登录手机验证码，通过RSA公钥加密的十六进制字符串
		SysUser bean = null;
		byte[] data = null;
		String url = null;
		if (StringUtils.isNotEmpty(userId) && StringUtils.isNotEmpty(systemCode) && StringUtils.isNotEmpty(validCode)) {
			try {
				LoginModuleQuery query = new LoginModuleQuery();
				query.systemCode(systemCode);
				query.type("server_rsa");
				...
				if (list != null && !list.isEmpty()) {
					...
					data = loginModuleService.decryptText(loginModule.getId(), Hex.hex2byte(validCode));
					validCode = new String(data, "UTF-8");

					if (StringUtils.isNotEmpty(userId) && StringUtils.isNotEmpty(validCode)) {
						bean = authorizeService.authorizeByTelephone(userId);
						ValidateCodeLoginHandler handler = new ValidateCodeLoginHandler();
						if(!handler.checkCode(request, userId,validCode)){
							bean = null;
						}
