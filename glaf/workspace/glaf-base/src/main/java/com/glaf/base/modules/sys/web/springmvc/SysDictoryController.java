...
public class SysDictoryController {
	...
	public ModelAndView showDictData(HttpServletRequest request, ModelMap modelMap) {
		...
		if (!(parent == -1)) {
			...
			view = new ModelAndView("/modules/sys/dictory/dictory_select", modelMap);
		}
	...
	public ModelAndView showImport(HttpServletRequest request, ModelMap modelMap) {
		...
		// 显示列表页面
		return new ModelAndView("/modules/sys/dictory/showImport", modelMap);
	}
	...
	public byte[] sort(HttpServletRequest request) {
		int parent = RequestUtils.getInt(request, "parent");
		int id = RequestUtils.getInt(request, "id");
		int operate = RequestUtils.getInt(request, "operate");
