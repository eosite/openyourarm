...
public class SysUserResource {
	...
	public byte[] addRoleUser(@Context HttpServletRequest request, @Context UriInfo uriInfo) {
	...
	public byte[] data(@Context HttpServletRequest request, @RequestBody DataRequest dataRequest) throws IOException {
		...
		if (dataRequest.getFilter() != null) {
			if (dataRequest.getFilter().getField() != null) {
				logger.debug(dataRequest.getFilter().getLevel() + "->" + dataRequest.getFilter().getField() + "="
						+ dataRequest.getFilter().getValue());
			} else {
				logger.debug(dataRequest.getFilter().getLevel() + "->" + dataRequest.getFilter().getLogic());
			}
			...
			for (FilterDescriptor filter : filters) {
				...
				if (filter.getField() != null) {
					logger.debug(filter.getLevel() + "->" + filter.getField() + "=" + filter.getValue());
				} else {
				...
				for (FilterDescriptor f : subFilters) {
					logger.debug(f.getLevel() + "->" + f.getField() + "=" + f.getValue());
	...
	@Produces({ MediaType.APPLICATION_JSON })
	@ResponseBody
	public byte[] dataEx(@Context HttpServletRequest request, @RequestBody DataRequest dataRequest) throws IOException {
		// logger.debug("dataRequest:" + dataRequest);
		if (dataRequest.getFilter() != null) {
			if (dataRequest.getFilter().getField() != null) {
				logger.debug(dataRequest.getFilter().getLevel() + "->" + dataRequest.getFilter().getField() + "="
		...
		JSONObject result = new JSONObject();
		int total = sysUserService.getSysUserExCountByQueryCriteria(query);
		if (total > 0) {
			result.put("total", total);
			result.put("totalCount", total);
			result.put("totalRecords", total);
			...
			List<SysUser> list = sysUserService.getSysUsersExByQueryCriteria(start, limit, query);
			if (list != null && !list.isEmpty()) {
				JSONArray rowsJSON = new JSONArray();
				for (SysUser sysUser : list) {
					JSONObject rowJSON = sysUser.toJsonObject();
	...
	@ResponseBody
	@Produces({ MediaType.APPLICATION_JSON })
	public byte[] delRoleUser(@Context HttpServletRequest request, @Context UriInfo uriInfo) {
		RequestUtils.setRequestParameterToAttribute(request);
		int roleId = ParamUtil.getIntParameter(request, "roleId", 0);
	...
	@ResponseBody
	@Produces({ MediaType.APPLICATION_JSON })
	public byte[] resetPwd(@Context HttpServletRequest request, @Context UriInfo uriInfo) {
		RequestUtils.setRequestParameterToAttribute(request);
	...
	@ResponseBody
	@Produces({ MediaType.APPLICATION_JSON })
	public byte[] resetStatus(@Context HttpServletRequest request, @Context UriInfo uriInfo) {
		RequestUtils.setRequestParameterToAttribute(request);
		boolean ret = false;
		...
		if (ids != null && ids.length > 0) {
			try {
				...
				for (String id : ids) {
					userIds.add(RequestUtils.decodeString(id));
				}
				sysUserService.resetStatus(userIds, status);
				ret = true;
			} catch (Exception ex) {
				logger.error("batch update status error.", ex);
			}
		}
	...
	@ResponseBody
	@Produces({ MediaType.APPLICATION_JSON })
	public byte[] saveModifyInfo(@Context HttpServletRequest request, @Context UriInfo uriInfo) {
		RequestUtils.setRequestParameterToAttribute(request);
	...
	@ResponseBody
	@Produces({ MediaType.APPLICATION_JSON })
	public byte[] savePwd(@Context HttpServletRequest request, @Context UriInfo uriInfo) {
		RequestUtils.setRequestParameterToAttribute(request);
		SysUser bean = RequestUtil.getLoginUser(request);
		boolean ret = false;
	...
	@ResponseBody
	@Produces({ MediaType.APPLICATION_JSON })
	public byte[] saveRole(@Context HttpServletRequest request, @Context UriInfo uriInfo) {
		RequestUtils.setRequestParameterToAttribute(request);
	...
	@ResponseBody
	@Produces({ MediaType.APPLICATION_JSON })
	public byte[] userMenusJson(@Context HttpServletRequest request, @Context UriInfo uriInfo) throws IOException {
