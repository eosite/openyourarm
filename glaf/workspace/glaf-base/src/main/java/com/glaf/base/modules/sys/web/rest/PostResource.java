...
@Controller("/rs/sys/post")
@Path("/rs/sys/post")
// /rs/sys/postbatchDelete?rowIds=181819
public class PostResource {
	private static final Log logger = LogFactory.getLog(PostResource.class);
	...
	public byte[] detail(@Context HttpServletRequest request, @Context UriInfo uriInfo) throws IOException {
		...
		if (id > 0) {
			Post post = postService.findById(id);
			if (post != null) {
				return post.toJsonObject().toJSONString().getBytes("UTF-8");
			}
		}
		return null;
	}
	...
	@GET
	@POST
	@Path("deptRolePostTreeJson")
	@ResponseBody
	@Produces({ MediaType.APPLICATION_JSON })
	public byte[] deptRolePostTreeJson(@Context HttpServletRequest request, @Context UriInfo uriInfo)
			throws IOException {
		JSONObject result = new JSONObject();
		List<DeptRolePostTreeNode> deptRolePostTreeNodes = postService.getDeptRolePostTreeNodes();
		JSONArray rowsJSON = null;
		if (deptRolePostTreeNodes != null && !deptRolePostTreeNodes.isEmpty()) {
			rowsJSON = new JSONArray();
			for (DeptRolePostTreeNode deptRolePostTree : deptRolePostTreeNodes) {
				JSONObject rowJSON = deptRolePostTree.toJsonObject();
				if (deptRolePostTree.getPostId() == null) {
					rowJSON.put("nocheck", true);
				}
				rowsJSON.add(rowJSON);
			}
	...
	@ResponseBody
	@Produces({ MediaType.APPLICATION_JSON })
	public byte[] usersJson(@Context HttpServletRequest request, @Context UriInfo uriInfo) throws IOException {
		List<SysUser> postUsers = null;
		JSONObject result = new JSONObject();
		long postId = RequestUtils.getLong(request, "postId");
		if (postId > 0) {
			postUsers = postService.getSysUsers(postId);
		}
	...
	@ResponseBody
	@Produces({ MediaType.APPLICATION_JSON })
	public byte[] saveUsers(@Context HttpServletRequest request, @Context UriInfo uriInfo) throws IOException {
		long postId = RequestUtils.getLong(request, "postId");
		if (postId > 0) {
			...
			if (post != null) {
				...
				if (!actorIds.isEmpty()) {
					postService.saveUsers(postId, actorIds);
	...
	public byte[] sort(@Context HttpServletRequest request, @Context UriInfo uriInfo) {
		...
		postService.sort(postService.findById(id), operate);
		return ResponseUtils.responseResult(true);
	}
