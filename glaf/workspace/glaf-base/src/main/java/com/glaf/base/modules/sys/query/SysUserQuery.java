...
public class SysUserQuery extends DataQuery {
	...
	protected String deptCodeLike;
	protected String deptNameOrCode;
	protected Long sysRoleId;
	protected Long sysDeptId;
	protected String groupUserId;
	protected String groupLeaderId;
	protected List<String> accountsNotIn;
	...
	public List<String> getAccountsNotIn() {
		return accountsNotIn;
	}
	...
	public String getDeptCodeLike() {
		if (deptCodeLike != null && deptCodeLike.trim().length() > 0) {
			if (!deptCodeLike.startsWith("%")) {
				deptCodeLike = "%" + deptCodeLike;
			}
			if (!deptCodeLike.endsWith("%")) {
				deptCodeLike = deptCodeLike + "%";
			}
		}
		return deptCodeLike;
	...
	public String getDeptNameOrCode() {
		return deptNameOrCode;
	}
	...
	public void processDataRequest(DataRequest dataRequest) {
		if (dataRequest.getFilter() != null) {
			if (dataRequest.getFilter().getField() != null) {
				dataRequest.getFilter().setColumn(this.getColumnMap().get(dataRequest.getFilter().getField()));
				dataRequest.getFilter().setJavaType(this.getJavaTypeMap().get(dataRequest.getFilter().getField()));
			}
			...
			for (FilterDescriptor filter : filters) {
				...
				if (filter.getField() != null) {
					...
					filter.setJavaType(this.getJavaTypeMap().get(filter.getField()));
				}
	...
	public void setAccountsNotIn(List<String> accountsNotIn) {
		this.accountsNotIn = accountsNotIn;
	}
	...
	public void setDeptNameOrCode(String deptNameOrCode) {
		this.deptNameOrCode = deptNameOrCode;
	}
