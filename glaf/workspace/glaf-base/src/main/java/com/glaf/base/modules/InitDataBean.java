...
public class InitDataBean {
	...
	public void load(File file, InputStream inputStream) {
		...
		try {
			...
			if (tableName != null && elements != null) {
				...
				if (dataList.size() > 0) {
					...
					insertBean.bulkInsert(conn, tableDefinition, dataList);
				}

				if (StringUtils.equalsIgnoreCase(tableName, "SYS_KEY")) {
					updatable = "false";
				}

				if (StringUtils.equalsIgnoreCase(tableName, "SYS_DBID")) {
					updatable = "false";
				}

				if (StringUtils.equalsIgnoreCase(tableName, "SYS_PROPERTY")) {
					updatable = "false";
				}
				...
				if (dataList2.size() > 0 && StringUtils.equals(updatable, "true")) {
					...
					for (ColumnDefinition col : columns) {
						...
						} else if (StringUtils.equalsIgnoreCase(primaryKey, col.getColumnName())) {
							idColumn = col;
						} else {
							cols.add(col);
						}
					...
					tableDefinition.setColumns(cols);
					BatchUpdateBean updateBean = new BatchUpdateBean();
					updateBean.executeBatch(conn, tableDefinition, dataList2);
				}
			...
			ex.printStackTrace();
			logger.error("load xml error:" + file.getAbsolutePath());
		} finally {
			JdbcUtils.close(rs);
			JdbcUtils.close(psmt);
