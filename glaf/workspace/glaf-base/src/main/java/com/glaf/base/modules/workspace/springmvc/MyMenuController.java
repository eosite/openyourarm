...
public class MyMenuController {
	protected static final Log logger = LogFactory.getLog(MyMenuController.class);
	...
	public ModelAndView batchDelete(ModelMap modelMap, HttpServletRequest request) {
		boolean ret = true;
		...
		if (ret) {// 保存成功
			messages.add(ViewMessages.GLOBAL_MESSAGE, new ViewMessage("myMenu.delete_success"));
		} else {// 保存失败
			messages.add(ViewMessages.GLOBAL_MESSAGE, new ViewMessage("myMenu.delete_failure"));
		}
	...
	public ModelAndView prepareAdd(ModelMap modelMap, HttpServletRequest request) {
		...
		return new ModelAndView("/modules/workspace/mymenu/mymenu_add", modelMap);
	}
	...
	public ModelAndView prepareAddMyMenu(ModelMap modelMap, HttpServletRequest request) {
		RequestUtils.setRequestParameterToAttribute(request);
		...
		return new ModelAndView("/modules/workspace/mymenu/mymenu_addmymenu", modelMap);
	}
	...
	public ModelAndView prepareModify(ModelMap modelMap, HttpServletRequest request) {
		RequestUtils.setRequestParameterToAttribute(request);
		...
		// 显示修改页面
		return new ModelAndView("/modules/workspace/mymenu/mymenu_modify", modelMap);
	}
	...
	public ModelAndView saveAdd(ModelMap modelMap, MyMenuFormBean formBean, HttpServletRequest request) {
		...
		ViewMessages messages = new ViewMessages();
		if (ret) {// 保存成功
			messages.add(ViewMessages.GLOBAL_MESSAGE, new ViewMessage("myMenu.add_success"));
		} else {// 保存失败
			messages.add(ViewMessages.GLOBAL_MESSAGE, new ViewMessage("myMenu.add_failure"));
		}
		MessageUtils.addMessages(request, messages);
		...
		if (showList == 1) {// 添加到我的菜单
			...
			PageResult pager = myMenuService.getMyMenuList(userId, pageNo, pageSize);
			request.setAttribute("pager", pager);

			return new ModelAndView("/modules/workspace/mymenu/mymenu_list", modelMap);
		}
		return new ModelAndView("/show_msg", modelMap);
	}
	...
	public ModelAndView saveModify(ModelMap modelMap, MyMenuFormBean formBean, HttpServletRequest request) {
		...
		ViewMessages messages = new ViewMessages();
		if (ret) {// 保存成功
			messages.add(ViewMessages.GLOBAL_MESSAGE, new ViewMessage("myMenu.modify_success"));
		} else {// 保存失败
			messages.add(ViewMessages.GLOBAL_MESSAGE, new ViewMessage("myMenu.modify_failure"));
		}
		MessageUtils.addMessages(request, messages);
		// 显示提交后页面
		return new ModelAndView("/show_msg", modelMap);
