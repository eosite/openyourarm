...
public class BaseIdentityFactory {
	...
	public static SysDepartment getDepartmentById(Long id) {
		String cacheKey = "cache_sys_dept_" + id;
		String text = CacheFactory.getString("dept", cacheKey);
		if (text != null) {
			try {
		...
		if (model != null) {
			CacheFactory.put("dept", cacheKey, model.toJsonObject().toJSONString());
		}
		return model;
	}
	...
	public static SysDepartment getDepartmentByNo(String deptno) {
		String cacheKey = "cache_sys_deptno_" + deptno;
		String text = CacheFactory.getString("dept", cacheKey);
		if (text != null) {
			try {
				JSONObject jsonObject = JSON.parseObject(text);
	...
	public static JSONArray getRoleMenu(String roleCode) {
		...
		if (StringUtils.isNotEmpty(serviceUrl)
				&& (StringUtils.startsWith(serviceUrl, "http://") || StringUtils.startsWith(serviceUrl, "https://"))) {
	...
	public static List<String> getUserRoleCodes(List<String> actorIds) {
		...
		buffer.append("cache_sys_userrole_str");
	...
	public static List<SysRole> getUserRoles(List<String> actorIds) {
		...
		StringBuilder buffer = new StringBuilder();
		buffer.append("cache_sys_userrole");
		for (String actorId : actorIds) {
			buffer.append("_").append(actorId);
		}
	...
	public static void main(String[] args) {
		System.setProperty("spring-config-file", "E:/isdp/workspace/glaf-base/conf/spring/spring-config.xml");
		System.out.println(System.getProperty("spring-config-file"));
		System.out
				.println(BaseIdentityFactory.getRoleMenu("http://192.168.1.1:8080/glaf", "BranchAdmin").toString('\n'));
	}
