...
@Controller
@Path("/rs/sys/loginModule")
public class LoginModuleResource {
	protected static final Log logger = LogFactory.getLog(LoginModuleResource.class);
	...
	@ResponseBody
	@Produces({ MediaType.APPLICATION_JSON })
	public byte[] resetAppSecret(@Context HttpServletRequest request) {
		try {
			LoginModule loginModule = null;
			if (StringUtils.isNotEmpty(request.getParameter("id"))) {
				loginModule = loginModuleService.getLoginModule(request.getParameter("id"));
	...
	@POST
	@Path("/resetLoginRSAKey")
	@ResponseBody
	@Produces({ MediaType.APPLICATION_JSON })
	public byte[] resetLoginRSAKey(@Context HttpServletRequest request) {
		try {
			LoginModule loginModule = null;
			if (StringUtils.isNotEmpty(request.getParameter("id"))) {
				loginModule = loginModuleService.getLoginModule(request.getParameter("id"));
				if (StringUtils.equals(loginModule.getType(), "client_rsa")
					...
					loginModuleService.resetLoginRSAKey(loginModule.getId());
					return ResponseUtils.responseJsonResult(true);
				}
			}
		} catch (Exception ex) {
	...
	@POST
	@Path("/saveLoginModule")
	@ResponseBody
	@Produces({ MediaType.APPLICATION_JSON })
	public byte[] saveLoginModule(@Context HttpServletRequest request) {
		Map<String, Object> params = RequestUtils.getParameterMap(request);
	...
	public byte[] view(@Context HttpServletRequest request) throws IOException {
		...
		JSONObject result = new JSONObject();
		if (loginModule != null) {
			result = loginModule.toJsonObject();
			result.put("id", loginModule.getId());
			result.put("loginModuleId", loginModule.getId());
		}
		return result.toJSONString().getBytes("UTF-8");
	}
}
