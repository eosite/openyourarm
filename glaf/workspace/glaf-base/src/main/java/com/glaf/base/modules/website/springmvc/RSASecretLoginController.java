...
@Controller("/rsa_secret_login")
@RequestMapping("/rsa_secret_login")
public class RSASecretLoginController {
	private static final Log logger = LogFactory.getLog(RSASecretLoginController.class);
	...
	public ModelAndView login(HttpServletRequest request, HttpServletResponse response, ModelMap modelMap) {
		...
		String secret = request.getParameter("secret");// 登录密钥，通过RSA公钥加密的十六进制字符串
		SysUser bean = null;
		byte[] data = null;
		String url = null;
		if (StringUtils.isNotEmpty(userId) && StringUtils.isNotEmpty(systemCode) && StringUtils.isNotEmpty(secret)) {
			try {
				LoginModuleQuery query = new LoginModuleQuery();
				query.systemCode(systemCode);
				query.type("server_rsa");
				...
				if (list != null && !list.isEmpty()) {
					...
					data = loginModuleService.decryptText(loginModule.getId(), Hex.hex2byte(secret));
					secret = new String(data, "UTF-8");

					if (StringUtils.isNotEmpty(userId) && StringUtils.isNotEmpty(secret)) {
						// bean = authorizeService.login(userId, pwd);
						bean = authorizeService.authorizeLoginSecret(userId, secret);
