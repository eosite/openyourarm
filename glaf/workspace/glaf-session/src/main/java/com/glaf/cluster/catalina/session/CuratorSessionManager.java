...
public class CuratorSessionManager extends ManagerBase implements Lifecycle {
	...
	protected String servers = "127.0.0.1:2181";

	protected int timeout = 3600;

	private static final String GROUP_NAME = "/tomcat_sessions";

	protected byte[] NULL_SESSION = "null".getBytes();

	protected CuratorSessionHandlerValve handlerValve;
	protected FastThreadLocal<ZooKeeperSession> currentSession = new FastThreadLocal<ZooKeeperSession>();
	...
	public void afterRequest() {
		...
		if (zooKeeperSession != null) {
			...
			log.trace("Session removed from ThreadLocal :" + zooKeeperSession.getIdInternal());
		}
	}
	...
	public Session createSession(String sessionId) {
		...
		try {
			...
			zkClient.create().withMode(CreateMode.PERSISTENT).forPath(path, NULL_SESSION);
	...
	public ZooKeeperSession loadSessionFromZooKeeper(String id) throws IOException {
	...
	public void save(Session session) {
		try {
			...
			if (sessionIsDirty || currentSessionIsPersisted.get() != true) {
				...
				byte[] data = serializer.serializeFrom(zooKeeperSession);
				for (int i = 0; i < 3; i++) {
					try {
						if (zkClient.checkExists().forPath(path) == null) {
	...
	protected synchronized void startInternal() throws LifecycleException {
		...
		for (int i = 0; i < 5; i++) {
			try {
				if (zkClient.checkExists().forPath(GROUP_NAME) == null) {
